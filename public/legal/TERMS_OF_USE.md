...
TO THE MAXIMUM EXTENT PERMITTED BY LAW, STRUCTOUT’S TOTAL LIABILITY FOR ANY CLAIM WILL NOT EXCEED **US $0.00**. WE SHALL NOT BE LIABLE FOR INDIRECT, INCIDENTAL, SPECIAL, OR CONSEQUENTIAL DAMAGES.
...
1. **Required Skills** Using the downloadable StructOut bundle assumes familiarity with:
  * a code editor or IDE (e.g., Visual Studio Code, PyCharm);
  * Python 3.12 or later and virtual‑environment tools;
  * running command‑line scripts such as `python secure_key.py` and GUI helpers like `python secure_key_gui.py`.
