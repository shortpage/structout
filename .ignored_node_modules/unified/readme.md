...
Those programs, called _[plugins][api-plugin]_, take these trees and inspect and
modify them.
To get to the syntax tree from text, there is a _[parser][api-parser]_.
To get from that back to text, there is a _[compiler][api-compiler]_.
This is the _[process][api-process]_ of a _processor_.
...
module users.
Those processors are _[frozen][api-freeze]_ and they should be called to create
...
**Bridge** mode transforms the tree from one format (_origin_) to another
(_destination_).
...
New _[unfrozen][api-freeze]_ processor ([`processor`][api-processor]).
...
> 👉 **Note**: setting information cannot occur on _[frozen][api-freeze]_
> processors.
...
Run _[transformers][api-transformer]_ on a syntax tree.
...
> 👉 **Note**: `use` cannot be called on [_frozen_][api-freeze] processors.
> Call the processor first to create a new unfrozen processor.
...
> 👉 **Note**: plugins are called when the processor is _frozen_, not when they
> are applied.
...
) => Promise<Output | undefined> | Output | Error | undefined
```
