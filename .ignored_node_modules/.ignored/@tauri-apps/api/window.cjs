...
async function getAllWindows() {
  return core.invoke("plugin:window|get_all_windows").then((windows) =>
    windows.map(
      (w) =>
        new Window(w, {
          // @ts-expect-error `skip` is not defined in the public API but it is handled by the constructor
...
class Window {
  ...
  async innerPosition() {
    return core
      .invoke("plugin:window|inner_position", {
        label: this.label,
      })
  ...
  async outerPosition() {
    return core
      .invoke("plugin:window|outer_position", {
        label: this.label,
      })
      .then((p) => new dpi.PhysicalPosition(p));
  }
  ...
  async outerSize() {
    return core
      .invoke("plugin:window|outer_size", {
        label: this.label,
      })
      .then((s) => new dpi.PhysicalSize(s));
  }
  ...
  async setPosition(position) {
    return core.invoke("plugin:window|set_position", {
      label: this.label,
      value:
        position instanceof dpi.Position
          ? position
          : new dpi.Position(position),
    });
  }
  /**
  ...
  async onCloseRequested(handler) {
    ...
    return this.listen(
      event.TauriEvent.WINDOW_CLOSE_REQUESTED,
      async (event) => {
...
async function monitorFromPoint(x, y) {
  return core
    .invoke("plugin:window|monitor_from_point", {
      x,
      y,
    })
    .then(mapMonitor);
...
async function cursorPosition() {
  return core
    .invoke("plugin:window|cursor_position")
    .then((v) => new dpi.PhysicalPosition(v));
}
