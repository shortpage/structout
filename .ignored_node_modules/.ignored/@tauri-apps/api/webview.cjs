...
async function getAllWebviews() {
  return core.invoke("plugin:webview|get_all_webviews").then((webviews) =>
    webviews.map(
      (w) =>
        new Webview(
          new window$1.Window(w.windowLabel, {
            // @ts-expect-error `skip` is not defined in the public API but it is handled by the constructor
            skip: true,
          }),
          w.label,
...
class Webview {
  ...
  async position() {
    return core
      .invoke("plugin:webview|webview_position", {
        label: this.label,
      })
  ...
  async size() {
    return core
      .invoke("plugin:webview|webview_size", {
        label: this.label,
      })
