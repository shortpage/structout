...
const useUtilityClasses = (ownerState) => {
  ...
  return (0, _composeClasses.default)(
    slots,
    _hiddenCssClasses.getHiddenCssUtilityClass,
    classes,
  );
};
...
function HiddenCss(props) {
  ...
  if (process.env.NODE_ENV !== "production") {
    const unknownProps = Object.keys(other).filter((propName) => {
      const isUndeclaredBreakpoint = !theme.breakpoints.keys.some(
        (breakpoint) => {
          return (
            `${breakpoint}Up` === propName || `${breakpoint}Down` === propName
          );
        },
      );
      return (
        !["classes", "theme", "isRtl", "sx"].includes(propName) &&
        isUndeclaredBreakpoint
      );
    });
