...
var _areArraysEqual = _interopRequireDefault(
  require("../utils/areArraysEqual"),
);
...
function useSlider(parameters) {
  ...
  values = values.map((value) =>
    value == null ? min : (0, _utils.clamp)(value, min, max),
  );
  const marks =
  ...
  const handleFocusRef = (0, _utils.unstable_useForkRef)(
    focusVisibleRef,
    sliderRef,
  );
  ...
  const handleTouchMove = (0, _utils.unstable_useEventCallback)(
    (nativeEvent) => {
  ...
  const handleTouchEnd = (0, _utils.unstable_useEventCallback)(
    (nativeEvent) => {
      const finger = trackFinger(nativeEvent, touchId);
      setDragging(false);
      if (!finger) {
  ...
  const handleTouchStart = (0, _utils.unstable_useEventCallback)(
    (nativeEvent) => {
      if (disabled) {
        return;
      }
  ...
  const getRootProps = (externalProps = {}) => {
    ...
    const mergedEventHandlers = (0, _extends2.default)(
      {},
      externalHandlers,
      ownEventHandlers,
    );
