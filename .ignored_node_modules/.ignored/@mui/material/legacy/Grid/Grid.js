...
function getOffset(val) {
  ...
  return ""
    .concat(parse)
    .concat(String(val).replace(String(parse), "") || "px");
}
export function generateGrid(_ref) {
  ...
  return theme.breakpoints.keys.reduce(function (globalStyles, breakpoint) {
    ...
    if (size === true) {
      ...
      var width = "".concat(
        Math.round((size / columnValue) * 10e7) / 10e5,
        "%",
      );
      var more = {};
      if (
...
function extractZeroValueBreakpointKeys(_ref3) {
  ...
  var sortedBreakpointKeysByValue = Object.keys(breakpoints).sort(
    function (a, b) {
...
export function resolveSpacingStyles(spacing, breakpoints) {
  ...
  breakpoints.forEach(function (breakpoint) {
    ...
    if (Number(value) > 0) {
      spacingStyles.push(
        styles["spacing-".concat(breakpoint, "-").concat(String(value))],
      );
    }
  });
    ...
    breakpoints.forEach(function (breakpoint) {
      ...
      if (value) {
        breakpointsStyles.push(
          styles["grid-".concat(breakpoint, "-").concat(String(value))],
        );
      }
    });
    return [
      ...
      [
        direction !== "row" &&
          styles["direction-xs-".concat(String(direction))],
...
var useUtilityClasses = function useUtilityClasses(ownerState) {
  ...
  breakpoints.forEach(function (breakpoint) {
    var value = ownerState[breakpoint];
    if (value) {
      breakpointsClasses.push(
        "grid-".concat(breakpoint, "-").concat(String(value)),
      );
    }
  });
  var slots = {
