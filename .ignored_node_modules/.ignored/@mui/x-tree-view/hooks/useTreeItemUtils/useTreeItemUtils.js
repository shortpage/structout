...
const useTreeItemUtils = ({ itemId, children }) => {
  const { instance, store, publicAPI } = (0,
  _TreeViewProvider.useTreeViewContext)();
  const isItemExpandable = (0, _useSelector.useSelector)(
    store,
    _useTreeViewExpansion.selectorIsItemExpandable,
    itemId,
  );
  const isLazyLoadingEnabled = (0, _useSelector.useSelector)(
    store,
    _useTreeViewLazyLoading.selectorIsLazyLoadingEnabled,
  );
  const isMultiSelectEnabled = (0, _useSelector.useSelector)(
    store,
    _useTreeViewSelection.selectorIsMultiSelectEnabled,
  );
  const loading = (0, _useSelector.useSelector)(store, (state) =>
    isLazyLoadingEnabled
      ? (0, _useTreeViewLazyLoading.selectorIsItemLoading)(state, itemId)
      : false,
  );
  const error = (0, _useSelector.useSelector)(store, (state) =>
    isLazyLoadingEnabled
      ? Boolean(
          (0, _useTreeViewLazyLoading.selectorGetTreeItemError)(state, itemId),
        )
      : false,
  );
  const isExpandable = itemHasChildren(children) || isItemExpandable;
  const isExpanded = (0, _useSelector.useSelector)(
    store,
    _useTreeViewExpansion.selectorIsItemExpanded,
    itemId,
  );
  const isFocused = (0, _useSelector.useSelector)(
    store,
    _useTreeViewFocus.selectorIsItemFocused,
    itemId,
  );
  const isSelected = (0, _useSelector.useSelector)(
    store,
    _useTreeViewSelection.selectorIsItemSelected,
    itemId,
  );
  const isDisabled = (0, _useSelector.useSelector)(
    store,
    _useTreeViewItems.selectorIsItemDisabled,
    itemId,
  );
  const isEditing = (0, _useSelector.useSelector)(
    store,
    _useTreeViewLabel2.selectorIsItemBeingEdited,
    itemId,
  );
  const isEditable = (0, _useSelector.useSelector)(
    store,
    _useTreeViewLabel2.selectorIsItemEditable,
    itemId,
  );
  const status = {
  ...
  const handleExpansion = (event) => {
    ...
    if (
      ...
      !(
        multiple &&
        (0, _useTreeViewExpansion.selectorIsItemExpanded)(store.value, itemId)
      )
    ) {
  ...
  const toggleItemEditing = () => {
    if (
      !(0, _plugins.hasPlugin)(instance, _useTreeViewLabel.useTreeViewLabel)
    ) {
      return;
    }
    if (isEditing) {
