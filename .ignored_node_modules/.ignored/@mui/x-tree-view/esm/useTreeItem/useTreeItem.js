...
export const useTreeItem = (parameters) => {
  ...
  const isSelectionEnabledForItem = useSelector(
    store,
    selectorIsItemSelectionEnabled,
    itemId,
  );
  const isCheckboxSelectionEnabled = useSelector(
    store,
    selectorIsCheckboxSelectionEnabled,
  );
  ...
  const createRootHandleBlur = (otherHandlers) => (event) => {
    ...
    if (
      ...
      (event.relatedTarget &&
        isTargetInDescendants(event.relatedTarget, rootElement) &&
        ((event.target &&
          event.target?.dataset?.element === "labelInput" &&
          isTargetInDescendants(event.target, rootElement)) ||
  ...
  const getRootProps = (externalProps = {}) => {
    ...
    const enhancedRootProps =
      propsEnhancers.root?.(
        _extends({}, sharedPropsEnhancerParams, {
          externalEventHandlers,
        }),
      ) ?? {};
