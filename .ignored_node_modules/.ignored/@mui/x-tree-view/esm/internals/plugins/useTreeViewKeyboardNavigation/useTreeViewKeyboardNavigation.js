...
} from "../useTreeViewItems/useTreeViewItems.selectors.js";
import {
  selectorIsItemBeingEdited,
  selectorIsItemEditable,
} from "../useTreeViewLabel/useTreeViewLabel.selectors.js";
import {
  selectorIsItemSelected,
  selectorIsMultiSelectEnabled,
  selectorIsSelectionEnabled,
} from "../useTreeViewSelection/useTreeViewSelection.selectors.js";
import {
  selectorIsItemExpandable,
  selectorIsItemExpanded,
} from "../useTreeViewExpansion/useTreeViewExpansion.selectors.js";
function isPrintableKey(string) {
  return !!string && string.length === 1 && !!string.match(/\S/);
}
export const useTreeViewKeyboardNavigation = ({ instance, store, params }) => {
  const isRtl = useRtl();
  ...
  const canToggleItemSelection = (itemId) =>
    ...
    !selectorIsItemDisabled(store.value, itemId);
  ...
  const handleItemKeyDown = async (event, itemId) => {
    ...
    if (
      event.altKey ||
      isTargetInDescendants(event.target, event.currentTarget)
    ) {
      return;
    }
