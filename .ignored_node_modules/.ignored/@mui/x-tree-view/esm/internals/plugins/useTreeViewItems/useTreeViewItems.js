...
} from "./useTreeViewItems.utils.js";
...
import {
  ...
  selectorItemModel,
  selectorItemDepth,
} from "./useTreeViewItems.selectors.js";
...
export const useTreeViewItems = ({ instance, params, store }) => {
  const getItem = React.useCallback(
    (itemId) => selectorItemModel(store.value, itemId),
    [store],
  );
  ...
  const setIsItemDisabled = useEventCallback(({ itemId, shouldBeDisabled }) => {
    store.update((prevState) => {
  ...
  const getItemTree = React.useCallback(() => {
    ...
    return selectorItemOrderedChildrenIds(store.value, null).map(
      getItemFromItemId,
    );
  }, [store]);
  const getItemOrderedChildrenIds = React.useCallback(
    (itemId) => selectorItemOrderedChildrenIds(store.value, itemId),
    [store],
  );
