...
export const useTreeViewBuildContext = (parameters) => {
  ...
  const runItemPlugins = React.useCallback(
    (itemPluginProps) => {
      ...
      plugins.forEach((plugin) => {
        ...
        if (itemPluginResponse?.propsEnhancers) {
          ...
          Object.keys(itemPluginResponse.propsEnhancers).forEach(
            (propsEnhancerName) => {
      ...
      const resolvePropsEnhancer = (currentSlotName) => (currentSlotParams) => {
        ...
        pluginPropEnhancers.forEach((propsEnhancersForCurrentPlugin) => {
          ...
          if (propsEnhancerForCurrentPluginAndSlot != null) {
            Object.assign(
              enhancedProps,
              propsEnhancerForCurrentPluginAndSlot(currentSlotParams),
            );
          }
        });
      ...
      const propsEnhancers = Object.fromEntries(
        Object.keys(pluginPropEnhancersNames).map((propEnhancerName) => [
          propEnhancerName,
