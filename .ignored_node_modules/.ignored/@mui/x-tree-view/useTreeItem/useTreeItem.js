...
var _extractEventHandlers = _interopRequireDefault(
  require("@mui/utils/extractEventHandlers"),
);
...
const useTreeItem = (parameters) => {
  const { runItemPlugins, instance, publicAPI, store } = (0,
  _TreeViewProvider.useTreeViewContext)();
  const depthContext = React.useContext(
    _TreeViewItemDepthContext.TreeViewItemDepthContext,
  );
  const depth = (0, _useSelector.useSelector)(
    store,
    (...params) => {
  ...
  const { interactions, status } = (0, _useTreeItemUtils.useTreeItemUtils)({
    itemId,
    children,
  });
  const rootRefObject = React.useRef(null);
  const contentRefObject = React.useRef(null);
  const handleRootRef = (0, _useForkRef.default)(
    rootRef,
    pluginRootRef,
    rootRefObject,
  );
  const handleContentRef = (0, _useForkRef.default)(
    contentRef,
    contentRefObject,
  );
  const checkboxRef = React.useRef(null);
  const treeId = (0, _useSelector.useSelector)(
    store,
    _useTreeViewId2.selectorTreeViewId,
  );
  const isSelectionEnabledForItem = (0, _useSelector.useSelector)(
    store,
    _useTreeViewSelection.selectorIsItemSelectionEnabled,
    itemId,
  );
  const isCheckboxSelectionEnabled = (0, _useSelector.useSelector)(
    store,
    _useTreeViewSelection.selectorIsCheckboxSelectionEnabled,
  );
  ...
  const shouldBeAccessibleWithTab = (0, _useSelector.useSelector)(
    store,
    _useTreeViewFocus.selectorIsItemTheDefaultFocusableItem,
    itemId,
  );
  ...
  const createRootHandleFocus = (otherHandlers) => (event) => {
    ...
    if (
      !status.focused &&
      (0, _useTreeViewItems.selectorCanItemBeFocused)(store.value, itemId) &&
  ...
  const createRootHandleBlur = (otherHandlers) => (event) => {
    ...
    if (
      ...
      (event.relatedTarget &&
        (0, _tree.isTargetInDescendants)(event.relatedTarget, rootElement) &&
        ((event.target &&
          event.target?.dataset?.element === "labelInput" &&
          (0, _tree.isTargetInDescendants)(event.target, rootElement)) ||
          event.relatedTarget?.dataset?.element === "labelInput"))
    ) {
      return;
    }
  ...
  const createContentHandleClick = (otherHandlers) => (event) => {
    ...
    if (
      (0, _useTreeViewExpansion.selectorItemExpansionTrigger)(store.value) ===
      "content"
    ) {
  ...
  const getRootProps = (externalProps = {}) => {
    ...
    const enhancedRootProps =
      propsEnhancers.root?.(
        (0, _extends2.default)({}, sharedPropsEnhancerParams, {
          externalEventHandlers,
        }),
      ) ?? {};
  ...
  const getContentProps = (externalProps = {}) => {
    const externalEventHandlers = (0, _extractEventHandlers.default)(
      externalProps,
    );
    const props = (0, _extends2.default)(
      {},
