...
exports.isTargetInDescendants =
  exports.getPreviousNavigableItem =
  exports.getNonDisabledItemsInRange =
  exports.getNextNavigableItem =
  exports.getLastNavigableItem =
  exports.getFirstNavigableItem =
  exports.getAllNavigableItems =
  exports.findOrderInTremauxTree =
    void 0;
...
const getLastNavigableItemInArray = (state, items) => {
  ...
  while (
    itemIndex >= 0 &&
    !(0, _useTreeViewItems.selectorCanItemBeFocused)(state, items[itemIndex])
  ) {
    itemIndex -= 1;
  }
  if (itemIndex === -1) {
...
const getPreviousNavigableItem = (state, itemId) => {
  ...
  const siblings = (0, _useTreeViewItems.selectorItemOrderedChildrenIds)(
    state,
    itemMeta.parentId,
  );
  ...
  while (
    !(0, _useTreeViewItems.selectorCanItemBeFocused)(
      state,
      siblings[previousNavigableSiblingIndex],
    ) &&
  ...
  let lastNavigableChild = getLastNavigableItemInArray(
    state,
    (0, _useTreeViewItems.selectorItemOrderedChildrenIds)(state, currentItemId),
  );
  while (
    (0, _useTreeViewExpansion.selectorIsItemExpanded)(state, currentItemId) &&
    lastNavigableChild != null
  ) {
...
const getNextNavigableItem = (state, itemId) => {
  ...
  if ((0, _useTreeViewExpansion.selectorIsItemExpanded)(state, itemId)) {
    const firstNavigableChild = (0,
    _useTreeViewItems.selectorItemOrderedChildrenIds)(state, itemId).find(
      (childId) =>
        (0, _useTreeViewItems.selectorCanItemBeFocused)(state, childId),
    );
  ...
  while (itemMeta != null) {
    ...
    const currentItemIndex = (0, _useTreeViewItems.selectorItemIndex)(
      state,
      itemMeta.id,
    );
    ...
    itemMeta = (0, _useTreeViewItems.selectorItemMeta)(
      state,
      itemMeta.parentId,
    );
  }
...
const getLastNavigableItem = (state) => {
  ...
  while (
    itemId == null ||
    (0, _useTreeViewExpansion.selectorIsItemExpanded)(state, itemId)
  ) {
...
const getFirstNavigableItem = (state) =>
  (0, _useTreeViewItems.selectorItemOrderedChildrenIds)(state, null).find(
    (itemId) => (0, _useTreeViewItems.selectorCanItemBeFocused)(state, itemId),
  );
...
const findOrderInTremauxTree = (state, itemAId, itemBId) => {
  ...
  if (
    itemMetaA.parentId === itemMetaB.id ||
    itemMetaB.parentId === itemMetaA.id
  ) {
  ...
  while (!bAncestorIsCommon && !aAncestorIsCommon) {
    if (continueA) {
      ...
      if (!aAncestorIsCommon && continueA) {
        aAncestor = (0, _useTreeViewItems.selectorItemParentId)(
          state,
          aAncestor,
        );
      }
    ...
    if (continueB && !aAncestorIsCommon) {
      ...
      if (!bAncestorIsCommon && continueB) {
        bAncestor = (0, _useTreeViewItems.selectorItemParentId)(
          state,
          bAncestor,
        );
      }
  ...
  const ancestorFamily = (0, _useTreeViewItems.selectorItemOrderedChildrenIds)(
    state,
    commonAncestor,
  );
...
const getNonDisabledItemsInRange = (state, itemAId, itemBId) => {
  const getNextItem = (itemId) => {
    ...
    if (
      (0, _useTreeViewExpansion.selectorIsItemExpandable)(state, itemId) &&
      (0, _useTreeViewExpansion.selectorIsItemExpanded)(state, itemId)
    ) {
      return (0, _useTreeViewItems.selectorItemOrderedChildrenIds)(
        state,
        itemId,
      )[0];
    }
    ...
    while (itemMeta != null) {
      ...
      itemMeta = itemMeta.parentId
        ? (0, _useTreeViewItems.selectorItemMeta)(state, itemMeta.parentId)
        : null;
    }
