...
const useTreeViewExpansion = ({ instance, store, params }) => {
  ...
  (0, _useEnhancedEffect.default)(() => {
    store.update((prevState) => {
      const newExpansionTrigger = (0,
      _useTreeViewExpansion2.getExpansionTrigger)({
  ...
  const applyItemExpansion = (0, _useEventCallback.default)(
    ({ itemId, event, shouldBeExpanded }) => {
      const oldExpanded = (0, _useTreeViewExpansion.selectorExpandedItems)(
        store.value,
      );
      let newExpanded;
      if (shouldBeExpanded) {
  ...
  const setItemExpansion = (0, _useEventCallback.default)(
    ({ itemId, event = null, shouldBeExpanded }) => {
      const isExpandedBefore = (0,
      _useTreeViewExpansion.selectorIsItemExpanded)(store.value, itemId);
  ...
  const expandAllSiblings = (event, itemId) => {
    const itemMeta = (0, _useTreeViewItems.selectorItemMeta)(
      store.value,
      itemId,
    );
    if (itemMeta == null) {
    ...
    const diff = siblings.filter(
      (child) =>
        (0, _useTreeViewExpansion.selectorIsItemExpandable)(
          store.value,
          child,
        ) &&
        !(0, _useTreeViewExpansion.selectorIsItemExpanded)(store.value, child),
    );
    const newExpanded = (0, _useTreeViewExpansion.selectorExpandedItems)(
      store.value,
    ).concat(diff);
    if (diff.length > 0) {
      if (params.onItemExpansionToggle) {
...
useTreeViewExpansion.getInitialState = (params) => ({
  expansion: {
    expandedItems:
      params.expandedItems === undefined
