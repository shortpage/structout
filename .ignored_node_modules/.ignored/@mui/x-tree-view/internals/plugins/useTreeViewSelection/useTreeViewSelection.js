...
const useTreeViewSelection = ({ store, params }) => {
  ...
  const setSelectedItems = (event, newModel, additionalItemsToPropagate) => {
    const oldModel = (0, _useTreeViewSelection2.selectorSelectionModel)(
      store.value,
    );
    let cleanModel;
    const isMultiSelectEnabled = (0,
    _useTreeViewSelection2.selectorIsMultiSelectEnabled)(store.value);
    if (
  ...
  const setItemSelection = ({
    ...
    if (keepExistingSelection) {
      const oldSelected = (0,
      _useTreeViewSelection2.selectorSelectionModelArray)(store.value);
      const isSelectedBefore = (0,
      _useTreeViewSelection2.selectorIsItemSelected)(store.value, itemId);
      if (
        isSelectedBefore &&
        (shouldBeSelected === false || shouldBeSelected == null)
      ) {
        newSelected = oldSelected.filter((id) => id !== itemId);
      } else if (
        !isSelectedBefore &&
        (shouldBeSelected === true || shouldBeSelected == null)
      ) {
      ...
      if (
        ...
        (shouldBeSelected == null &&
          (0, _useTreeViewSelection2.selectorIsItemSelected)(
            store.value,
            itemId,
          ))
      ) {
  ...
  const selectRange = (event, [start, end]) => {
    ...
    _useTreeViewSelection2.selectorSelectionModelArray)(store.value).slice();
    ...
    const selectedItemsLookup = (0, _useTreeViewSelection.getLookupFromArray)(
      newSelectedItems,
    );
    ...
    lastSelectedRange.current = (0, _useTreeViewSelection.getLookupFromArray)(
      range,
    );
  };
...
useTreeViewSelection.itemPlugin =
  _useTreeViewSelection3.useTreeViewSelectionItemPlugin;
