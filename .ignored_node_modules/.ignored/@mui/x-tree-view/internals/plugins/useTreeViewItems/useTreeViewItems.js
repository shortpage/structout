...
var _useEventCallback = _interopRequireDefault(
  require("@mui/utils/useEventCallback"),
);
...
const processItemsLookups = ({
  ...
  const processItem = (item, depth, parentId) => {
    ...
    const parentIdWithDefault =
      parentId ?? _useTreeViewItems.TREE_VIEW_ROOT_PARENT_ID;
  ...
  Object.keys(itemOrderedChildrenIdsLookup).forEach((parentId) => {
    ...
    _useTreeViewItems.buildSiblingIndexes)(
...
const useTreeViewItems = ({ instance, params, store }) => {
  const getItem = React.useCallback(
    (itemId) => (0, _useTreeViewItems2.selectorItemModel)(store.value, itemId),
    [store],
  );
  const getParentId = React.useCallback(
    (itemId) => {
      const itemMeta = (0, _useTreeViewItems2.selectorItemMeta)(
        store.value,
        itemId,
      );
  ...
  const setIsItemDisabled = (0, _useEventCallback.default)(
    ({ itemId, shouldBeDisabled }) => {
      store.update((prevState) => {
        ...
        const itemMetaLookup = (0, _extends2.default)(
          {},
          prevState.items.itemMetaLookup,
        );
        itemMetaLookup[itemId] = (0, _extends2.default)(
          {},
  ...
  const getItemTree = React.useCallback(() => {
    const getItemFromItemId = (itemId) => {
      const item = (0, _useTreeViewItems2.selectorItemModel)(
        store.value,
        itemId,
      );
      const newChildren = (0,
      _useTreeViewItems2.selectorItemOrderedChildrenIds)(store.value, itemId);
    ...
    return (0, _useTreeViewItems2.selectorItemOrderedChildrenIds)(
      store.value,
      null,
    ).map(getItemFromItemId);
  }, [store]);
  const getItemOrderedChildrenIds = React.useCallback(
    (itemId) =>
      (0, _useTreeViewItems2.selectorItemOrderedChildrenIds)(
        store.value,
        itemId,
      ),
    [store],
  ...
  const areItemUpdatesPrevented = React.useCallback(
  ...
  const addItems = ({ items, parentId, depth, getChildrenCount }) => {
    if (items) {
      ...
      store.update((prevState) => {
        ...
        if (parentId) {
          newItems = {
            itemModelLookup: (0, _extends2.default)(
              {},
              prevState.items.itemModelLookup,
              newState.itemModelLookup,
            ),
            itemMetaLookup: (0, _extends2.default)(
              {},
              prevState.items.itemMetaLookup,
              newState.itemMetaLookup,
            ),
            itemOrderedChildrenIdsLookup: (0, _extends2.default)(
              {},
              newState.itemOrderedChildrenIdsLookup,
              prevState.items.itemOrderedChildrenIdsLookup,
            ),
            itemChildrenIndexesLookup: (0, _extends2.default)(
              {},
        ...
        Object.values(prevState.items.itemMetaLookup).forEach((item) => {
          if (!newState.itemMetaLookup[item.id]) {
            (0, _publishTreeViewEvent.publishTreeViewEvent)(
              instance,
              "removeItem",
              {
                id: item.id,
  ...
  const removeChildren = (parentId) => {
    store.update((prevState) => {
      ...
      const newMetaMap = Object.keys(prevState.items.itemMetaLookup).reduce(
        (acc, key) => {
...
useTreeViewItems.wrapRoot = ({ children }) => {
  return /*#__PURE__*/ (0, _jsxRuntime.jsx)(
    _TreeViewItemDepthContext.TreeViewItemDepthContext.Provider,
    {
...
if (process.env.NODE_ENV !== "production")
  useTreeViewItems.wrapRoot.displayName = "useTreeViewItems.wrapRoot";
