...
if (process.env.NODE_ENV !== "production")
  RichTreeViewItemsContext.displayName = "RichTreeViewItemsContext";
const WrappedTreeItem = /*#__PURE__*/ React.memo(function WrappedTreeItem({
  ...
  const { store } = (0, _TreeViewProvider.useTreeViewContext)();
  const itemMeta = (0, _useSelector.useSelector)(
    store,
    _useTreeViewItems.selectorItemMeta,
    itemId,
  );
  const children = (0, _useSelector.useSelector)(
    store,
    _useTreeViewItems.selectorItemOrderedChildrenIds,
    itemId,
  );
  ...
  const _useSlotProps = (0, _useSlotProps2.default)({
    ...
    itemProps = (0, _objectWithoutPropertiesLoose2.default)(
      _useSlotProps,
      _excluded,
    );
  return /*#__PURE__*/ (0, _jsxRuntime.jsx)(
    Item,
    (0, _extends2.default)({}, itemProps, {
      children: children?.map(renderItemForRichTreeView),
    }),
  );
}, _fastObjectShallowCompare.fastObjectShallowCompare);
if (process.env.NODE_ENV !== "production")
  WrappedTreeItem.displayName = "WrappedTreeItem";
function RichTreeViewItems(props) {
  const { slots, slotProps } = props;
  const { store } = (0, _TreeViewProvider.useTreeViewContext)();
  const itemSlot = slots?.item;
  const itemSlotProps = slotProps?.item;
  const items = (0, _useSelector.useSelector)(
    store,
    _useTreeViewItems.selectorItemOrderedChildrenIds,
    null,
  );
