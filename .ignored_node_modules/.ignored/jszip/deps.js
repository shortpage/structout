...
async function main() {
    ...
    const typedPaths = parsedConfig.fileNames.map((filename) =>
    ...
    const untypedLeaves = res
        .leaves()
        .map((filename) => path.resolve(DIR, filename))
        .filter(excludeTypedPaths);
    if (untypedLeaves.length) {
        ...
        console.log(
            "Try looking at circular dependencies, or the image to decide what to convert next:",
        );
        const untypedCircular = res
            .circular()
            .flat()
            .map((filename) => path.resolve(DIR, filename))
            .filter(excludeTypedPaths);
