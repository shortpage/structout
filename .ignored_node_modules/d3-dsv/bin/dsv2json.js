...
const options = program
  ...
  .option(
    "-o, --out <file>",
    "output file name; defaults to “-” for stdout",
    "-",
  )
  .option(
    "-r, --input-delimiter <character>",
  ...
  .option(
    ...
    "input character encoding; defaults to “utf8”",
    "utf8",
  )
  .option(
    "--output-encoding <encoding>",
    "output character encoding; defaults to “utf8”",
    "utf8",
  )
  .parse(process.argv)
  .opts();
...
rw.dash.readFile(program.args[0] || "-", (error, text) => {
  ...
  const rows = inFormat.parse(
    iconv.decode(text, options.inputEncoding),
    rowConverter,
  );
  rw.dash.writeFile(
    options.out,
    iconv.encode(
      options.newlineDelimited
        ? rows.map((row) => JSON.stringify(row)).join("\n") + "\n"
        : JSON.stringify(rows) + EOL,
