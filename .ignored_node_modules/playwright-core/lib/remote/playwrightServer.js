  ...
  constructor(options) {
    ...
    if (options.preLaunchedBrowser)
      this._preLaunchedPlaywright =
        options.preLaunchedBrowser.attribution.playwright;
    if (options.preLaunchedAndroidDevice)
      this._preLaunchedPlaywright =
        options.preLaunchedAndroidDevice._android.attribution.playwright;
    const browserSemaphore = new import_semaphore.Semaphore(
    ...
    this._wsServer = new import_wsServer.WSServer({
      onUpgrade: (request, socket) => {
        const uaError = userAgentVersionMatchesErrorMessage(
...
${uaError}`,
      ...
      onConnection: (request, url, ws, id) => {
        ...
        const launchOptionsHeader =
          request.headers["x-playwright-launch-options"] || "";
        ...
        try {
          launchOptions = JSON.parse(
            launchOptionsParam || launchOptionsHeaderValue,
          );
        } catch (e) {}
        const isExtension = this._options.mode === "extension";
        if (isExtension) {
          if (!this._preLaunchedPlaywright)
            this._preLaunchedPlaywright = (0,
            import_playwright.createPlaywright)({
        ...
        if (this._preLaunchedPlaywright)
          await Promise.all(
            this._preLaunchedPlaywright
              .allBrowsers()
              .map((browser) =>
                browser.close({ reason: "Playwright Server stopped" }),
              ),
          );
