...
class PlaywrightConnection {
  constructor(lock, clientType, ws, options, preLaunched, id, onClose) {
    ...
    options.launchOptions = filterLaunchOptions(
      options.launchOptions,
      options.allowFSPaths,
    );
    if (
    ...
    if (clientType === "pre-launched-browser-or-android")
      (0, import_assert.assert)(
        preLaunched.browser || preLaunched.androidDevice,
      );
  ...
  async _initLaunchBrowserMode(scope, options) {
    ...
    if ("bidi" === browserName) {
      if (
        this._options.launchOptions?.channel
          ?.toLocaleLowerCase()
          .includes("firefox")
      )
    ...
    return new import_server.PlaywrightDispatcher(
      scope,
      playwright,
      ownedSocksProxy,
      browser,
  ...
  async _initPreLaunchedBrowserMode(scope) {
    import_debugLogger.debugLogger.log(
      "server",
      `[${this._id}] engaged pre-launched (browser) mode`,
    );
    ...
    const playwrightDispatcher = new import_server.PlaywrightDispatcher(
      scope,
      playwright,
  ...
  async _initPreLaunchedAndroidMode(scope) {
    import_debugLogger.debugLogger.log(
      "server",
      `[${this._id}] engaged pre-launched (Android) mode`,
    );
    const playwright = this._preLaunched.playwright;
  ...
  _initDebugControllerMode() {
    import_debugLogger.debugLogger.log(
      "server",
      `[${this._id}] engaged reuse controller mode`,
    );
    const playwright = this._preLaunched.playwright;
    return new import_debugControllerDispatcher.DebugControllerDispatcher(
      this._dispatcherConnection,
  ...
  async _initReuseBrowsersMode(scope) {
    ...
    if (!browser) {
      browser = await playwright[
        this._options.browserName || "chromium"
      ].launch((0, import_instrumentation.serverSideCallMetadata)(), {
        ...this._options.launchOptions,
  ...
  async _onDisconnect(error) {
    ...
    import_debugLogger.debugLogger.log(
      "server",
      `[${this._id}] disconnected. error: ${error}`,
    );
    this._root._dispose();
    import_debugLogger.debugLogger.log(
      "server",
      `[${this._id}] starting cleanup`,
    );
  ...
  logServerMetadata(message, messageString, direction) {
    ...
    import_debugLogger.debugLogger.log(
      ...
      (direction === "SEND" ? "SEND \u25BA " : "\u25C0 RECV ") +
        JSON.stringify(serverLogMetadata),
    );
  }
  async close(reason) {
...
function filterLaunchOptions(options, allowFSPaths) {
  return {
    ...
    executablePath:
      (0, import_debug.isUnderTest)() || allowFSPaths
