...
function transform(template, params, testIdAttributeName) {
  while (true) {
    const hasMatch = template.match(
      /filter\(,?(has=|hasnot=|sethas\(|sethasnot\()/,
    );
    if (!hasMatch) break;
  ...
  template = template
    ...
    .replace(
      /getbytestid\(([^)]+)\)/g,
      `internal:testid=[${testIdAttributeName}=$1]`,
    )
    .replace(
      ...
      t = t
        .replace(/(?:r)\$(\d+)(i)?/g, (_, ordinal, suffix) => {
          ...
          if (
            ...
            return (
              (0, import_stringUtils.escapeForAttributeSelector)(
                new RegExp(param.text),
                false,
              ) + (suffix || "")
            );
        ...
        .replace(/\$(\d+)(i|s)?/g, (_, ordinal, suffix) => {
          ...
          if (t.startsWith("internal:testid"))
            return (0, import_stringUtils.escapeForAttributeSelector)(
              param.text,
              true,
            );
...
function unsafeLocatorOrSelectorAsSelector(
  language,
  locator,
  testIdAttributeName,
) {
  ...
  const { selector, preferredQuote } = parseLocator(
    locator,
    testIdAttributeName,
  );
  ...
  if (
    locators.some(
      (candidate) => digestForComparison(language, candidate) === digest,
    )
  )
    return selector;
  return "";
