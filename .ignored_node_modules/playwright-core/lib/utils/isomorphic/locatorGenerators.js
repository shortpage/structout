...
function innerAsLocators(
  ...
  for (let index = 0; index < parts.length; index++) {
    ...
    if (part.name === "internal:role") {
      const attrSelector = (0, import_selectorParser.parseAttributeSelector)(
        part.body,
        true,
      );
      const options = { attrs: [] };
      ...
      if (["xpath", "css"].includes(lastPart.name)) {
        transformed.push(
          ...
          factory.generateLocator(
            base,
            "frame-locator",
            (0, import_selectorParser.stringifySelector)(
              { parts: [lastPart] },
              true,
            ),
          ),
    ...
    const selectorPart = (0, import_selectorParser.stringifySelector)({
      parts: [part],
    });
...
class JavaScriptLocatorFactory {
  ...
  quote(text) {
    return (0, import_stringUtils.escapeWithQuotes)(
      text,
      this.preferredQuote ?? "'",
    );
  }
    ...
    switch (kind) {
      ...
      case "role":
        ...
        for (const { name, value } of options.attrs) {
          ...
          attrs.push(
            `${(0, import_stringUtils.toSnakeCase)(name)}=${valueString}`,
          );
        }
        const attrString = attrs.length ? `, ${attrs.join(", ")}` : "";
