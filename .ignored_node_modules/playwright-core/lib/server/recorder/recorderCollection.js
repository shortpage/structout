...
class RecorderCollection extends import_events.EventEmitter {
  ...
  async performAction(actionInContext) {
    await this._addAction(actionInContext, async () => {
      await (0, import_recorderRunner.performAction)(
        this._pageAliases,
        actionInContext,
      );
    });
  ...
  async _addAction(actionInContext, callback) {
    ...
    if (
      actionInContext.action.name === "openPage" ||
      actionInContext.action.name === "closePage"
    ) {
      this._actions.push(actionInContext);
      this._fireChange();
      return;
  ...
  _fireChange() {
    ...
    this.emit(
      "change",
      (0, import_recorderUtils.collapseActions)(this._actions),
    );
  }
}
