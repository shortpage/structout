...
class ContextRecorder extends import_events.EventEmitter {
  constructor(context, params, delegate) {
    ...
    const language =
      params.language || context.attribution.playwright.options.sdkLanguage;
    ...
    this._collection = new import_recorderCollection.RecorderCollection(
      this._pageAliases,
    );
    this._collection.on("change", (actions) => {
      this._recorderSources = [];
      for (const languageGenerator of this._orderedLanguages) {
        const { header, footer, actionTexts, text } = (0,
        import_language.generateCode)(
          actions,
          languageGenerator,
    ...
    this._listeners.push(
      import_utils.eventsHelper.addEventListener(process, "exit", () => {
        this._throttledOutputFile?.flush();
      }),
    );
...
`);
    ...
    this._throttledOutputFile = outputFile
      ? new import_throttledFile.ThrottledFile(outputFile)
      : null;
  ...
  async install() {
    ...
    this._context.on(
      ...
      (dialog) => this._onDialog(dialog.page()),
    );
  ...
  async _createActionInContext(frame, action) {
    ...
    await this._delegate.rewriteActionInContext?.(
      this._pageAliases,
      actionInContext,
    );
  ...
  async _performAction(frame, action) {
    await this._collection.performAction(
      await this._createActionInContext(frame, action),
    );
  }
