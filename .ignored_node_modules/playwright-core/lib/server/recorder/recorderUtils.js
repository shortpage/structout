...
function mainFrameForAction(pageAliases, actionInContext) {
  ...
  const page = [...pageAliases.entries()].find(
...
async function frameForAction(pageAliases, actionInContext, action) {
  ...
  const fullSelector = buildFullSelector(
    actionInContext.frame.framePath,
    action.selector,
  );
  const result = await page
    .mainFrame()
    .selectors.resolveFrameForSelector(fullSelector);
  ...
  for (const action of actions) {
    ...
    const shouldMerge =
      ...
      (action.action.name === "navigate" ||
        (action.action.name === "fill" && isSameSelector));
    if (!shouldMerge) {
      result.push(action);
      continue;
    }
