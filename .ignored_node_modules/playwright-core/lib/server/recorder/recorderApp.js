...
class RecorderApp extends import_events.EventEmitter {
            ...
            headers: [
              {
                ...
                value:
                  import_utilsBundle.mime.getType(
                    import_path.default.extname(file),
  ...
  static async _open(recorder, inspectedContext) {
    const sdkLanguage =
      inspectedContext.attribution.playwright.options.sdkLanguage;
      ...
      recorderPlaywright.chromium,
      {
        ...
        persistentContextOptions: {
          ...
          headless:
            ...
            ((0, import_debug.isUnderTest)() && !headed),
    ...
    const result = new RecorderApp(
      recorder,
      page,
      context._browser.options.wsEndpoint,
    );
    await result._init();
    return result;
  }
  ...
  async setSources(sources, primaryPageURL) {
    await this._page
      .mainFrame()
      .evaluateExpression(
        (({ sources: sources2, primaryPageURL: primaryPageURL2 }) => {
          window.playwrightSetSources(sources2, primaryPageURL2);
        }).toString(),
        { isFunction: true },
        { sources, primaryPageURL },
      )
      .catch(() => {});
