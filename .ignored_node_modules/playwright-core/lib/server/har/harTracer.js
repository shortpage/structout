...
class HarTracer {
  ...
  _onAPIRequest(event) {
    ...
    harEntry.request.headers = Object.entries(event.headers).map(
  ...
  _onRequest(request) {
    ...
    harEntry.request.postData = this._postDataForRequest(
      request,
      this._options.content,
    );
    if (!this._options.omitSizes)
  ...
  _recordRequestHeadersAndCookies(harEntry, headers) {
    if (!this._options.omitCookies) {
      ...
      for (const header of headers.filter(
        (header2) => header2.name.toLowerCase() === "cookie",
      ))
  ...
  async _onRequestFinished(request, response) {
    ...
    if (compressionCalculationBarrier)
      this._addBarrier(
        page || request.serviceWorker(),
        compressionCalculationBarrier.barrier,
      );
    ...
    if (!this._options.omitSizes) {
      this._addBarrier(
        ...
        response.sizes().then((sizes) => {
          ...
          compressionCalculationBarrier?.setEncodedBodySize(
  ...
  _storeResponseContent(buffer, content, resourceType) {
    ...
    if (this._options.content === "embed") {
      if (
        (0, import_utils2.isTextualMimeType)(content.mimeType) &&
        resourceType !== "font"
      ) {
  ...
  _onResponse(response) {
    ...
    if (!this._options.omitTiming) {
      const startDateTime = pageEntry
        ? pageEntry[startedDateSymbol].valueOf()
        : 0;
      const timing = response.timing();
  ...
  stop() {
    ...
    const log = {
      ...
      pages: this._pageEntries.size
        ? Array.from(this._pageEntries.values())
        : void 0,
      entries: [],
    };
    if (!this._options.omitTiming) {
      for (const pageEntry of log.pages || []) {
        ...
        if (
          typeof pageEntry.pageTimings.onLoad === "number" &&
          pageEntry.pageTimings.onLoad >= 0
        )
...
function createHarEntry(method, url, frameref, options) {
  const harEntry = {
    ...
    _monotonicTime: options.includeTraceInfo
      ? (0, import_utils.monotonicTime)()
      : void 0,
