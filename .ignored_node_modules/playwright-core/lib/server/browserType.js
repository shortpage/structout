...
class BrowserType extends import_instrumentation.SdkObject {
    ...
    const browser = await controller.run(async (progress) => {
      ...
      if (options.clientCertificates?.length) {
        clientCertificatesProxy =
          new import_socksClientCertificatesInterceptor.ClientCertificatesProxy(
            options,
          );
      ...
      const browser2 = await this._innerLaunchWithRetries(
        progress,
        launchOptions,
        options,
      ...
      browser2._defaultContext._clientCertificatesProxy =
        clientCertificatesProxy;
      return browser2;
  ...
  async _launchProcess(
    ...
    if (userDataDir) {
      ...
      if (!(await (0, import_fileUtils.existsAsync)(userDataDir)))
        await import_fs.default.promises.mkdir(userDataDir, {
          recursive: true,
          mode: 448,
        });
    } else {
      userDataDir = await import_fs.default.promises.mkdtemp(
        import_path.default.join(
          import_os.default.tmpdir(),
          `playwright_${this._name}dev_profile-`,
        ),
      );
      ...
      close: () =>
        closeOrKill(
          options.__testHookBrowserCloseTimeout ||
            import_timeoutSettings.DEFAULT_TIMEOUT,
        ),
      kill,
    };
    ...
    if (options.useWebSocket) {
      transport = await import_transport.WebSocketTransport.connect(
        progress,
        wsEndpoint,
      );
    } else {
  ...
  async _createArtifactDirs(options) {
    if (options.downloadsPath)
      await import_fs.default.promises.mkdir(options.downloadsPath, {
        recursive: true,
      });
    if (options.tracesDir)
      await import_fs.default.promises.mkdir(options.tracesDir, {
        recursive: true,
      });
  }
  ...
  _validateLaunchOptions(options) {
    ...
    if ((0, import_debug.debugMode)()) headless = false;
