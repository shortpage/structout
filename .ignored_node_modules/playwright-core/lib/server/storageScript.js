...
async function collect(
  ...
  async function collectDB(dbInfo) {
    ...
    function trySerialize(value) {
      ...
      if (trivial) return { trivial: value };
      return { encoded };
    }
        ...
        const records = await Promise.all(
          keys.map(async (key) => {
            ...
            if (objectStore.keyPath === null) {
              const { encoded: encoded2, trivial: trivial2 } =
                trySerialize(key);
        ...
        return {
          ...
          keyPath:
            typeof objectStore.keyPath === "string"
              ? objectStore.keyPath
              : void 0,
          keyPathArray: Array.isArray(objectStore.keyPath)
            ? objectStore.keyPath
            : void 0,
        };
      }),
  ...
  return {
    ...
    indexedDB: recordIndexedDB
      ? await Promise.all((await indexedDB.databases()).map(collectDB)).catch(
          (e) => {
...
async function restore(serializersSource, builtins, originState) {
  ...
  await Promise.all(
    (originState.indexedDB ?? []).map(async (dbInfo) => {
        ...
        for (const store of dbInfo.stores) {
          ...
          for (const index of store.indexes)
            objectStore.createIndex(
              index.name,
              index.keyPathArray ?? index.keyPath,
          ...
          await Promise.all(
            store.records.map(async (record) => {
              await idbRequestToPromise(
                objectStore.add(
                  record.value ??
                    parseEvaluationResultValue(record.valueEncoded),
