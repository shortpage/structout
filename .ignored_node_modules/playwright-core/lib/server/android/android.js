...
class Android extends import_instrumentation.SdkObject {
  ...
  async devices(options) {
    const devices = (await this._backend.devices(options)).filter(
      (d) => d.status === "device",
    );
...
class AndroidDevice extends import_instrumentation.SdkObject {
  ...
  async _installDriver() {
    ...
    if (!this._options.omitDriverInstall) {
      ...
      import_env.getPackageManagerExecCommand)();
      for (const file of ["android-driver.apk", "android-driver-target.apk"]) {
        ...
        await this.installApk(
          await import_fs.default.promises.readFile(fullName),
        );
      }
    } else {
      (0, import_utilsBundle.debug)("pw:android")(
  ...
  async launchBrowser(pkg = "com.android.chrome", options) {
    ...
    await this._backend.runCommand(
      `shell:rm /data/local/tmp/chrome-command-line`,
    );
  ...
  async _connectToBrowser(socketName, options = {}) {
    ...
    socket.on("close", async () => {
      ...
      cleanupArtifactsDir().catch((e) =>
        (0, import_utilsBundle.debug)("pw:android")(
          `could not cleanup artifacts dir: ${e}`,
        ),
      );
    });
  ...
  async _refreshWebViews() {
    ...
    for (const line of sockets) {
      const matchSocketName = line.match(
