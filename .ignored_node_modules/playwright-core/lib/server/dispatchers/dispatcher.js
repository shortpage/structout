...
class Dispatcher extends import_events.EventEmitter {
  ...
  async _handleCommand(callMetadata, method, validParams) {
    ...
    try {
      ...
      if (
        callMetadata.potentiallyClosesScope &&
        (0, import_errors.isTargetClosedError)(e)
      )
  ...
  _dispatchEvent(method, params) {
    if (this._disposed) {
      if ((0, import_debug.isUnderTest)())
        throw new Error(
          `${this._guid} is sending "${String(method)}" event after being disposed`,
        );
      return;
    }
  ...
  _disposeRecursively(error) {
    (0, import_utils.assert)(
      !this._disposed,
      `${this._guid} is disposed more than once`,
    );
    this._onDispose();
    this._disposed = true;
...
class DispatcherConnection {
  ...
  maybeDisposeStaleDispatchers(gcBucket) {
    ...
    this._dispatchersByBucket.set(
      gcBucket,
      new Set(dispatchersArray.slice(disposeCount)),
    );
  ...
  async dispatch(message) {
    ...
    if (!dispatcher) {
      this.onmessage({
        id,
        error: (0, import_errors.serializeError)(
    ...
    const sdkObject =
      dispatcher._object instanceof import_instrumentation.SdkObject
        ? dispatcher._object
        : void 0;
    const callMetadata = {
      id: `call@${id}`,
    ...
    const response = { id };
    try {
      const result = await dispatcher._handleCommand(
        callMetadata,
        method,
        validParams,
      );
      ...
      if ((0, import_errors.isTargetClosedError)(e) && sdkObject) {
        ...
        if (e.type === "closed") {
          ...
          e = new import_errors.TargetClosedError(
            reason,
            e.browserLogMessage(),
          );
        } else if (e.type === "crashed") {
          (0, import_utils.rewriteErrorMessage)(
            e,
            "Target crashed " + e.browserLogMessage(),
          );
        }
      }
...
function closeReason(sdkObject) {
  return (
    sdkObject.attribution.page?._closeReason ||
