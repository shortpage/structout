...
class FrameDispatcher extends import_dispatcher.Dispatcher {
  constructor(scope, frame) {
    ...
    const pageDispatcher = (0, import_dispatcher.existingDispatcher)(
      frame._page,
    );
    super(
    ...
    this.addObjectListener(
      import_frames.Frame.Events.AddLifecycle,
      (lifecycleEvent) => {
        this._dispatchEvent("loadstate", { add: lifecycleEvent });
      },
    );
    this.addObjectListener(
      import_frames.Frame.Events.RemoveLifecycle,
      (lifecycleEvent) => {
        this._dispatchEvent("loadstate", { remove: lifecycleEvent });
      },
    );
    this.addObjectListener(
      import_frames.Frame.Events.InternalNavigation,
      (event) => {
  ...
  async waitForSelector(params, metadata) {
    return {
      element:
        import_elementHandlerDispatcher.ElementHandleDispatcher.fromNullable(
          this,
  ...
  async expect(params, metadata) {
    ...
    if (result.received !== void 0)
      result.received = (0, import_jsHandleDispatcher.serializeResult)(
        result.received,
      );
    return result;
  }
  async ariaSnapshot(params, metadata) {
    return {
      snapshot: await this._frame.ariaSnapshot(
        metadata,
        params.selector,
        params,
      ),
