...
class ElementHandleDispatcher extends import_jsHandleDispatcher.JSHandleDispatcher {
  ...
  static from(scope, handle) {
    return (
      (0, import_dispatcher.existingDispatcher)(handle) ||
  ...
  static fromJSHandle(scope, handle) {
    ...
    return handle.asElement()
      ...
      : new import_jsHandleDispatcher.JSHandleDispatcher(scope, handle);
  }
  async ownerFrame(params, metadata) {
    const frame = await this._elementHandle.ownerFrame();
    return {
      frame: frame
        ? import_frameDispatcher.FrameDispatcher.from(
  ...
  async querySelectorAll(params, metadata) {
    ...
    return {
      elements: elements.map((e) =>
        ElementHandleDispatcher.from(this.parentScope(), e),
      ),
    };
  }
  ...
  _browserContextDispatcher() {
    ...
    if (
      scope instanceof import_browserContextDispatcher.BrowserContextDispatcher
    )
      return scope;
    if (scope instanceof import_pageDispatcher.PageDispatcher)
      return scope.parentScope();
    if (
      scope instanceof import_pageDispatcher.WorkerDispatcher ||
      scope instanceof import_frameDispatcher.FrameDispatcher
    ) {
      const parentScope = scope.parentScope();
      if (
        parentScope instanceof
        import_browserContextDispatcher.BrowserContextDispatcher
      )
        return parentScope;
