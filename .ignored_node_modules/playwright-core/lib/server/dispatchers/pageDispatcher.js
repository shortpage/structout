...
class PageDispatcher extends import_dispatcher.Dispatcher {
  constructor(parentScope, page) {
    const mainFrame = import_frameDispatcher.FrameDispatcher.from(
      parentScope,
      page.mainFrame(),
    );
    ...
    this.addObjectListener(import_page.Page.Events.Crash, () =>
      this._dispatchEvent("crash"),
    );
    ...
    this.addObjectListener(
      import_page.Page.Events.LocatorHandlerTriggered,
      (uid) => this._dispatchEvent("locatorHandlerTriggered", { uid }),
    );
    this.addObjectListener(import_page.Page.Events.WebSocket, (webSocket) =>
      this._dispatchEvent("webSocket", {
        webSocket: new import_networkDispatchers3.WebSocketDispatcher(
          this,
          webSocket,
        ),
      }),
  ...
  async registerLocatorHandler(params, metadata) {
    const uid = this._page.registerLocatorHandler(
      params.selector,
      params.noWaitAfter,
    );
    return { uid };
  ...
  async setNetworkInterceptionPatterns(params, metadata) {
    ...
    await this._page.setClientRequestInterceptor((route, request) => {
      ...
      this._dispatchEvent("route", {
        route: import_networkDispatchers3.RouteDispatcher.from(
          import_networkDispatchers.RequestDispatcher.from(
            this.parentScope(),
            request,
          ),
          route,
  ...
  async screenshot(params, metadata) {
    ...
    return {
      binary: await this._page.screenshot(metadata, { ...params, mask }),
    };
  }
  ...
  _onFrameAttached(frame) {
    this._dispatchEvent("frameAttached", {
      frame: import_frameDispatcher.FrameDispatcher.from(
        this.parentScope(),
        frame,
      ),
    });
...
class WorkerDispatcher extends import_dispatcher.Dispatcher {
  constructor(scope, worker) {
    ...
    this.addObjectListener(import_page.Worker.Events.Close, () =>
      this._dispatchEvent("close"),
    );
  }
...
class BindingCallDispatcher extends import_dispatcher.Dispatcher {
  constructor(scope, name, needsHandle, source, args) {
    super(
      scope,
      { guid: "bindingCall@" + (0, import_crypto.createGuid)() },
      "BindingCall",
      {
        frame: import_frameDispatcher.FrameDispatcher.from(
          scope.parentScope(),
        ...
        args: needsHandle
          ? void 0
          : args.map(import_jsHandleDispatcher.serializeResult),
        handle: needsHandle
          ? import_elementHandlerDispatcher.ElementHandleDispatcher.fromJSHandle(
              scope,
              args[0],
