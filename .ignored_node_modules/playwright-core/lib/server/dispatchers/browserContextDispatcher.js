...
class BrowserContextDispatcher extends import_dispatcher.Dispatcher {
  constructor(parentScope, context) {
    const requestContext =
      import_networkDispatchers.APIRequestContextDispatcher.from(
        parentScope,
        context.fetchRequest,
      );
    const tracing = import_tracingDispatcher.TracingDispatcher.from(
      parentScope,
      context.tracing,
    );
    ...
    const onVideo = (artifact) => {
      const artifactDispatcher =
        import_artifactDispatcher.ArtifactDispatcher.from(
          parentScope,
          artifact,
        );
    ...
    this.addObjectListener(
      import_browserContext.BrowserContext.Events.VideoStarted,
      onVideo,
    );
    ...
    this.addObjectListener(
      import_browserContext.BrowserContext.Events.Close,
      () => {
    ...
    this.addObjectListener(
      ...
      (message) => {
        ...
        if (this._shouldDispatchEvent(page, "console")) {
          const pageDispatcher = import_pageDispatcher.PageDispatcher.from(
            this,
            page,
          );
          this._dispatchEvent("console", {
            ...
            args: message
              .args()
              .map((a) =>
                import_elementHandlerDispatcher.ElementHandleDispatcher.fromJSHandle(
                  pageDispatcher,
                  a,
                ),
              ),
    ...
    if (context._browser.options.name === "chromium") {
      ...
      for (const serviceWorker of context.serviceWorkers())
        this._dispatchEvent("serviceWorker", {
          worker: new import_pageDispatcher.WorkerDispatcher(
            this,
            serviceWorker,
          ),
        });
    ...
    this.addObjectListener(
      ...
      (request) => {
        const redirectFromDispatcher =
          ...
          (0, import_dispatcher.existingDispatcher)(request.redirectedFrom());
        if (
    ...
    this.addObjectListener(
      ...
      (response) => {
        const requestDispatcher = (0, import_dispatcher.existingDispatcher)(
          response.request(),
        );
        if (
          !requestDispatcher &&
          !this._shouldDispatchNetworkEvent(response.request(), "response")
        )
          return;
        this._dispatchEvent("response", {
          response: import_networkDispatchers.ResponseDispatcher.from(
            this,
            response,
          ),
          page: import_pageDispatcher.PageDispatcher.fromNullable(
    ...
    this.addObjectListener(
      ...
      (request) => {
        ...
        if (
          !requestDispatcher &&
          !this._shouldDispatchNetworkEvent(request, "requestFailed")
        )
          return;
        this._dispatchEvent("requestFailed", {
          request: import_networkDispatchers.RequestDispatcher.from(
            this,
            request,
          ),
    ...
    this.addObjectListener(
      ...
      ({ request, response }) => {
        ...
        if (
          !requestDispatcher &&
          !this._shouldDispatchNetworkEvent(request, "requestFinished")
        )
          return;
  ...
  _shouldDispatchEvent(page, event) {
    ...
    const pageDispatcher = page
      ? (0, import_dispatcher.existingDispatcher)(page)
      : void 0;
  ...
  async createTempFiles(params) {
    ...
    const tmpDir = import_path.default.join(
      dir,
      "upload-" + (0, import_crypto.createGuid)(),
    );
    const tempDirWithRootName = params.rootDirName
      ? import_path.default.join(
          tmpDir,
          import_path.default.basename(params.rootDirName),
        )
      : tmpDir;
    await import_fs.default.promises.mkdir(tempDirWithRootName, {
      recursive: true,
    });
    this._context._tempDirs.push(tmpDir);
    return {
      rootDir: params.rootDirName
        ? new import_writableStreamDispatcher.WritableStreamDispatcher(
            this,
            tempDirWithRootName,
          )
        : void 0,
  ...
  async exposeBinding(params) {
    await this._context.exposeBinding(
      ...
      (source, ...args) => {
        ...
        const binding = new import_pageDispatcher.BindingCallDispatcher(
          pageDispatcher,
          params.name,
          !!params.needsHandle,
          source,
  ...
  async newPage(params, metadata) {
    return {
      page: import_pageDispatcher.PageDispatcher.from(
        this,
    ...
    await this._context.setRequestInterceptor((route, request) => {
      const matchesSome = urlMatchers.some((urlMatch) =>
        (0, import_urlMatch.urlMatches)(
          this._context._options.baseURL,
          request.url(),
          urlMatch,
        ),
      );
      ...
      this._dispatchEvent("route", {
        route: import_networkDispatchers.RouteDispatcher.from(
          import_networkDispatchers.RequestDispatcher.from(this, request),
          route,
        ),
      });
  ...
  async setWebSocketInterceptionPatterns(params, metadata) {
    ...
    if (params.patterns.length)
      await import_webSocketRouteDispatcher.WebSocketRouteDispatcher.installIfNeeded(
        this._context,
      );
  }
  ...
  async enableRecorder(params) {
    await import_recorder.Recorder.show(
      this._context,
      import_recorderApp.RecorderApp.factory(this._context),
      params,
    );
  }
