...
class BrowserDispatcher extends import_dispatcher2.Dispatcher {
  constructor(scope, browser) {
    ...
    this.addObjectListener(import_browser.Browser.Events.Disconnected, () =>
      this._didClose(),
    );
  }
  _didClose() {
  ...
  async newContext(params, metadata) {
    ...
    return {
      context: new import_browserContextDispatcher.BrowserContextDispatcher(
        this,
        context,
      ),
    };
  ...
  async startTracing(params) {
    ...
    await crBrowser.startTracing(
      params.page ? params.page._object : void 0,
      params,
    );
  }
  async stopTracing() {
...
class ConnectedBrowserDispatcher extends import_dispatcher2.Dispatcher {
  ...
  async newContextForReuse(params, metadata) {
    return await newContextForReuse(
      this._object,
      this,
      params,
      this.selectors,
  ...
  async defaultUserAgentForTest() {
    throw new Error(
      "Client should not send us Browser.defaultUserAgentForTest",
    );
  }
  async newBrowserCDPSession() {
    if (!this._object.options.isChromium)
...
async function newContextForReuse(browser, scope, params, selectors, metadata) {
  const { context, needsReset } = await browser.newContextForReuse(
    params,
    metadata,
  );
  if (needsReset) {
    const oldContextDispatcher = (0, import_dispatcher.existingDispatcher)(
      context,
    );
  ...
  const contextDispatcher =
    new import_browserContextDispatcher.BrowserContextDispatcher(
      scope,
      context,
    );
