...
class AndroidDeviceDispatcher extends import_dispatcher.Dispatcher {
  constructor(scope, device) {
    ...
    this.addObjectListener(
      import_android.AndroidDevice.Events.WebViewAdded,
      (webView) => this._dispatchEvent("webViewAdded", { webView }),
    );
    this.addObjectListener(
      import_android.AndroidDevice.Events.WebViewRemoved,
      (socketName) => this._dispatchEvent("webViewRemoved", { socketName }),
    );
    this.addObjectListener(
      import_android.AndroidDevice.Events.Close,
      (socketName) => this._dispatchEvent("close"),
    );
  }
  ...
  async inputType(params) {
    ...
    await Promise.all(
      keyCodes.map((keyCode) => this._object.send("inputPress", { keyCode })),
    );
  }
