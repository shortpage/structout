...
var webSocketMockSource = __toESM(
  require("../../generated/webSocketMockSource"),
);
...
class WebSocketRouteDispatcher extends import_dispatcher.Dispatcher {
  constructor(scope, id, url, frame) {
    super(
      scope,
      { guid: "webSocketRoute@" + (0, import_crypto.createGuid)() },
      "WebSocketRoute",
      { url },
    );
  ...
  static async installIfNeeded(target) {
    ...
    const context =
      target instanceof import_page.Page ? target.context() : target;
    if (!context.hasBinding(kBindingName)) {
      await context.exposeBinding(kBindingName, false, (source, payload) => {
        if (payload.type === "onCreate") {
          const contextDispatcher = (0, import_dispatcher.existingDispatcher)(
            context,
          );
          const pageDispatcher = contextDispatcher
            ? import_pageDispatcher.PageDispatcher.fromNullable(
                contextDispatcher,
                source.page,
              )
            : void 0;
          ...
          if (scope) {
            ...
            source.frame
              .evaluateExpression(
                `globalThis.__pwWebSocketDispatch(${JSON.stringify(request)})`,
              )
              .catch(() => {});
          }
          return;
        }
        const dispatcher = WebSocketRouteDispatcher._idToDispatcher.get(
          payload.id,
        );
