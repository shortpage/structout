...
class ArtifactDispatcher extends import_dispatcher.Dispatcher {
    ...
    return await new Promise((resolve, reject) => {
      this._object.saveAs(async (localPath, error) => {
        ...
        try {
          ...
          const stream = new import_streamDispatcher.StreamDispatcher(
            this,
            readable,
          );
          resolve({ stream });
  ...
  async stream() {
    ...
    const readable = import_fs.default.createReadStream(fileName, {
      highWaterMark: 1024 * 1024,
    });
    return {
      stream: new import_streamDispatcher.StreamDispatcher(this, readable),
    };
  }
  async failure() {
