...
class PlaywrightDispatcher extends import_dispatcher.Dispatcher {
  constructor(
    ...
    const browserDispatcher = preLaunchedBrowser
      ? new import_browserDispatcher.ConnectedBrowserDispatcher(
          scope,
          preLaunchedBrowser,
        )
      : void 0;
    const android = new import_androidDispatcher.AndroidDispatcher(
      scope,
      playwright.android,
    );
    const prelaunchedAndroidDeviceDispatcher = prelaunchedAndroidDevice
      ? new import_androidDispatcher2.AndroidDeviceDispatcher(
          android,
          prelaunchedAndroidDevice,
        )
      : void 0;
    super(scope, playwright, "Playwright", {
      chromium: new import_browserTypeDispatcher.BrowserTypeDispatcher(
        scope,
        playwright.chromium,
      ),
      firefox: new import_browserTypeDispatcher.BrowserTypeDispatcher(
        scope,
        playwright.firefox,
      ),
      webkit: new import_browserTypeDispatcher.BrowserTypeDispatcher(
        scope,
        playwright.webkit,
      ),
      bidiChromium: new import_browserTypeDispatcher.BrowserTypeDispatcher(
        scope,
        playwright.bidiChromium,
      ),
      bidiFirefox: new import_browserTypeDispatcher.BrowserTypeDispatcher(
        scope,
        playwright.bidiFirefox,
      ),
      android,
      ...
      utils: playwright.options.isServer
        ? void 0
        : new import_localUtilsDispatcher.LocalUtilsDispatcher(
            scope,
            playwright,
          ),
      selectors: new import_selectorsDispatcher.SelectorsDispatcher(
        scope,
        browserDispatcher?.selectors || playwright.selectors,
      ),
      preLaunchedBrowser: browserDispatcher,
      preConnectedAndroidDevice: prelaunchedAndroidDeviceDispatcher,
      socksSupport: socksProxy
        ? new SocksSupportDispatcher(scope, socksProxy)
        : void 0,
    });
  ...
  async newRequest(params) {
    const request = new import_fetch.GlobalAPIRequestContext(
      this._object,
      params,
    );
    return {
...
class SocksSupportDispatcher extends import_dispatcher.Dispatcher {
  constructor(scope, socksProxy) {
    super(
      scope,
      { guid: "socksSupport@" + (0, import_crypto.createGuid)() },
      "SocksSupport",
      {},
    );
