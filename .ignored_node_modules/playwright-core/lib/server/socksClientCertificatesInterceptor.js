  ...
  async connect() {
    if (this.socksProxy.proxyAgentFromOptions)
      this.target = await this.socksProxy.proxyAgentFromOptions.callback(
        new import_events.EventEmitter(),
        {
      ...
      this.target = await (0, import_happyEyeballs.createSocket)(
      ...
      (alpnProtocolChosenByServer) => {
        ...
        this._dummyServer.once("secureConnection", (internalTLS) => {
          ...
          const handleError = (error) => {
            ...
            if (internalTLS?.alpnProtocol === "h2") {
              if ("performServerHandshake" in import_http2.default) {
                ...
                const session =
                  import_http2.default.performServerHandshake(internalTLS);
                session.on("error", () => {
                  this.target.destroy();
                  this._targetCloseEventListener();
                });
          ...
          targetTLS = import_tls.default.connect({
            ...
            servername: !import_net.default.isIP(this.host)
              ? this.host
              : void 0,
...
class ClientCertificatesProxy {
  constructor(contextOptions) {
    ...
    this.proxyAgentFromOptions = (0, import_network.createProxyAgent)(
    ...
    for (const [origin, certs] of origin2certs) {
      try {
        this.secureContextMap.set(
          origin,
          import_tls.default.createSecureContext(
            convertClientCertificatesToTLSOptions(certs),
          ),
        );
      } catch (error) {
