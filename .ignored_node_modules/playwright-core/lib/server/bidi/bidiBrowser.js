...
class BidiBrowser extends import_browser.Browser {
  ...
  static async connect(parent, transport, options) {
    ...
    if (options.proxy) {
      ...
      switch (url.protocol) {
        ...
        default:
          throw new Error(
            "Invalid proxy server protocol: " + options.proxy.server,
          );
      }
  ...
  async doCreateNewContext(options) {
    const { userContext } = await this._browserSession.send(
  ...
  _onBrowsingContextCreated(event) {
    ...
    if (!context) return;
    const session = this._connection.createMainFrameBrowsingContextSession(
      event.context,
    );
    const opener =
      event.originalOpener && this._bidiPages.get(event.originalOpener);
...
class BidiBrowserContext extends import_browserContext.BrowserContext {
  ...
  async doCreateNewPage() {
    ...
    const { context } = await this._browser._browserSession.send(
  ...
  async doAddInitScript(initScript) {
    const { script } = await this._browser._browserSession.send(
  ...
  async doRemoveNonInternalInitScripts() {
    const promise = Promise.all(
      this._initScriptIds.map((script) =>
        this._browser._browserSession.send("script.removePreloadScript", {
          script,
        }),
      ),
    );
