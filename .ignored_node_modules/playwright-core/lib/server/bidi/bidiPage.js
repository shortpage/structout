...
class BidiPage {
  ...
  async _addAllInitScripts() {
    return Promise.all(
      this._page
        .allInitScripts()
        .map((initScript) => this.addInitScript(initScript)),
    );
  }
  didClose() {
  ...
  _onRealmCreated(realmInfo) {
    ...
    const delegate = new import_bidiExecutionContext.BidiExecutionContext(
      this._session,
      realmInfo,
    );
  ...
  _onLogEntryAdded(params) {
    ...
    this._page._addConsoleMessage(
      entry.method,
      entry.args.map((arg) =>
        (0, import_bidiExecutionContext.createHandle)(context, arg),
      ),
      location,
      params.text || void 0,
    );
  ...
  async navigateFrame(frame, url, referrer) {
    const { navigation } = await this._session.send(
  ...
  async updateHttpCredentials() {
    await this._networkManager.setCredentials(
      this._browserContext._options.httpCredentials,
    );
  }
  ...
  async _installMainBinding() {
    ...
    promises.push(
      this._session.send("script.callFunction", {
        functionDeclaration,
        arguments: args,
        target: toBidiExecutionContext(
          await this._page.mainFrame()._mainContext(),
        )._target,
        awaitPromise: false,
  ...
  async removeNonInternalInitScripts() {
    const promises = this._initScriptIds.map((script) =>
      this._session.send("script.removePreloadScript", { script }),
    );
    this._initScriptIds = [];
    await Promise.all(promises);
  }
  ...
  async takeScreenshot(
    ...
    const { data } = await this._session.send(
