...
class BidiExecutionContext {
  ...
  async rawEvaluateJSON(expression) {
    ...
    if (response.type === "success")
      return import_bidiDeserializer.BidiDeserializer.deserialize(
        response.result,
      );
  ...
  async evaluateWithArguments(
    ...
    if (response.type === "success") {
      if (returnByValue)
        return js.parseEvaluationResultValue(
          import_bidiDeserializer.BidiDeserializer.deserialize(response.result),
        );
  ...
  async getProperties(handle) {
    ...
    const values = await Promise.all(
      names.map((name) =>
        handle.evaluateHandle((object, name2) => object[name2], name),
      ),
    );
