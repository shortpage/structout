  ...
  return cookies.map((c) => {
    ...
    (0, import_utils.assert)(
      !(
        c.expires &&
        c.expires > 0 &&
        c.expires > kMaxCookieExpiresDateInSeconds
      ),
      "Cookie should have a valid expires, only -1 or a positive number for the unix timestamp in seconds is allowed",
    );
    const copy = { ...c };
...
class Request extends import_instrumentation.SdkObject {
  constructor(
    ...
    this._isFavicon =
      url.endsWith("/favicon.ico") || !!redirectedFrom?._isFavicon;
  }
...
class Route extends import_instrumentation.SdkObject {
  ...
  async redirectNavigationRequest(url) {
    ...
    this._request
      ...
      .redirectNavigation(
        url,
        this._request._documentId,
        this._request.headerValue("referer"),
      );
  }
  async fulfill(overrides) {
    ...
    if (body === void 0) {
      if (overrides.fetchResponseUid) {
        const buffer =
          this._request._context.fetchRequest.fetchResponses.get(
            overrides.fetchResponseUid,
          ) ||
          import_fetch.APIRequestContext.findResponseBody(
            overrides.fetchResponseUid,
          );
  ...
  async continue(overrides) {
    ...
    if (overrides.headers)
      overrides.headers = overrides.headers?.filter(
