...
class APIRequestContext extends import_instrumentation.SdkObject {
  ...
  async fetch(params, metadata) {
    ...
    const requestUrl = new URL(
      (0, import_utils.constructURLBasedOnBaseURL)(
        defaults.baseURL,
        params.url,
      ),
    );
    ...
    const options = {
      ...
      deadline,
      ...(0,
      import_socksClientCertificatesInterceptor.getMatchingTLSOptionsForOrigin)(
    ...
    const failOnStatusCode =
      params.failOnStatusCode !== void 0
        ? params.failOnStatusCode
        : !!defaults.failOnStatusCode;
    if (
      failOnStatusCode &&
...
${text}`;
      ...
      throw new Error(
        `${fetchResponse.status} ${fetchResponse.statusText}${responseText}`,
      );
    }
    ...
    for (const header of setCookie) {
      ...
      if (!cookie.domain) cookie.domain = url.hostname;
        ...
        (0, import_utils.assert)(
          cookie.domain.startsWith(".") || !cookie.domain.includes("."),
        );
  ...
  async _sendRequestWithRetries(progress, url, options, postData, maxRetries) {
    ...
    for (let i = 0; i <= maxRetries; i++) {
      try {
        ...
        e = (0,
        import_socksClientCertificatesInterceptor.rewriteOpenSSLErrorIfNeeded)(
          e,
        );
        ...
        if (
          i === maxRetries ||
          (options.deadline &&
            (0, import_utils.monotonicTime)() + backoff > options.deadline)
        )
    ...
    return new Promise((fulfill, reject) => {
      ...
      const request = requestConstructor(
        ...
        async (response) => {
          ...
          const notifyRequestFinished = (body2) => {
            ...
            this.emit(
              APIRequestContext.Events.RequestFinished,
          ...
          if (cookies.length) {
            try {
              ...
              await Promise.all(
                cookies.map((c) => this._addCookies([c]).catch(() => {})),
              );
            }
          }
          if (
          ...
          if (transform) {
            const emptyStreamTransform = new SafeEmptyStreamTransform(
              notifyBodyFinished,
            );
            body = (0, import_stream.pipeline)(
              response,
              emptyStreamTransform,
              transform,
              (e) => {
                if (e)
                  reject(
                    new Error(
                      `failed to decompress '${encoding}' encoding: ${e.message}`,
                    ),
                  );
              },
            );
      ...
      listeners.push(
        import_utils.eventsHelper.addEventListener(
          this,
          APIRequestContext.Events.Dispose,
          () => {
      ...
      request.on("close", () =>
        import_utils.eventsHelper.removeEventListeners(listeners),
      );
      request.on("socket", (socket) => {
        ...
        const happyEyeBallsTimings = (0, import_happyEyeballs.timingForSocket)(
          socket,
        );
            ...
            () => {
              ...
              if (socket instanceof import_tls.TLSSocket) {
                ...
                securityDetails = {
                  ...
                  validFrom:
                    new Date(peerCertificate.valid_from).getTime() / 1e3,
  ...
  _getHttpCredentials(url) {
    if (
      ...
      url.origin.toLowerCase() ===
        this._defaultOptions().httpCredentials?.origin?.toLowerCase()
    )
...
class BrowserContextAPIRequestContext extends APIRequestContext {
  constructor(context) {
    ...
    context.once(import_browserContext.BrowserContext.Events.Close, () =>
      this._disposeImpl(),
    );
  }
  tracing() {
...
class GlobalAPIRequestContext extends APIRequestContext {
  constructor(playwright, options) {
    ...
    (0, import_browserContext.verifyClientCertificates)(
