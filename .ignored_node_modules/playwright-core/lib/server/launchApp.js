...
async function launchApp(browserType, options) {
  ...
  const context = await browserType.launchPersistentContext(
    ...
    "",
    {
      ...
      channel,
      noDefaultViewport:
        options.persistentContextOptions?.noDefaultViewport ?? true,
      acceptDownloads:
...
async function installAppIcon(page) {
  const icon = await import_fs.default.promises.readFile(
...
async function syncLocalStorageWithSettings(page, appName) {
  ...
  await page.exposeBinding("_saveSerializedSettings", false, (_, settings2) => {
    import_fs.default.mkdirSync(import_path.default.dirname(settingsFile), {
      recursive: true,
    });
