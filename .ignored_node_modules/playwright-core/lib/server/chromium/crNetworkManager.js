...
class CRNetworkManager {
  ...
  async _updateProtocolRequestInterception() {
    ...
    await this._forEachSession((info) =>
      this._updateProtocolRequestInterceptionForSession(info),
    );
  }
  ...
  async setExtraHTTPHeaders(extraHTTPHeaders) {
    ...
    await this._forEachSession((info) =>
      this._setExtraHTTPHeadersForSession(info),
    );
  }
  ...
  _onAuthRequired(sessionInfo, event) {
    ...
    const shouldProvideCredentials = this._shouldProvideCredentials(
      event.request.url,
    );
  ...
  _shouldProvideCredentials(url) {
    ...
    return (
      ...
      new URL(url).origin.toLowerCase() ===
        this._credentials.origin.toLowerCase()
    );
  }
  _onRequestPaused(sessionInfo, event) {
    ...
    if (requestWillBeSentEvent) {
      ...
      const alreadyContinuedParams =
        existingRequest?._route?._alreadyContinuedParams;
  ...
  _onRequest(
    ...
    if (requestWillBeSentEvent.redirectResponse) {
      const request2 = this._requestIdToRequest.get(
    ...
    if (requestPausedEvent) {
      if (
        ...
        route = new RouteImpl(
          requestPausedSessionInfo.session,
    ...
    if (route) {
      request.request.setRawRequestHeaders(
        (0, import_utils.headersObjectToArray)(
  ...
  _createResponse(request, responsePayload, hasExtraInfo) {
    const getResponseBody = async () => {
      ...
      if (response2.body || !expectedLength)
        return Buffer.from(
          response2.body,
          response2.base64Encoded ? "base64" : "utf8",
        );
  ...
  _onLoadingFailed(sessionInfo, event) {
    ...
    this._deleteRequest(request);
    request.request._setFailureText(
...
class InterceptableRequest {
  constructor(options) {
    ...
    if (entries && entries.length)
      postDataBuffer = Buffer.concat(
        entries.map((entry) => Buffer.from(entry.bytes, "base64")),
      );
...
async function catchDisallowedErrors(callback) {
  try {
    ...
    if (
      (0, import_protocolError.isProtocolError)(e) &&
      e.message.includes("Unsafe header")
    )
      throw e;
  }
}
