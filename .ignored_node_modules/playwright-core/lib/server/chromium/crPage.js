...
class CRPage {
  constructor(client, targetId, browserContext, opener, bits) {
    ...
    this._networkManager = new import_crNetworkManager.CRNetworkManager(
      this._page,
      null,
    );
    this.updateOffline();
    ...
    if (opener && !browserContext._options.noDefaultViewport) {
      ...
      const viewportSize =
        import_helper.helper.getViewportSizeFromWindowFeatures(features);
      if (viewportSize)
    ...
    const createdEvent = this._isBackgroundPage
      ? import_crBrowser.CRBrowserContext.CREvents.BackgroundPage
    ...
    this._mainFrameSession._initialize(bits.hasUIWindow).then(
      ...
      (error) =>
        this._page.reportAsNew(this._opener?._page, error, createdEvent),
    );
  }
  ...
  async updateOffline() {
    await this._networkManager.setOffline(
      !!this._browserContext._options.offline,
    );
  }
  async updateHttpCredentials() {
    await this._networkManager.authenticate(
      this._browserContext._options.httpCredentials || null,
    );
  }
  ...
  async updateRequestInterception() {
    await this._networkManager.setRequestInterception(
      this._page.needsRequestInterception(),
    );
  }
  async updateFileChooserInterception() {
    await this._forAllFrameSessions((frame) =>
      frame._updateFileChooserInterception(false),
    );
  }
  async reload() {
  ...
  async addInitScript(initScript, world = "main") {
    await this._forAllFrameSessions((frame) =>
      frame._evaluateOnNewDocument(initScript, world),
    );
  }
  async removeNonInternalInitScripts() {
    await this._forAllFrameSessions((frame) =>
      frame._removeEvaluatesOnNewDocument(),
    );
  }
  ...
  async takeScreenshot(
    ...
    if (scale === "css") {
      const deviceScaleFactor =
        this._browserContext._options.deviceScaleFactor || 1;
      clip.scale /= deviceScaleFactor;
    }
    progress.throwIfAborted();
    const result = await this._mainFrameSession._client.send(
      "Page.captureScreenshot",
      { format, quality, clip, captureBeyondViewport: !fitsViewport },
    );
  ...
  async getAccessibilityTree(needle) {
    return (0, import_crAccessibility.getAccessibilityTree)(
...
class FrameSession {
  ...
  async _initialize(hasUIWindow) {
    const isSettingStorageState =
      this._page._browserContext.isSettingStorageState();
    if (
      ...
      const { windowId } = await this._client.send(
    ...
    const promises = [
      ...
      this._client.send("Runtime.addBinding", {
        name: import_page.PageBinding.kPlaywrightBinding,
      }),
      this._client.send("Page.addScriptToEvaluateOnNewDocument", {
        source: "",
        worldName: UTILITY_WORLD_NAME,
      }),
      this._crPage._networkManager.addSession(
        this._client,
        void 0,
        this._isMainFrame(),
      ),
    ...
    if (!isSettingStorageState) {
      ...
      if (options.bypassCSP)
        promises.push(
          this._client.send("Page.setBypassCSP", { enabled: true }),
        );
  ...
  async _navigate(frame, url, referrer) {
    ...
    if (response.errorText)
      throw new frames.NavigationAbortedError(
        response.loaderId,
        `${response.errorText} at ${url}`,
      );
  ...
  _onExecutionContextCreated(contextPayload) {
    ...
    const delegate = new import_crExecutionContext.CRExecutionContext(
      this._client,
      contextPayload,
    );
    let worldName = null;
  ...
  _onAttachedToTarget(event) {
    ...
    session.once("Runtime.executionContextCreated", async (event2) => {
      worker._createExecutionContext(
        new import_crExecutionContext.CRExecutionContext(
          session,
          event2.context,
        ),
      );
    ...
    this._crPage._networkManager
      .addSession(
        session,
        this._page._frameManager.frame(this._targetId) ?? void 0,
      )
      .catch(() => {});
    ...
    session.on("Target.attachedToTarget", (event2) =>
      this._onAttachedToTarget(event2),
    );
    session.on("Target.detachedFromTarget", (event2) =>
      this._onDetachedFromTarget(event2),
    );
    session.on("Runtime.consoleAPICalled", (event2) => {
      const args = event2.args.map((o) =>
        (0, import_crExecutionContext.createHandle)(
          worker._existingExecutionContext,
          o,
        ),
      );
      this._page._addConsoleMessage(
        event2.type,
        args,
        (0, import_crProtocolHelper.toConsoleMessageLocation)(
          event2.stackTrace,
        ),
      );
    });
    session.on("Runtime.exceptionThrown", (exception) =>
      this._page.emitOnContextOnceInitialized(
        ...
        (0, import_crProtocolHelper.exceptionToError)(
  ...
  async _onConsoleAPI(event) {
    ...
    const values = event.args.map((arg) =>
      (0, import_crExecutionContext.createHandle)(context, arg),
    );
    this._page._addConsoleMessage(
      event.type,
      values,
  ...
  _onDialog(event) {
    ...
    this._page.emitOnContext(
      import_browserContext.BrowserContext.Events.Dialog,
      new dialog.Dialog(
        this._page,
        event.type,
        event.message,
  ...
  _handleException(exceptionDetails) {
    this._page.emitOnContextOnceInitialized(
      import_browserContext.BrowserContext.Events.PageError,
      (0, import_crProtocolHelper.exceptionToError)(exceptionDetails),
      this._page,
    );
  }
  ...
  _onLogEntryAdded(event) {
    ...
    if (args)
      args.map((arg) =>
        (0, import_crProtocolHelper.releaseObject)(this._client, arg.objectId),
      );
    if (source !== "worker") {
      const location = {
        url: url || "",
  ...
  _onScreencastFrame(payload) {
    ...
    this._page.emit(import_page2.Page.Events.ScreencastFrame, {
      buffer,
      frameSwapWallTime: payload.metadata.timestamp
        ? payload.metadata.timestamp * 1e3
        : void 0,
  ...
  async _createVideoRecorder(screencastId, options) {
    ...
    const ffmpegPath = import_registry.registry
      ...
      .executablePathOrDie(
        this._page.attribution.playwright.options.sdkLanguage,
      );
    this._videoRecorder = await import_videoRecorder.VideoRecorder.launch(
      this._crPage._page,
      ffmpegPath,
      options,
    );
  ...
  async _startVideoRecording(options) {
    ...
    this._page.once(import_page2.Page.Events.Close, () =>
    ...
    gotFirstFrame.then(() => {
      this._crPage._browserContext._browser._videoStarted(
        ...
        screencastId,
        options.outputFile,
        this._crPage._page.waitForInitializedOrError(),
      );
    });
  }
  async _stopVideoRecording() {
    ...
    const video =
      this._crPage._browserContext._browser._takeVideo(screencastId);
  ...
  async _updateViewport(preserveWindowBoundaries) {
    ...
    if (
      JSON.stringify(this._metricsOverride) === JSON.stringify(metricsOverride)
    )
      return;
    const promises = [
  ...
  async _updateEmulateMedia() {
    ...
    const media =
      emulatedMedia.media === "no-override" ? "" : emulatedMedia.media;
    const colorScheme =
      emulatedMedia.colorScheme === "no-override"
        ? ""
        : emulatedMedia.colorScheme;
    const reducedMotion =
      emulatedMedia.reducedMotion === "no-override"
        ? ""
        : emulatedMedia.reducedMotion;
    const forcedColors =
      emulatedMedia.forcedColors === "no-override"
        ? ""
        : emulatedMedia.forcedColors;
    const contrast =
      emulatedMedia.contrast === "no-override" ? "" : emulatedMedia.contrast;
    const features = [
  ...
  async _updateFileChooserInterception(initial) {
    ...
    await this._client
      .send("Page.setInterceptFileChooserDialog", { enabled })
      .catch(() => {});
  }
  async _evaluateOnNewDocument(initScript, world) {
    ...
    const { identifier } = await this._client.send(
      ...
      { source: initScript.source, worldName },
    );
  ...
  async _adoptBackendNodeId(backendNodeId, to) {
    ...
    return (0, import_crExecutionContext.createHandle)(
      to,
      result.object,
    ).asElement();
  }
