...
class Chromium extends import_browserType.BrowserType {
  constructor(parent) {
    ...
    if ((0, import_utils.debugMode)()) this._devtools = this._createDevTools();
  }
  ...
  async _connectOverCDPInternal(progress, endpointURL, options, onClose) {
    let headersMap;
    if (options.headers)
      headersMap = (0, import_utils.headersArrayToObject)(
        options.headers,
        false,
      );
    if (!headersMap)
  ...
  _createDevTools() {
    const directory =
      import_registry.registry.findExecutable("chromium").directory;
    return directory
  ...
  async connectToTransport(transport, options) {
    ...
    return import_crBrowser.CRBrowser.connect(
      this.attribution.playwright,
      transport,
      options,
      devtools,
    ...
    if (error.logs.includes("Missing X server"))
      error.logs =
        ...
        (0, import_ascii.wrapInASCIIBox)(
          import_browserType.kNoXServerRunningError,
          1,
        );
    if (
  ...
  _innerDefaultArgs(options) {
    ...
    if (import_os.default.platform() === "darwin") {
      ...
      if (
        options.headless &&
        (!options.channel || options.channel === "chromium-headless-shell")
      )
...
function parseSeleniumRemoteParams(env, progress) {
  try {
    ...
    progress.log(
      `<selenium> ignoring additional ${env.name} "${env.value}": ${e}`,
    );
  }
}
