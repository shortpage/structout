...
class CRBrowser extends import_browser.Browser {
  ...
  async doCreateNewContext(options) {
    ...
    if (proxy) {
      if (process.env.PLAYWRIGHT_DISABLE_FORCED_CHROMIUM_PROXIED_LOOPBACK)
        ...
        proxyBypassList =
          "<-loopback>" + (proxy.bypass ? `,${proxy.bypass}` : "");
    }
  ...
  async _waitForAllPagesToBeInitialized() {
    await Promise.all(
      [...this._crPages.values()].map((crPage) =>
        crPage._page.waitForInitializedOrError(),
      ),
    );
  }
  _onAttachedToTarget({ targetInfo, sessionId, waitingForDebugger }) {
    ...
    const treatOtherAsPage =
      targetInfo.type === "other" && process.env.PW_CHROMIUM_ATTACH_TO_OTHER;
    ...
    if (targetInfo.type === "background_page") {
      const backgroundPage = new import_crPage.CRPage(
        session,
        targetInfo.targetId,
        context,
        null,
        { hasUIWindow: false, isBackgroundPage: true },
      );
    ...
    if (targetInfo.type === "page" || treatOtherAsPage) {
      const opener = targetInfo.openerId
        ...
        : null;
      const crPage = new import_crPage.CRPage(
        session,
        targetInfo.targetId,
        context,
        opener,
        { hasUIWindow: targetInfo.type === "page", isBackgroundPage: false },
      );
...
class CRBrowserContext extends import_browserContext.BrowserContext {
    ...
    if (
      ...
      promises.push(
        this._browser._session.send("Browser.setDownloadBehavior", {
          behavior:
            this._options.acceptDownloads === "accept"
              ? "allowAndName"
              : "deny",
  ...
  _crPages() {
    return [...this._browser._crPages.values()].filter(
      (crPage) => crPage._browserContext === this,
    );
  }
  ...
  async doCreateNewPage() {
    ...
    const { targetId } = await this._browser._session.send(
      "Target.createTarget",
  ...
  async doClose(reason) {
    ...
    await Promise.all(
      openedBeforeUnloadDialogs.map((dialog) => dialog.dismiss()),
    );
  ...
  async stopVideoRecording() {
    await Promise.all(
      this._crPages().map((crPage) =>
        crPage._mainFrameSession._stopVideoRecording(),
      ),
    );
  }
  onClosePersistent() {
    for (const [
      ...
      if (
        ...
        backgroundPage._page.initializedOrUndefined()
      ) {
        backgroundPage.didClose();
