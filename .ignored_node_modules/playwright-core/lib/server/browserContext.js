...
var utilityScriptSerializers = __toESM(
  require("../utils/isomorphic/utilityScriptSerializers"),
);
  ...
  constructor(browser, options, browserContextId) {
    ...
    if (this._options.recordHar)
      this._harRecorders.set(
        "",
        new import_harRecorder.HarRecorder(this, null, this._options.recordHar),
      );
  ...
  async _initialize() {
    ...
    if ((0, import_debug.debugMode)() === "inspector")
      await import_recorder.Recorder.show(
        this,
        import_recorderApp.RecorderApp.factory(this),
        { pauseOnNextStatement: true },
      );
    if (this._debugger.isPaused())
      import_recorder.Recorder.showInspectorNoReply(
        this,
        import_recorderApp.RecorderApp.factory(this),
      );
    this._debugger.on(
      import_debugger.Debugger.Events.PausedStateChanged,
      () => {
        if (this._debugger.isPaused())
          import_recorder.Recorder.showInspectorNoReply(
            this,
...
`);
  ...
  pages() {
    return this.possiblyUninitializedPages().filter((page) =>
      page.initializedOrUndefined(),
    );
  }
  async cookies(urls = []) {
  ...
  async exposeBinding(name, needsHandle, playwrightBinding) {
    ...
    const binding = new import_page2.PageBinding(
      name,
      playwrightBinding,
      needsHandle,
    );
  ...
  async _deleteAllTempDirs() {
    await Promise.all(
      this._tempDirs.map(
        async (dir) =>
          await import_fs.default.promises.unlink(dir).catch((e) => {}),
      ),
    );
  }
  ...
  async storageState(indexedDB = false) {
    ...
    if (originsToSave.size) {
      ...
      import_instrumentation.serverSideCallMetadata)();
  ...
  async _resetStorage() {
    ...
    const newOrigins = new Map(
      this._options.storageState?.origins?.map((p) => [p.origin, p]) || [],
    );
    ...
    for (const origin of /* @__PURE__ */ new Set([
      ...oldOrigins,
      ...newOrigins.keys(),
    ])) {
      const frame = page.mainFrame();
      await frame.goto(internalMetadata, origin);
      await frame.resetStorageForCurrentOriginBestEffort(
        newOrigins.get(origin),
      );
    }
      ...
      if (state.origins && state.origins.length) {
        ...
        for (const originState of state.origins) {
          ...
          await frame.evaluateExpression(
            `(${storageScript.restore})(${utilityScriptSerializers.source}, (${import_builtins.builtins})(), ${JSON.stringify(originState)})`,
            { world: "utility" },
          );
        }
        await page.close(internalMetadata);
  ...
  async extendInjectedScript(source, arg) {
    ...
    const installInPage = (page) => {
      page.on(
        import_page2.Page.Events.InternalFrameNavigatedToNewDocument,
        installInFrame,
      );
  ...
  async _harStart(page, options) {
    ...
    this._harRecorders.set(
      harId,
      new import_harRecorder.HarRecorder(this, page, options),
    );
    return harId;
  }
...
function validateBrowserContextOptions(options, browserOptions) {
  ...
  if (options.acceptDownloads === void 0 && browserOptions.name !== "electron")
    ...
    options.acceptDownloads === void 0 &&
    browserOptions.name === "electron"
  )
