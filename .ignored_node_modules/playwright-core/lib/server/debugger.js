...
function shouldPauseOnCall(sdkObject, metadata) {
  ...
  if (
    !sdkObject.attribution.browser?.options.headful &&
...
function shouldPauseBeforeStep(metadata) {
  ...
  if (
    ...
    metadata.method === "querySelector" ||
    metadata.method === "querySelectorAll"
  )
    return false;
  const step = metadata.type + "." + metadata.method;
  return (
    import_debug.commandsWithTracingSnapshots.has(step) &&
    !import_debug.pausesBeforeInputActions.has(
