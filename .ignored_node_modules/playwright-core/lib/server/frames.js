...
class FrameManager {
  ...
  frameRequestedNavigation(frameId, documentId) {
    ...
    const request = documentId
      ? Array.from(frame._inflightRequests).find(
  ...
  frameCommittedSameDocumentNavigation(frameId, url) {
    ...
    if (
      pending &&
      pending.documentId === void 0 &&
      pending.request === void 0
    ) {
      frame.setPendingDocument(void 0);
    }
    frame._url = url;
  ...
  async closeOpenDialogs() {
    await Promise.all(
      [...this._openedDialogs].map((dialog) => dialog.close()),
    ).catch(() => {});
...
class Frame extends import_instrumentation.SdkObject {
  constructor(page, id, parentFrame) {
    ...
    this._raceAgainstEvaluationStallingEventsPromises =
      /* @__PURE__ */ new Set();
  ...
  redirectNavigation(url, documentId, referer) {
    const controller = new import_progress.ProgressController(
      (0, import_instrumentation.serverSideCallMetadata)(),
      this,
    );
    const data = {
      url,
      gotoPromise: controller.run(
        (progress) => this._gotoAction(progress, url, { referer }),
        0,
      ),
    };
  ...
  async goto(metadata, url, options = {}) {
    ...
    import_utils.constructURLBasedOnBaseURL)(
  ...
  async _gotoAction(progress, url, options) {
    ...
    if (navigateResult.newDocumentId) {
      const predicate = (event2) => {
        return (
          event2.newDocument &&
          (event2.newDocument.documentId === navigateResult.newDocumentId ||
            !event2.error)
        );
      };
  ...
  _context(world) {
    return this._contextData
      .get(world)
      .contextPromise.then((contextOrDestroyedReason) => {
  ...
  async waitForSelector(metadata, selector, options, scope) {
    ...
    return controller.run(async (progress) => {
      ...
      return await this.waitForSelectorInternal(
        progress,
        selector,
        true,
        options,
  ...
  async waitForSelectorInternal(
        ...
        const result = await resolved.injected.evaluateHandle(
          (injected, { info, root }) => {
            ...
            const visible2 = element2
              ? injected.utils.isElementVisible(element2)
              : false;
            let log2 = "";
    ...
    return scope
      ? scope._context._raceAgainstContextDestroyed(promise)
      : promise;
  }
  async dispatchEvent(
    metadata,
  ...
  async content() {
    try {
      ...
      if (
        ...
        (0, import_protocolError.isSessionClosedError)(e)
      )
        throw e;
      throw new Error(
  ...
  _isErrorThatCannotBeRetried(e) {
    if (
      ...
      return true;
    if (
      dom.isNonRecoverableDOMError(e) ||
      (0, import_selectorParser.isInvalidSelectorError)(e)
    )
      return true;
  ...
  async rafrafTimeoutScreenshotElementWithProgress(
    ...
    return await this._retryWithProgressIfNotConnected(
      ...
      async (handle) => {
        ...
        return await this._page._screenshotter.screenshotElement(
          progress,
          handle,
          options,
        );
  ...
  async setInputFiles(metadata, selector, params) {
    ...
    import_fileUploadUtils.prepareFilesForUpload)(this, params);
    const controller = new import_progress.ProgressController(metadata, this);
    return controller.run(async (progress) => {
      return dom.assertDone(
        await this._retryWithProgressIfNotConnected(
  ...
  async ariaSnapshot(metadata, selector, options = {}) {
    ...
    return controller.run(async (progress) => {
      return await this._retryWithProgressIfNotConnected(
        ...
        true,
        true,
        (handle) => handle.ariaSnapshot(options),
      );
    }, this._page._timeoutSettings.timeout(options));
  }
  ...
  async _expectImpl(metadata, selector, options) {
    ...
    try {
      ...
      await new import_progress.ProgressController(metadata, this).run(
        async (progress) => {
          progress.log(
            `${metadata.apiName}${timeout ? ` with timeout ${timeout}ms` : ""}`,
          );
          progress.log(`waiting for ${this._asLocator(selector)}`);
          await this._page.performActionPreChecks(progress);
        },
      ...
      try {
        const resultOneShot = await new import_progress.ProgressController(
          metadata,
          this,
        ).run(async (progress) => {
          return await this._expectInternal(
            progress,
            selector,
            options,
      ...
      if (timeout < 0)
        return {
          ...
          log: (0, import_callLog.compressCallLog)(metadata.log),
          timedOut: true,
          received: lastIntermediateResult.received,
        };
      return await new import_progress.ProgressController(metadata, this).run(
        async (progress) => {
  ...
  async _expectInternal(progress, selector, options, lastIntermediateResult) {
    const selectorInFrame = await this.selectors.resolveFrameForSelector(
      selector,
      { strict: true },
    );
    progress.throwIfAborted();
    ...
    const { log, matches, received, missingReceived } = await injected.evaluate(
      async (injected2, { info: info2, options: options2, callId }) => {
        const elements = info2
          ? injected2.querySelectorAll(info2.parsed, document)
          : [];
        if (callId) injected2.markTargetElements(new Set(elements), callId);
        const isArray =
          options2.expression === "to.have.count" ||
          options2.expression.endsWith(".array");
        let log2 = "";
        if (isArray)
    ...
    if (matches === options.isNot) {
      ...
      if (!missingReceived && !Array.isArray(received))
        progress.log(
          `  unexpected value "${renderUnexpectedValue(options.expression, received)}"`,
        );
    }
    return { matches, received };
  }
    ...
    return controller.run(async (progress) => {
      return this.retryWithProgressAndTimeouts(progress, [100], async () => {
        ...
        progress.cleanupWhenAborted(() =>
          handle.evaluate((h) => h.abort()).catch(() => {}),
        );
