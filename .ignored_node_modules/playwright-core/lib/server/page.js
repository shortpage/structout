...
class Page extends import_instrumentation.SdkObject {
  ...
  async reportAsNew(
    opener,
    error = void 0,
    contextEvent = import_browserContext.BrowserContext.Events.Page,
  ) {
    if (opener) {
  ...
  emitOnContextOnceInitialized(event, ...args) {
    ...
    else this._eventsToEmitAfterInitialized.push({ event, args });
  }
  ...
  async _onFileChooserOpened(handle) {
    ...
    const fileChooser = new import_fileChooser.FileChooser(
      this,
      handle,
      multiple,
    );
  ...
  async _performWaitForNavigationCheck(progress) {
    ...
    const toUrl = url
      ? `" ${(0, import_utils.trimStringWithEllipsis)(url, 200)}"`
      : "";
  ...
  async _performLocatorHandlersCheckpoint(progress) {
    ...
    for (const [uid, handler] of this._locatorHandlers) {
      ...
      if (handler.resolved) {
        ...
        progress.log(
          `  found ${(0, import_utils2.asLocator)(this.attribution.playwright.options.sdkLanguage, handler.selector)}, intercepting action to run the handler`,
        );
        const promise = handler.resolved.then(async () => {
          progress.throwIfAborted();
          if (!handler.noWaitAfter) {
            progress.log(
              `  locator handler has finished, waiting for ${(0, import_utils2.asLocator)(this.attribution.playwright.options.sdkLanguage, handler.selector)} to be hidden`,
            );
            await this.mainFrame().waitForSelectorInternal(
              progress,
              handler.selector,
              false,
              { state: "hidden" },
  ...
  async expectScreenshot(metadata, options = {}) {
    ...
    const rafrafScreenshot = locator
      ...
      : async (progress, timeout) => {
          ...
          return await this._screenshotter.screenshotPage(
            progress,
            options || {},
          );
        };
    ...
    try {
      const format = (0, import_screenshotter.validateScreenshotOptions)(
        options || {},
      );
      if (format !== "png")
    ...
    const areEqualScreenshots = (actual, expected, previous) => {
      const comparatorResult =
        actual && expected ? comparator(actual, expected, options) : void 0;
    ...
    return controller
      .run(async (progress) => {
        ...
        progress.log(
          `${metadata.apiName}${callTimeout ? ` with timeout ${callTimeout}ms` : ""}`,
        );
        if (options.expected)
        ...
        while (true) {
          ...
          if (screenshotTimeout)
            progress.log(
              `waiting ${screenshotTimeout}ms before taking screenshot`,
            );
          previous = actual;
          actual = await rafrafScreenshot(progress, screenshotTimeout).catch(
            (e) => {
          ...
          if (!actual) continue;
          const expectation =
            options.expected && isFirstIteration ? options.expected : previous;
      ...
      .catch((e) => {
        ...
        if (
          e instanceof import_errors.TimeoutError &&
          intermediateResult?.previous
        )
          errorMessage = `Failed to take two consecutive stable screenshots.`;
        return {
          log: (0, import_callLog.compressCallLog)(
            e.message ? [...metadata.log, e.message] : metadata.log,
          ),
          ...intermediateResult,
          errorMessage,
  ...
  async hideHighlight() {
    await Promise.all(
      this.frames().map((frame) => frame.hideHighlight().catch(() => {})),
    );
  }
...
class Worker extends import_instrumentation.SdkObject {
  ...
  _createExecutionContext(delegate) {
    this._existingExecutionContext = new js.ExecutionContext(
      this,
      delegate,
      "worker",
    );
...
class PageBinding {
  ...
  constructor(name, playwrightFunction, needsHandle) {
    ...
    this.initScript = new InitScript(
      (0, import_pageBinding.createPageBindingScript)(
        PageBinding.kPlaywrightBinding,
        name,
        needsHandle,
      ),
      true,
    ...
    try {
      ...
      if (binding.needsHandle) {
        const handle = await context
          .evaluateHandle(import_pageBinding.takeBindingHandle, { name, seq })
          .catch((e) => null);
        result = await binding.playwrightFunction(
          { frame: context.frame, page, context: page._browserContext },
          handle,
        );
      } else {
        ...
        const args = serializedArgs.map((a) =>
          js.parseEvaluationResultValue(a),
        );
        result = await binding.playwrightFunction(
          { frame: context.frame, page, context: page._browserContext },
          ...args,
      ...
      context
        .evaluate(import_pageBinding.deliverBindingResult, {
          name,
          seq,
          result,
        })
      ...
      context
        .evaluate(import_pageBinding.deliverBindingResult, { name, seq, error })
        .catch((e) => import_debugLogger.debugLogger.log("error", e));
    }
  }
}
