...
if (process.env.PW_TEST_CDN_THAT_SHOULD_WORK) {
  for (let i = 0; i < PLAYWRIGHT_CDN_MIRRORS.length; i++) {
    ...
    if (cdn !== process.env.PW_TEST_CDN_THAT_SHOULD_WORK) {
      ...
      parsedCDN.hostname =
        parsedCDN.hostname + ".does-not-resolve.playwright.dev";
...
const registryDirectory = (() => {
  ...
  if (envDefined === "0") {
    result = import_path.default.join(
      __dirname,
      "..",
      "..",
      "..",
    ...
    if (process.platform === "linux")
      ...
      cacheDirectory = import_path.default.join(
...
function readDescriptors(browsersJSON) {
  return browsersJSON["browsers"].map((obj) => {
    ...
    const revisionOverride = (obj.revisionOverrides || {})[
      import_hostPlatform.hostPlatform
    ];
...
class Registry {
  constructor(browsersJSON) {
    ...
    const executablePathOrDie = (name, e, installByDefault, sdkLanguage) => {
      if (!e)
        throw new Error(
          `${name} is not supported on ${import_hostPlatform.hostPlatform}`,
        );
      ...
      if (!(0, import_fileUtils.canAccessFile)(e)) {
        const currentDockerVersion = (0,
        import_dependencies.readDockerVersionSync)();
        const preferredDockerVersion = currentDockerVersion
          ? (0, import_dependencies.dockerVersion)(
              currentDockerVersion.dockerImageNameTemplate,
            )
          : null;
        const isOutdatedDockerImage =
          currentDockerVersion &&
          preferredDockerVersion &&
          currentDockerVersion.dockerImageName !==
            preferredDockerVersion.dockerImageName;
...
${(0, import_ascii.wrapInASCIIBox)(prettyMessage, 1)}`);
    ...
    const chromiumHeadlessShell = descriptors.find(
      (d) => d.name === "chromium-headless-shell",
    );
    ...
    const chromiumTipOfTree = descriptors.find(
      (d) => d.name === "chromium-tip-of-tree",
    );
    ...
    this._executables.push({
      ...
      _install: () =>
        this._downloadExecutable(firefoxBeta, firefoxBetaExecutable),
      _dependencyGroup: "firefox",
      _isHermeticInstallation: true,
    });
    ...
    this._executables.push({
      ...
      executablePathOrDie: (sdkLanguage) =>
        executablePathOrDie(
          "winldd",
          winlddExecutable,
          winldd.installByDefault,
          sdkLanguage,
        ),
      installType:
        process.platform === "win32" ? "download-by-default" : "none",
      _validateHostRequirements: () => Promise.resolve(),
...
Run "${buildPlaywrightCLICommand(sdkLanguage, "install " + name)}"`
  ...
  _createBidiFirefoxChannel(name, lookAt, install) {
    const executablePath = (sdkLanguage, shouldThrow) => {
      ...
      if (shouldThrow)
        throw new Error(
          `Cannot find Firefox installation for channel '${name}' at the standard system paths.`,
        );
      return void 0;
    };
    return {
...
Run "${buildPlaywrightCLICommand(sdkLanguage, "install " + name)}"`
    ...
    if (import_os.default.platform() === "linux")
      return await (0, import_dependencies2.validateDependenciesLinux)(
        ...
        linuxLddDirectories.map((d) =>
          import_path.default.join(browserDirectory, d),
        ),
        dlOpenLibraries,
      );
    if (
      import_os.default.platform() === "win32" &&
      import_os.default.arch() === "x64"
    )
  ...
  async installDeps(executablesToInstallDeps, dryRun) {
    ...
    if (import_os.default.platform() === "win32")
      return await (0, import_dependencies2.installDependenciesWindows)(
        targets,
        dryRun,
      );
    if (import_os.default.platform() === "linux")
      return await (0, import_dependencies2.installDependenciesLinux)(
        targets,
        dryRun,
      );
  }
  async install(executablesToInstall, forceReinstall) {
    ...
    await import_fs.default.promises.mkdir(registryDirectory, {
      recursive: true,
    });
    const lockfilePath = import_path.default.join(
      registryDirectory,
      "__dirlock",
    );
    ...
    try {
      ...
      await import_fs.default.promises.writeFile(
        import_path.default.join(
          linksDir,
          (0, import_utils.calculateSha1)(PACKAGE_PATH),
        ),
        PACKAGE_PATH,
      );
  ...
  async uninstall(all) {
    const linksDir = import_path.default.join(registryDirectory, ".links");
    if (all) {
      const links = await import_fs.default.promises
        .readdir(linksDir)
        .catch(() => []);
      for (const link of links)
        await import_fs.default.promises.unlink(
          import_path.default.join(linksDir, link),
        );
    } else {
    ...
    return {
      numberOfBrowsersLeft: (
        await import_fs.default.promises
          .readdir(registryDirectory)
          .catch(() => [])
      ).filter((browserDirectory) => isBrowserDirectory(browserDirectory))
        .length,
    };
  }
  async validateHostRequirementsForExecutablesIfNeeded(
    executables,
    sdkLanguage,
  ) {
    if (
      (0, import_utils.getAsBooleanFromENV)(
    ...
    for (const executable of executables)
      await this._validateHostRequirementsForExecutableIfNeeded(
        executable,
        sdkLanguage,
      );
  }
  async _validateHostRequirementsForExecutableIfNeeded(
    executable,
    sdkLanguage,
  ) {
    const kMaximumReValidationPeriod = 30 * 24 * 60 * 60 * 1e3;
    if (!executable.directory) return;
    const markerFile = import_path.default.join(
    ...
    try {
      ...
      import_debugLogger.debugLogger.log(
        "install",
        `validation passed for ${executable.name}`,
      );
    } catch (error) {
      import_debugLogger.debugLogger.log(
        "install",
        `validation failed for ${executable.name}`,
      );
      throw error;
    }
  ...
  _downloadURLs(descriptor) {
    ...
    const downloadPathTemplate =
      paths[import_hostPlatform.hostPlatform] || paths["<unknown>"];
...
${e.stack}`);
  ...
  async _installMSEdgeChannel(channel, scripts) {
    ...
    if (process.platform !== "linux") {
      ...
      const product = products.find(
        (product2) => product2.product === productName,
      );
      const searchConfig = {
      ...
      const artifact = release
        ? release.artifacts.find(
            (artifact2) => artifact2.artifactname === searchConfig.artifact,
          )
        : null;
      if (artifact)
        scriptArgs.push(
  ...
  async _validateInstallationCache(linksDir) {
    ...
    for (const fileName of await import_fs.default.promises.readdir(linksDir)) {
      ...
      try {
        linkTarget = (
          await import_fs.default.promises.readFile(linkPath)
        ).toString();
        const browsersJSON = require(
          import_path.default.join(linkTarget, "browsers.json"),
        );
        const descriptors = readDescriptors(browsersJSON);
        for (const browserName of allDownloadable) {
          ...
          const shouldHaveMarkerFile =
            (browserName === "chromium" &&
              (browserRevision >= 786218 || browserRevision < 3e5)) ||
            (browserName === "firefox" && browserRevision >= 1128) ||
            (browserName === "webkit" && browserRevision >= 1307) || // All new applications have a marker file right away.
            (browserName !== "firefox" &&
              browserName !== "chromium" &&
              browserName !== "webkit");
          if (
            ...
            (await (0, import_fileUtils.existsAsync)(
              browserDirectoryToMarkerFilePath(usedBrowserPath),
            ))
          )
    ...
    if (!(0, import_utils.getAsBooleanFromENV)("PLAYWRIGHT_SKIP_BROWSER_GC")) {
      let downloadedBrowsers = (
        await import_fs.default.promises.readdir(registryDirectory)
      ).map((file) => import_path.default.join(registryDirectory, file));
...
function buildPlaywrightCLICommand(sdkLanguage, parameters) {
  switch (sdkLanguage) {
    ...
    default: {
      ...
      import_utils.getPackageManagerExecCommand)();
...
async function installBrowsersForNpmInstall(browsers) {
  if (
    (0, import_utils.getAsBooleanFromENV)("PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD")
  ) {
    (0, import_browserFetcher.logPolitely)(
