...
async function downloadBrowserWithProgressBar(
  ...
  const zipPath = import_path.default.join(
  ...
  try {
    ...
    for (let attempt = 1; attempt <= retryCount; ++attempt) {
      import_debugLogger.debugLogger.log(
        "install",
        `downloading ${title} - attempt #${attempt}`,
      );
      const url = downloadURLs[(attempt - 1) % downloadURLs.length];
      logPolitely(
        `Downloading ${title}` + import_utilsBundle.colors.dim(` from ${url}`),
      );
      ...
      if (await (0, import_fileUtils.existsAsync)(browserDirectory))
        await import_fs.default.promises.rmdir(browserDirectory, {
          recursive: true,
        });
      const errorMessage = error?.message || "";
      import_debugLogger.debugLogger.log(
        "install",
        `attempt #${attempt} - ERROR: ${errorMessage}`,
      );
    ...
    import_debugLogger.debugLogger.log(
      "install",
      `FAILED installation ${title} with error: ${e}`,
    );
    process.exitCode = 1;
    throw e;
  } finally {
...
function downloadBrowserWithProgressBarOutOfProcess(
  ...
  const cp = childProcess.fork(
    import_path.default.join(__dirname, "oopDownloadBrowserMain.js"),
  );
