...
const BIN_DIRECTORY = import_path.default.join(
  __dirname,
  "..",
  "..",
  "..",
...
async function writeDockerVersion(dockerImageNameTemplate) {
  await import_fs.default.promises.mkdir(
    import_path.default.dirname(dockerVersionFilePath),
    { recursive: true },
  );
...
function dockerVersion(dockerImageNameTemplate) {
  return {
    ...
    dockerImageName: dockerImageNameTemplate.replace(
      "%version%",
      languageBindingVersion,
    ),
  };
}
function readDockerVersionSync() {
  try {
    const data = JSON.parse(
      import_fs.default.readFileSync(dockerVersionFilePath, "utf8"),
    );
    return {
      ...data,
      dockerImageNameTemplate: data.dockerImageName.replace(
        data.driverVersion,
        "%version%",
      ),
    };
...
async function installDependenciesLinux(targets, dryRun) {
  ...
  const { command, args, elevatedPermissions } =
...
async function validateDependenciesWindows(
  sdkLanguage,
  ...
  const allMissingDeps = await Promise.all(
    lddPaths.map((lddPath) =>
      missingFileDependenciesWindows(sdkLanguage, lddPath),
    ),
  );
...
async function validateDependenciesLinux(
  ...
  const libraryToPackageNameMapping = import_nativeDeps.deps[
    import_hostPlatform.hostPlatform
  ]
    ? {
        ...(import_nativeDeps.deps[import_hostPlatform.hostPlatform]
          ?.lib2package || {}),
  ...
  const maybeSudo =
    process.getuid?.() && import_os.default.platform() !== "win32"
      ? "sudo "
      : "";
...
async function executablesOrSharedLibraries(directoryPath) {
  ...
  const allPaths = (
    await import_fs.default.promises.readdir(directoryPath)
  ).map((file) => import_path.default.resolve(directoryPath, file));
  const allStats = await Promise.all(
    allPaths.map((aPath) => import_fs.default.promises.stat(aPath)),
  );
...
async function missingFileDependenciesWindows(sdkLanguage, filePath) {
  const executable = import__.registry
    .findExecutable("winldd")
    .executablePathOrDie(sdkLanguage);
  const dirname = import_path.default.dirname(filePath);
  const { stdout, code } = await (0, import_spawnAsync.spawnAsync)(
    executable,
    [filePath],
    {
      cwd: dirname,
