    ...
    (response) => {
      ...
      file.on("finish", () => {
        if (downloadedBytes !== totalBytes) {
          log(
            `-- download failed, size mismatch: ${downloadedBytes} != ${totalBytes}`,
          );
          promise.reject(
            new Error(
      ...
      response.on("error", (error) => {
        ...
        if (error?.code === "ECONNRESET") {
          ...
          promise.reject(
            new Error(
              `Download failed: server closed connection. URL: ${options.url}`,
            ),
          );
        } else {
          ...
          promise.reject(
            new Error(
              `Download failed: ${error?.message ?? error}. URL: ${options.url}`,
            ),
          );
        }
      });
...
async function main(options) {
  ...
  await (0, import_zipBundle.extract)(options.zipPath, {
