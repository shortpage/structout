...
const utilityScriptSerializers = (0, import_utilityScriptSerializers.source)(
  (0, import_builtins.builtins)(),
);
const parseEvaluationResultValue =
  utilityScriptSerializers.parseEvaluationResultValue;
const serializeAsCallArgument =
  utilityScriptSerializers.serializeAsCallArgument;
class ExecutionContext extends import_instrumentation.SdkObject {
  ...
  rawEvaluateJSON(expression) {
    return this._raceAgainstContextDestroyed(
      this.delegate.rawEvaluateJSON(expression),
    );
  }
  rawEvaluateHandle(expression) {
    return this._raceAgainstContextDestroyed(
      this.delegate.rawEvaluateHandle(this, expression),
    );
  }
  ...
  getProperties(object) {
    return this._raceAgainstContextDestroyed(
      this.delegate.getProperties(object),
    );
  }
  ...
  _utilityScript() {
    if (!this._utilityScriptPromise) {
      ...
      this._utilityScriptPromise = this._raceAgainstContextDestroyed(
        this.delegate.rawEvaluateHandle(this, source2),
      ).then((handle) => {
...
async function evaluateExpression(context, expression, options, ...args) {
  ...
  args = args.map((arg) =>
    serializeAsCallArgument(arg, (handle) => {
