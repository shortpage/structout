  ...
  class Streamer {
    ...
    _refreshListenersWhenNeeded() {
      ...
      const customEventName =
        "__playwright_snapshotter_global_listeners_check__";
      let seenEvent = false;
    ...
    __sanitizeMetaAttribute(name, value, httpEquiv) {
      ...
      const charsetParamIdx = params.findIndex((param) =>
        param.trim().startsWith("charset="),
      );
      ...
      const visitNode = (node) => {
        ...
        if (nodeType === Node.ELEMENT_NODE) {
          ...
          if (
            ...
            window.customElements?.get(element.localName)
          )
        ...
        if (nodeName === "IMG" || nodeName === "PICTURE") {
          const value =
            nodeName === "PICTURE" ? "" : this._sanitizeUrl(node.currentSrc);
