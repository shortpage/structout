...
class Tracing extends import_instrumentation.SdkObject {
  constructor(context, tracesDir) {
    ...
    const testIdAttributeName =
      "selectors" in context
        ? context.selectors().testIdAttributeName()
        : void 0;
    ...
    if (context instanceof import_browserContext.BrowserContext) {
      ...
      (0, import_assert.assert)(
        tracesDir,
        "tracesDir must be specified for BrowserContext",
      );
  ...
  _onConsoleMessage(message) {
    const event = {
      ...
      args: message
        .args()
        .map((a) => ({ preview: a.toString(), value: a.rawValue() })),
  ...
  _appendResource(sha1, buffer) {
    ...
    const resourcePath = import_path.default.join(
      this._state.resourcesDir,
      sha1,
    );
    this._fs.writeFile(
      resourcePath,
...
function shouldCaptureSnapshot(metadata) {
  return import_debug.commandsWithTracingSnapshots.has(
