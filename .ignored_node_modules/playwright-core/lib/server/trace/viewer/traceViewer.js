...
function validateTraceUrls(traceUrls) {
  for (const traceUrl of traceUrls) {
    ...
    if (
      ...
      !import_fs.default.existsSync(traceFile) &&
      !import_fs.default.existsSync(traceFile + ".trace")
    )
...
async function startTraceViewerServer(options) {
    ...
    if (relativePath.startsWith("/file")) {
      try {
        ...
        if (filePath.endsWith(".json")) {
          const traceName = filePath.substring(
            0,
  ...
  const transport =
    options?.transport || (options?.isServer ? new StdinServer() : void 0);
...
async function installRootRedirect(server, traceUrls, options) {
  ...
  server.routePath("/", (_, response) => {
    ...
    if (process.env.EXPERIMENTAL_OPENAI_API_KEY)
      response.appendHeader(
        "Set-Cookie",
        `openai_api_key=${process.env.EXPERIMENTAL_OPENAI_API_KEY}`,
      );
    ...
    if (process.env.EXPERIMENTAL_ANTHROPIC_API_KEY)
      response.appendHeader(
        "Set-Cookie",
        `anthropic_api_key=${process.env.EXPERIMENTAL_ANTHROPIC_API_KEY}`,
      );
...
async function openTraceViewerApp(url, browserName, options) {
  ...
  const traceViewerBrowser = (0, import_utils2.isUnderTest)()
    ? "chromium"
    : browserName;
  ...
  if (process.env.PWTEST_PRINT_WS_ENDPOINT)
    process.stderr.write(
      "DevTools listening on: " + context._browser.options.wsEndpoint + "\n",
    );
  if (!(0, import_utils2.isUnderTest)())
    await (0, import_launchApp.syncLocalStorageWithSettings)(
      page,
      "traceviewer",
    );
  if ((0, import_utils2.isUnderTest)())
    page.on("close", () =>
      context.close({ reason: "Trace viewer closed" }).catch(() => {}),
    );
  await page
    .mainFrame()
    .goto((0, import_instrumentation.serverSideCallMetadata)(), url);
  return page;
}
async function openTraceInBrowser(url) {
  console.log("\nListening on " + url);
  if (!(0, import_utils2.isUnderTest)())
    await (0, import_utilsBundle.open)(
...
function traceDescriptor(traceName) {
  ...
  for (const name of import_fs.default.readdirSync(traceDir)) {
    if (name.startsWith(traceFile))
      result.entries.push({
        name,
        path: import_path.default.join(traceDir, name),
      });
  }
  ...
  if (import_fs.default.existsSync(resourcesDir)) {
    for (const name of import_fs.default.readdirSync(resourcesDir))
      result.entries.push({
        name: "resources/" + name,
        path: import_path.default.join(resourcesDir, name),
      });
  }
  return result;
}
