...
class FrameSelectors {
  ...
  async resolveFrameForSelector(selector, options = {}, scope) {
    ...
    const frameChunks = (0, import_selectorParser.splitSelectorByFrame)(
      selector,
    );
    for (const chunk of frameChunks) {
      (0, import_selectorParser.visitAllSelectorParts)(
        chunk,
        (part, nested) => {
          if (
            nested &&
    ...
    for (let i = 0; i < frameChunks.length - 1; ++i) {
      ...
      const handle = await injectedScript.evaluateHandle(
        (injected, { info: info2, scope: scope2, selectorString }) => {
          const element2 = injected.querySelector(
            info2.parsed,
            scope2 || document,
            info2.strict,
          );
          if (
            element2 &&
            element2.nodeName !== "IFRAME" &&
            element2.nodeName !== "FRAME"
          )
        ...
        },
        {
          ...
          selectorString: (0, import_selectorParser.stringifySelector)(
            info.parsed,
          ),
        },
      );
    ...
    return {
      frame,
      info: frame.selectors._parseSelector(
        frameChunks[frameChunks.length - 1],
        options,
      ),
      scope,
    };
  ...
  async resolveInjectedForSelector(selector, options, scope) {
    const resolved = await this.resolveFrameForSelector(
      selector,
      options,
      scope,
    );
...
async function adoptIfNeeded(handle, context) {
  ...
  const adopted = await handle._page._delegate.adoptElementHandle(
    handle,
    context,
  );
  handle.dispose();
