...
async function zip(stackSessions, params) {
  ...
  const stackSession = params.stacksId
    ? stackSessions.get(params.stacksId)
    : void 0;
  if (stackSession?.callStacks.length) {
    ...
    if (process.env.PW_LIVE_TRACE_STACKS) {
      ...
      const buffer = Buffer.from(
        JSON.stringify(
          (0, import_traceUtils.serializeClientSideCallMetadata)(
  ...
  if (params.includeSources) {
    ...
    for (const sourceFile of sourceFiles)
      addFile(
        sourceFile,
        "resources/src@" +
          (await (0, import_crypto.calculateSha1)(sourceFile)) +
          ".txt",
      );
  }
  if (params.mode === "write") {
    await import_fs.default.promises.mkdir(
      import_path.default.dirname(params.zipFile),
      { recursive: true },
    );
    zipFile.end(void 0, () => {
...
async function harOpen(harBackends, params) {
  ...
  if (params.file.endsWith(".zip")) {
    ...
    const harFile = JSON.parse(
      await import_fs.default.promises.readFile(params.file, "utf-8"),
    );
  ...
  for (const entry of await zipFile.entries()) {
    ...
    if (entry === "har.har")
      ...
      await import_fs.default.promises.writeFile(
        import_path.default.join(dir, entry),
        buffer,
      );
  }
  zipFile.close();
...
async function tracingStarted(stackSessions, params) {
  let tmpDir = void 0;
  if (!params.tracesDir)
    tmpDir = await import_fs.default.promises.mkdtemp(
