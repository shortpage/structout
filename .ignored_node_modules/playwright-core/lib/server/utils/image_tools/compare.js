...
function compare(actual, expected, diff, width, height, options = {}) {
  ...
  const [r1, g1, b1] = import_imageChannel.ImageChannel.intoRGB(
    width,
    height,
    expected,
    {
  ...
  const [r2, g2, b2] = import_imageChannel.ImageChannel.intoRGB(
    width,
    height,
    actual,
    {
  ...
  const drawRedPixel = diff
    ? (x, y) =>
        drawPixel(width, diff, x - paddingSize, y - paddingSize, 255, 0, 0)
    : noop;
  const drawYellowPixel = diff
    ? (x, y) =>
        drawPixel(width, diff, x - paddingSize, y - paddingSize, 255, 255, 0)
    : noop;
  const drawGrayPixel = diff
    ? (x, y) => {
        const gray = (0, import_colorUtils.rgb2gray)(
          r1.get(x, y),
          g1.get(x, y),
          b1.get(x, y),
        );
  ...
  for (let y = paddingSize; y < r1.height - paddingSize; ++y) {
    for (let x = paddingSize; x < r1.width - paddingSize; ++x) {
      ...
      const [varX1, varY1] = r1.boundXY(
        x - VARIANCE_WINDOW_RADIUS,
        y - VARIANCE_WINDOW_RADIUS,
      );
      const [varX2, varY2] = r1.boundXY(
        x + VARIANCE_WINDOW_RADIUS,
        y + VARIANCE_WINDOW_RADIUS,
      );
      const var1 =
      ...
      const [ssimX1, ssimY1] = r1.boundXY(
        x - SSIM_WINDOW_RADIUS,
        y - SSIM_WINDOW_RADIUS,
      );
      const [ssimX2, ssimY2] = r1.boundXY(
        x + SSIM_WINDOW_RADIUS,
        y + SSIM_WINDOW_RADIUS,
      );
      const ssimRGB =
