...
function httpRequest(params, onResponse, onError) {
  ...
  const request =
    options.protocol === "https:"
      ? import_https.default.request(options, requestCallback)
      : import_http.default.request(options, requestCallback);
  request.on("error", onError);
  if (timeout !== void 0) {
    const rejectOnTimeout = () => {
      onError(
        new Error(`Request to ${params.url} timed out after ${timeout}ms`),
      );
      request.abort();
    };
    if (timeout <= 0) {
...
async function isURLAvailable(url2, ignoreHTTPSErrors, onLog, onStdErr) {
  ...
  if (statusCode === 404 && url2.pathname === "/") {
    ...
    statusCode = await httpStatusCode(
      indexUrl,
      ignoreHTTPSErrors,
      onLog,
      onStdErr,
  ...
  return new Promise((resolve) => {
    ...
    httpRequest(
      ...
      (error) => {
        ...
        onLog?.(
          `Error while checking if ${url2} is available: ${error.message}`,
        );
        resolve(0);
      },
    );
