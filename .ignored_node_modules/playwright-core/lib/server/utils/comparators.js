...
function compareImages(mimeType, actualBuffer, expectedBuffer, options = {}) {
  ...
  const diff2 = new import_utilsBundle3.PNG({
    width: size.width,
    height: size.height,
  });
  let count;
  if (options.comparator === "ssim-cie94") {
    ...
    throw new Error(
      `Configuration specifies unknown comparator "${options.comparator}"`,
    );
  }
  ...
  const ratio =
    Math.ceil((count / (expected.width * expected.height)) * 100) / 100;
