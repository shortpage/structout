...
var SocksReply = /* @__PURE__ */ ((SocksReply2) => {
  ...
  SocksReply2[(SocksReply2["GeneralServerFailure"] = 1)] =
  ...
  SocksReply2[(SocksReply2["AddressTypeNotSupported"] = 8)] =
...
class SocksConnection {
  ...
  socketConnected(host, port) {
    ...
    this._socket.on("data", (data) =>
      this._client.onSocketData({ uid: this._uid, data }),
    );
  }
...
function parsePattern(pattern) {
  ...
  const matchers = pattern.split(",").map((token) => {
    ...
    if (
      import_net.default.isIPv4(tokenHost) ||
      import_net.default.isIPv6(tokenHost)
    )
...
class SocksProxy extends import_events.default {
  ...
  async _handleDirect(request) {
    try {
      const socket = await (0, import_happyEyeballs.createSocket)(
        request.host,
        request.port,
      );
      socket.on("data", (data) =>
        this._connections.get(request.uid)?.sendData(data),
      );
      ...
      this._connections
        .get(request.uid)
        ?.socketConnected(localAddress, localPort);
    } catch (error) {
...
class SocksProxyHandler extends import_events.default {
  ...
  async socketRequested({ uid, host, port }) {
    import_debugLogger.debugLogger.log(
      "socks",
      `[${uid}] => request ${host}:${port}`,
    );
    if (!this._patternMatcher(host, port)) {
      const payload = { uid, errorCode: "ERULESET" };
      import_debugLogger.debugLogger.log(
        "socks",
        `[${uid}] <= pattern error ${payload.errorCode}`,
      );
    ...
    try {
      ...
      import_debugLogger.debugLogger.log(
        "socks",
        `[${uid}] <= connect error ${payload.errorCode}`,
      );
      this.emit(SocksProxyHandler.Events.SocksFailed, payload);
    }
  }
