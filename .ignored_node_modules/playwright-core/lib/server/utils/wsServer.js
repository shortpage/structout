...
class WSServer {
  ...
  async listen(port = 0, hostname, path) {
    import_debugLogger.debugLogger.log(
      "server",
      `Server started at ${/* @__PURE__ */ new Date()}`,
    );
    ...
    server.on("error", (error) =>
      import_debugLogger.debugLogger.log("server", String(error)),
    );
    this.server = server;
    ...
    if (this._delegate.onHeaders)
      this._wsServer.on("headers", (headers) =>
        this._delegate.onHeaders(headers),
      );
...
`);
    ...
    this._wsServer.on("connection", (ws, request) => {
      ...
      import_debugLogger.debugLogger.log(
        "server",
        `[${id}] serving connection: ${request.url}`,
      );
