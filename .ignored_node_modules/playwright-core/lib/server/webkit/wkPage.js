...
class WKPage {
  constructor(browserContext, pageProxySession, opener) {
    ...
    this.rawTouchscreen = new import_wkInput.RawTouchscreenImpl(
      pageProxySession,
    );
    ...
    if (
      ...
      const viewportSize =
        import_helper.helper.getViewportSizeFromWindowFeatures(
          opener._nextWindowOpenPopupFeatures,
        );
  ...
  async _initializePageProxySession() {
    ...
    if (this._browserContext._options.recordVideo) {
      const outputFile = import_path.default.join(
        ...
        (0, import_crypto.createGuid)() + ".webm",
      );
      promises.push(
  ...
  _setSession(session) {
    import_eventsHelper.eventsHelper.removeEventListeners(
      this._sessionListeners,
    );
  ...
  async _initializeSessionMayThrow(session, resourceTreeHandler) {
    ...
    const promises = [
      ...
      session
        .send("Page.createUserWorld", { name: UTILITY_WORLD_NAME })
        .catch((_) => {}),
    ...
    if (this._page.needsRequestInterception()) {
      promises.push(
        session.send("Network.setInterceptionEnabled", { enabled: true }),
      );
      promises.push(
        session.send("Network.setResourceCachingDisabled", { disabled: true }),
      );
      promises.push(
    ...
    if (bootstrapScript.length)
      promises.push(
        session.send("Page.setBootstrapScript", { source: bootstrapScript }),
      );
    this._page
      .frames()
      .map((frame) =>
        frame.evaluateExpression(bootstrapScript).catch((e) => {}),
      );
    ...
    if (contextOptions.offline)
      promises.push(
        session.send("Network.setEmulateOfflineState", { offline: true }),
      );
    promises.push(
    ...
    if (contextOptions.timezoneId) {
      promises.push(
        session
          .send("Page.setTimeZone", { timeZone: contextOptions.timezoneId })
          .catch((e) => {
            throw new Error(
  ...
  async _onTargetCreated(event) {
    ...
    if (!targetInfo.isProvisional) {
      ...
      if (
        pageOrError instanceof import_page2.Page &&
      ...
      this._page.reportAsNew(
        this._opener?._page,
        pageOrError instanceof import_page2.Page ? void 0 : pageOrError,
      );
    } else {
      (0, import_utils.assert)(targetInfo.isProvisional);
      (0, import_utils.assert)(!this._provisionalPage);
      this._provisionalPage = new import_wkProvisionalPage.WKProvisionalPage(
        session,
        this,
      );
      if (targetInfo.isPaused) {
  ...
  _addSessionListeners() {
    this._sessionListeners = [
      ...
      import_eventsHelper.eventsHelper.addEventListener(
        ...
        (event) =>
          this._onDidCheckNavigationPolicy(event.frameId, event.cancel),
      ),
      import_eventsHelper.eventsHelper.addEventListener(
        this._session,
        "Page.frameScheduledNavigation",
        (event) =>
          this._onFrameScheduledNavigation(
            event.frameId,
            event.delay,
            event.targetIsCurrentFrame,
          ),
      ),
      import_eventsHelper.eventsHelper.addEventListener(
        this._session,
      ...
      import_eventsHelper.eventsHelper.addEventListener(
        ...
        (e) => this._onLoadingFailed(this._session, e),
      ),
      import_eventsHelper.eventsHelper.addEventListener(
        this._session,
  ...
  _onDidCheckNavigationPolicy(frameId, cancel) {
    ...
    this._page._frameManager.frameAbortedNavigation(
      frameId,
      "Navigation canceled by policy check",
    );
  }
  ...
  async _onFileChooserOpened(event) {
    ...
    try {
      const context = await this._page._frameManager
        .frame(event.frameId)
        ._mainContext();
      handle = (0, import_wkExecutionContext.createHandle)(
        context,
        event.element,
      ).asElement();
    } catch (e) {
  ...
  static async _setEmulateMedia(
    ...
    promises.push(
      session.send("Page.setForcedColors", { forcedColors: forcedColorsWk }),
    );
  ...
  validateScreenshotDimension(side, omitDeviceScaleFactor) {
    ...
    if (
      ...
      this._page._browserContext._options.deviceScaleFactor
    )
      side = Math.ceil(
        side * this._page._browserContext._options.deviceScaleFactor,
      );
    if (side > 32767)
      throw new Error(
  ...
  _onScreencastFrame(event) {
    ...
    this._page.throttleScreencastFrameAck(() => {
      this._pageProxySession
        .send("Screencast.screencastFrameAck", { generation })
  ...
  async setInputFilePaths(handle, paths) {
    ...
    await Promise.all([
      this._pageProxySession.connection.browserSession.send(
  ...
  async getAccessibilityTree(needle) {
    return (0, import_wkAccessibility.getAccessibilityTree)(
      this._session,
      needle,
    );
  }
  ...
  _onRequest(session, event, intercepted) {
    ...
    let route;
    if (intercepted) {
      route = new import_wkInterceptableRequest.WKRouteImpl(
        session,
        event.requestId,
      );
  ...
  _handleRequestRedirect(request, requestId, responsePayload, timestamp) {
    ...
    response._requestFinished(
      responsePayload.timing
        ? import_helper.helper.secondsToRoundishMillis(
            timestamp - request._timestamp,
          )
        : -1,
    );
  ...
  _onLoadingFinished(event) {
    ...
    if (response) {
      const responseReceivedPayload =
        this._requestIdToResponseReceivedPayloadEvent.get(event.requestId);
      response._serverAddrFinished(
        parseRemoteAddress(event?.metrics?.remoteAddress),
      );
      response._securityDetailsFinished({
        protocol: isLoadedSecurely(response.url(), response.timing())
          ...
          : void 0,
        subjectName:
          responseReceivedPayload?.response.security?.certificate?.subject,
        validFrom:
          responseReceivedPayload?.response.security?.certificate?.validFrom,
        validTo:
          responseReceivedPayload?.response.security?.certificate?.validUntil,
      });
      if (event.metrics?.protocol)
        response._setHttpVersion(event.metrics.protocol);
      response.setEncodedBodySize(
        event.metrics?.responseBodyBytesReceived ?? null,
      );
      response.setResponseHeadersSize(
        event.metrics?.responseHeaderBytesReceived ?? null,
      );
      response._requestFinished(
        import_helper.helper.secondsToRoundishMillis(
