...
class WebKit extends import_browserType.BrowserType {
  ...
  connectToTransport(transport, options) {
    return import_wkBrowser.WKBrowser.connect(
  ...
  amendEnvironment(env, userDataDir, executable, browserArguments) {
    return {
      ...env,
      CURL_COOKIE_JAR_PATH: import_path.default.join(
        userDataDir,
        "cookiejar.db",
      ),
    };
  ...
  doRewriteStartupLog(error) {
    ...
    if (
      error.logs.includes("Failed to open display") ||
      error.logs.includes("cannot open display")
    )
      error.logs =
        "\n" +
  ...
  defaultArgs(options, isPersistent, userDataDir) {
    ...
    if (proxy) {
      if (process.platform === "darwin") {
        ...
        webkitArguments.push(
          `--curl-proxy=${proxy.server.replace(/^socks5:\/\//, "socks5h://")}`,
        );
        if (proxy.bypass)
