...
class WKWorkers {
      ...
      import_eventsHelper.eventsHelper.addEventListener(
        ...
        (event) => {
          ...
          worker._createExecutionContext(
            new import_wkExecutionContext.WKExecutionContext(
              workerSession,
              void 0,
            ),
          );
          this._page._addWorker(event.workerId, worker);
          workerSession.on("Console.messageAdded", (event2) =>
            this._onConsoleMessage(worker, event2),
          );
          Promise.all([
  ...
  async _onConsoleMessage(worker, event) {
    ...
    const handles = (parameters || []).map((p) => {
      return (0, import_wkExecutionContext.createHandle)(
