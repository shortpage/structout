...
class Recorder {
  constructor(context, params) {
    ...
    this._contextRecorder = new import_contextRecorder.ContextRecorder(
      context,
      params,
      {},
    );
  ...
  async _install(recorderApp) {
    ...
    this._contextRecorder.on(
      import_contextRecorder.ContextRecorder.Events.Change,
      (data) => {
    ...
    await this._context.exposeBinding(
      ...
      async (source) => {
        let actionSelector;
        let actionPoint;
        const hasActiveScreenshotCommand = [
          ...this._currentCallsMetadata.keys(),
        ].some(isScreenshotCommand);
        if (!hasActiveScreenshotCommand) {
          actionSelector = await this._scopeHighlightedSelectorToFrame(
            source.frame,
          );
    ...
    await this._context.exposeBinding(
      ...
      async ({ frame }, elementInfo) => {
        ...
        import_contextRecorder.generateFrameSelector)(frame);
  ...
  async _scopeHighlightedSelectorToFrame(frame) {
    ...
    try {
      ...
      if (resolved?.frame === mainFrame)
        return (0, import_selectorParser.stringifySelector)(
  ...
  _pushAllSources() {
    ...
    this._recorderApp?.setSources(
      [...this._recorderSources, ...this._userSources.values()],
      primaryPage?.mainFrame().url(),
    );
  }
