...
class ElectronApplication extends import_instrumentation.SdkObject {
  constructor(parent, browser, nodeConnection, process2) {
    ...
    this._nodeSession.on("Runtime.executionContextCreated", async (event) => {
      ...
      const { result: remoteObject } = await crExecutionContext._client.send(
        "Runtime.evaluate",
        {
    ...
    this._nodeSession.on("Runtime.consoleAPICalled", (event) =>
      this._onConsoleAPI(event),
    );
    const appClosePromise = new Promise((f) =>
      this.once(ElectronApplication.Events.Close, f),
    );
  ...
  async browserWindow(page) {
    ...
    return await electronHandle.evaluateHandle(
      ({ BrowserWindow, webContents }, targetId2) => {
...
class Electron extends import_instrumentation.SdkObject {
  ...
  async launch(options) {
    ...
    return controller.run(async (progress) => {
      ...
      const artifactsDir =
        await import_fs.default.promises.mkdtemp(ARTIFACTS_FOLDER);
      ...
      const browser = await import_crBrowser.CRBrowser.connect(
