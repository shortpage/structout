...
async function filesExceedUploadLimit(files) {
  const sizes = await Promise.all(
    files.map(
      async (file) => (await import_fs.default.promises.stat(file)).size,
    ),
  );
...
async function prepareFilesForUpload(frame, params) {
  ...
  if (
    [payloads, localPaths, localDirectory, streams, directoryStream].filter(
      Boolean,
    ).length !== 1
  )
    throw new Error(
  ...
  if (localPaths) {
    for (const p of localPaths)
      (0, import_assert.assert)(
        import_path.default.isAbsolute(p) &&
          import_path.default.resolve(p) === p,
      ...
      fileBuffers = await Promise.all(
        localPaths.map(async (item) => {
          return {
            ...
            lastModifiedMs: (await import_fs.default.promises.stat(item))
              .mtimeMs,
          };
        }),
      );
