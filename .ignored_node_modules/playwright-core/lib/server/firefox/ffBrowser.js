...
class FFBrowser extends import_browser.Browser {
  ...
  async doCreateNewContext(options) {
    ...
    const { browserContextId } = await this.session.send(
  ...
  _onDownloadCreated(payload) {
    ...
    if (!ffPage) return;
    ffPage._page._frameManager.frameAbortedNavigation(
      payload.frameId,
...
class FFBrowserContext extends import_browserContext.BrowserContext {
  ...
  async _initialize() {
    ...
    if (
      ...
      this._options.internalIgnoreHTTPSErrors
    )
      promises.push(
    ...
    if (this._options.colorScheme !== "no-override") {
      promises.push(
        this._browser.session.send("Browser.setColorScheme", {
          browserContextId,
          colorScheme:
            this._options.colorScheme !== void 0
    ...
    if (this._options.reducedMotion !== "no-override") {
      promises.push(
        this._browser.session.send("Browser.setReducedMotion", {
          browserContextId,
          reducedMotion:
            this._options.reducedMotion !== void 0
    ...
    if (this._options.forcedColors !== "no-override") {
      promises.push(
        this._browser.session.send("Browser.setForcedColors", {
          browserContextId,
          forcedColors:
            this._options.forcedColors !== void 0
    ...
    if (this._options.contrast !== "no-override") {
      promises.push(
        this._browser.session.send("Browser.setContrast", {
          browserContextId,
          contrast:
            this._options.contrast !== void 0
              ? this._options.contrast
              : "no-preference",
        }),
      );
    }
  ...
  async _updateInitScripts() {
    ...
    await this._browser.session.send("Browser.setInitScripts", {
      browserContextId: this._browserContextId,
      scripts: [
        import_page.kBuiltinsScript.source,
        ...bindingScripts,
        ...initScripts,
      ].map((script) => ({ script })),
    });
