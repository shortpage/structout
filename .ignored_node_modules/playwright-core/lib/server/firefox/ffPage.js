...
class FFPage {
  constructor(session, browserContext, opener) {
    ...
    this._networkManager = new import_ffNetworkManager.FFNetworkManager(
      session,
      this._page,
    );
    ...
    this.addInitScript(
      new import_page.InitScript("", true),
      UTILITY_WORLD_NAME,
  ...
  _onExecutionContextsCleared() {
    for (const executionContextId of Array.from(
      this._contextIdToContext.keys(),
    ))
  ...
  _onUncaughtError(params) {
    const { name, message } = (0, import_stackTrace.splitErrorMessage)(
      params.message,
    );
  ...
  _onConsole(payload) {
    ...
    this._page._addConsoleMessage(
      ...
      args.map((arg) =>
        (0, import_ffExecutionContext.createHandle)(context, arg),
      ),
      location,
    );
  }
  ...
  async _onFileChooserOpened(payload) {
    ...
    const context = this._contextIdToContext.get(executionContextId);
    if (!context) return;
    const handle = (0, import_ffExecutionContext.createHandle)(
      context,
      element,
    ).asElement();
  ...
  async _onWorkerCreated(event) {
    ...
    workerSession.once("Runtime.executionContextCreated", (event2) => {
      worker._createExecutionContext(
        new import_ffExecutionContext.FFExecutionContext(
          workerSession,
          event2.executionContextId,
        ),
      );
    });
  ...
  async updateEmulateMedia() {
    ...
    const contrast =
      emulatedMedia.contrast === "no-override"
        ? void 0
  ...
  _onScreencastFrame(event) {
    ...
    this._page.throttleScreencastFrameAck(() => {
      this._session
        .send("Page.screencastFrameAck", { screencastId })
  ...
  async getAccessibilityTree(needle) {
    return (0, import_ffAccessibility.getAccessibilityTree)(
      this._session,
      needle,
    );
  }
  ...
  async getFrameElement(frame) {
    ...
    return (0, import_ffExecutionContext.createHandle)(
      context,
      result.remoteObject,
    );
  }
