...
var injectedScriptSource = __toESM(
...
class ElementHandle extends js.JSHandle {
  ...
  async scrollIntoViewIfNeeded(metadata, options = {}) {
    ...
    return controller.run(
      (progress) =>
        this._waitAndScrollIntoViewIfNeeded(
          progress,
          false,
          /* waitForVisible */
        ),
  ...
  async _clickablePoint() {
    ...
    const [quads, metrics] = await Promise.all([
      ...
      this._page
        ...
        ._utilityContext()
        .then((utility) =>
          utility.evaluate(() => ({ width: innerWidth, height: innerHeight })),
        ),
    ]);
  ...
  async _retryAction(progress, actionName, action, options) {
    ...
    while (progress.isRunning()) {
      if (retry) {
        progress.log(
          `retrying ${actionName} action${options.trial ? " (trial run)" : ""}`,
        );
        const timeout = waitTime[Math.min(retry - 1, waitTime.length - 1)];
        if (timeout) {
          progress.log(`  waiting ${timeout}ms`);
          const result2 = await this.evaluateInUtility(
            ([injected, node, timeout2]) =>
              new Promise((f) => setTimeout(f, timeout2)),
            timeout,
          );
        ...
        progress.log(
          `attempting ${actionName} action${options.trial ? " (trial run)" : ""}`,
        );
      }
  ...
  async _performPointerAction(
    ...
    if (force) {
      ...
      if (
        frameCheckResult === "error:notconnected" ||
        "hitTargetDescription" in frameCheckResult
      )
      ...
      const handle = await this.evaluateHandleInUtility(
        ([
          injected,
          node,
          { actionType: actionType2, hitPoint: hitPoint2, trial },
        ]) =>
          injected.setupHitTargetInterceptor(
            node,
            actionType2,
            hitPoint2,
            trial,
    ...
    const actionResult = await this._page._frameManager.waitForSignalsCreatedBy(
      ...
      async () => {
        ...
        if (options && options.modifiers)
          restoreModifiers = await this._page.keyboard.ensureModifiers(
            options.modifiers,
          );
        ...
        progress.log(
          `  ${options.trial ? "trial " : ""}${actionName} action done`,
        );
      ...
      async () => {
        ...
        const result2 = await this.evaluateInUtility(
          async ([
            injected,
            node,
            { optionsToSelect: optionsToSelect2, force },
          ]) => {
            if (!force) {
  ...
  async setInputFiles(metadata, params) {
    ...
    import_fileUploadUtils.prepareFilesForUpload)(this._frame, params);
    const controller = new import_progress.ProgressController(metadata, this);
    return controller.run(async (progress) => {
      await this._markAsTargetElement(metadata);
  ...
  async _setInputFiles(progress, items) {
    ...
    if (localPaths || localDirectory) {
      const localPathsOrDirectory = localDirectory
        ? [localDirectory]
        : localPaths;
      await Promise.all(
        localPathsOrDirectory.map((localPath) =>
          import_fs.default.promises.access(
            localPath,
      ...
      const waitForInputEvent = localDirectory
        ...
        : Promise.resolve();
      await this._page._delegate.setInputFilePaths(
        retargeted,
        localPathsOrDirectory,
      );
      await waitForInputEvent;
    } else {
      await retargeted.evaluateInUtility(
  ...
  async _setChecked(progress, state, options) {
    const isChecked = async () => {
      ...
      if (
        result2 === "error:notconnected" ||
        result2.received === "error:notconnected"
      )
  ...
  async ariaSnapshot(options) {
    return await this.evaluateInUtility(
      ([injected, element, options2]) =>
        injected.ariaSnapshot(element, options2),
      options,
    );
  }
  ...
  async evalOnSelector(selector, strict, expression, isFunction, arg) {
    return this._frame.evalOnSelector(
      selector,
      strict,
      expression,
      isFunction,
  ...
  async evalOnSelectorAll(selector, expression, isFunction, arg) {
    return this._frame.evalOnSelectorAll(
      selector,
      expression,
      isFunction,
      arg,
  ...
  async waitForElementState(metadata, state, options = {}) {
    ...
    return controller.run(async (progress) => {
      ...
      const result = await this._retryAction(
        ...
        async () => {
          return await this.evaluateInUtility(
            async ([injected, node, state2]) => {
              return (
                (await injected.checkElementStates(node, [state2])) || "done"
              );
            },
            state,
          );
  ...
  async _checkFrameIsHitTarget(point) {
    ...
    for (let i = data.length - 1; i > 0; i--) {
      ...
      const hitTargetResult = await element.evaluateInUtility(
        ([injected, element2, hitPoint]) => {
...
function isPointInsideQuad(point, quad) {
  ...
  const area2 =
    triangleArea(quad[0], quad[1], quad[2]) +
    triangleArea(quad[1], quad[2], quad[3]);
