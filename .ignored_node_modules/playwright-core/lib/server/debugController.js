...
class DebugController extends import_instrumentation.SdkObject {
  ...
  setReportStateChanged(enabled) {
    if (enabled && !this._trackHierarchyListener) {
      ...
      this._playwright.instrumentation.removeListener(
        this._trackHierarchyListener,
      );
    ...
    if (params.testIdAttributeName) {
      for (const page of this._playwright.allPages())
        page
          .context()
          .selectors()
          .setTestIdAttributeName(params.testIdAttributeName);
    }
    for (const recorder of await this._allRecorders()) {
      recorder.hideHighlightedSelector();
  ...
  async closeAllBrowsers() {
    await Promise.all(
      this.allBrowsers().map((browser) =>
        browser.close({ reason: "Close all browsers requested" }),
      ),
    );
  }
  ...
  async _allRecorders() {
    ...
    const result = await Promise.all(
      [...contexts].map((c) =>
        import_recorder.Recorder.showInspector(
          c,
...
class InspectingRecorderApp extends import_recorderApp.EmptyRecorderApp {
  ...
  async elementPicked(elementInfo) {
    const locator = (0, import_utils.asLocator)(
      this._debugController._sdkLanguage,
  ...
  async setSources(sources) {
    const source = sources.find(
      (s) => s.id === this._debugController._codegenId,
    );
