...
class SocksInterceptor {
  constructor(transport, pattern, redirectPortForTest) {
    ...
    this._channel.on("socksRequested", (payload) =>
      this._handler.socketRequested(payload),
    );
    this._channel.on("socksClosed", (payload) =>
      this._handler.socketClosed(payload),
    );
    this._channel.on("socksData", (payload) =>
      this._handler.sendSocketData(payload),
    );
  }
  cleanup() {
  ...
  interceptMessage(message) {
    ...
    if (
      ...
      message.params.type === "SocksSupport"
    ) {
      this._socksSupportObjectGuid = message.params.guid;
      return false;
    }
    if (
      this._socksSupportObjectGuid &&
      message.guid === this._socksSupportObjectGuid
    ) {
      const validator = (0, import_validator.findValidator)(
        "SocksSupport",
        message.method,
