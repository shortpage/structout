...
{`);
  ...
  _generateActionCall(subject, actionInContext) {
    ...
    switch (action.name) {
      ...
      case "click": {
        ...
        const options = (0, import_language.toClickOptionsForSourceCode)(
          action,
        );
      ...
      case "press": {
        const modifiers = (0, import_language.toKeyboardModifiers)(
          action.modifiers,
        );
  ...
  generateStandaloneHeader(options) {
    ...
    if (options.contextOptions.recordHar) {
      ...
      formatter.add(
        `      await context.RouteFromHARAsync(${quote(options.contextOptions.recordHar.path)}${url ? `, ${formatObject({ url }, "    ", "BrowserContextRouteFromHAROptions")}` : ""});`,
      );
    }
    formatter.newLine();
    return formatter.format();
...
function formatContextOptions(contextOptions, deviceName) {
  ...
  const device =
    deviceName && import_deviceDescriptors.deviceDescriptors[deviceName];
  if (!device) {
