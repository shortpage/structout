  ...
  async connect(wsEndpoint, options = {}) {
    return await this._wrapApiCall(async () => {
      const deadline = options.timeout
        ? (0, import_time.monotonicTime)() + options.timeout
        : 0;
      ...
      const result = await (0, import_timeoutRunner.raceAgainstDeadline)(
        async () => {
          ...
          device = AndroidDevice.from(
            playwright._initializer.preConnectedAndroidDevice,
          );
          device._shouldCloseConnectionOnClose = true;
          device.on(import_events.Events.AndroidDevice.Close, () =>
            connection.close(),
          );
          return device;
        },
...
class AndroidDevice extends import_channelOwner.ChannelOwner {
  constructor(parent, type, guid, initializer) {
    ...
    this._channel.on("webViewRemoved", ({ socketName }) =>
  ...
  async info(selector) {
    return (await this._channel.info({ selector: toSelectorChannel(selector) }))
      .info;
  }
...
class AndroidSocket extends import_channelOwner.ChannelOwner {
  ...
  constructor(parent, type, guid, initializer) {
    ...
    this._channel.on("close", () =>
      this.emit(import_events.Events.AndroidSocket.Close),
    );
  }
  async write(data) {
