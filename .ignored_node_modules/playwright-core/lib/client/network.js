...
class Request extends import_channelOwner.ChannelOwner {
  ...
  postData() {
    return (
      (
        this._fallbackOverrides.postDataBuffer || this._initializer.postData
      )?.toString("utf-8") || null
    );
  }
  postDataBuffer() {
    return (
      this._fallbackOverrides.postDataBuffer ||
  ...
  headers() {
    if (this._fallbackOverrides.headers)
      return RawHeaders._fromHeadersObjectLossy(
        this._fallbackOverrides.headers,
      ).headers();
    ...
    if (!this._actualHeadersPromise) {
      this._actualHeadersPromise = this._wrapApiCall(async () => {
        return new RawHeaders(
          (await this._channel.rawRequestHeaders()).headers,
        );
      });
    }
  ...
  _safePage() {
    return (
      import_frame.Frame.fromNullable(this._initializer.frame)?._page || null
    );
  }
  serviceWorker() {
  ...
  async _redirectNavigationRequest(url) {
    await this._handleRoute(async () => {
      await this._raceWithTargetClose(
        this._channel.redirectNavigationRequest({ url }),
      );
    });
  }
  async fetch(options = {}) {
  ...
  async _innerFulfill(options = {}) {
    ...
    if (options.contentType)
      ...
      headers["content-type"] =
        (0, import_mimeType.getMimeTypeForPath)(options.path) ||
...
class WebSocketRoute extends import_channelOwner.ChannelOwner {
  constructor(parent, type, guid, initializer) {
    ...
    this._server = {
      ...
      close: async (options = {}) => {
        await this._channel
          .closeServer({ ...options, wasClean: true })
          .catch(() => {});
      },
      send: (message) => {
        if ((0, import_rtti.isString)(message))
          this._channel
            .sendToServer({ message, isBase64: false })
            .catch(() => {});
        else
          this._channel
            .sendToServer({
    ...
    this._channel.on("messageFromPage", ({ message, isBase64 }) => {
      if (this._onPageMessage)
        this._onPageMessage(
          isBase64 ? Buffer.from(message, "base64") : message,
        );
  ...
  async close(options = {}) {
    await this._channel
      .closePage({ ...options, wasClean: true })
      .catch(() => {});
  }
  connectToServer() {
  ...
  async _actualHeaders() {
    if (!this._actualHeadersPromise) {
      this._actualHeadersPromise = (async () => {
        return new RawHeaders(
          (await this._channel.rawResponseHeaders()).headers,
        );
      })();
    }
    return await this._actualHeadersPromise;
    ...
    return await this._wrapApiCall(async () => {
      ...
      if (event !== import_events.Events.WebSocket.Error)
        waiter.rejectOnEvent(
          this,
          import_events.Events.WebSocket.Error,
...
class RouteHandler {
  ...
  async _handleImpl(route) {
    const handlerInvocation = {
      complete: new import_manualPromise.ManualPromise(),
      route,
    };
