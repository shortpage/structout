...
class BrowserContext extends import_channelOwner.ChannelOwner {
  constructor(parent, type, guid, initializer) {
    ...
    if (parent instanceof import_browser.Browser) this._browser = parent;
    this._browser?._contexts.add(this);
    this._isChromium = this._browser?._name === "chromium";
    this.tracing = import_tracing.Tracing.from(initializer.tracing);
    this.request = import_fetch.APIRequestContext.from(
    ...
    this._channel.on("page", ({ page }) =>
      this._onPage(import_page.Page.from(page)),
    );
    this._channel.on("route", ({ route }) =>
    ...
    this._channel.on("backgroundPage", ({ page }) => {
      ...
      this.emit(
        import_events.Events.BrowserContext.BackgroundPage,
        backgroundPage,
      );
    });
    this._channel.on("serviceWorker", ({ worker }) => {
      ...
      this.emit(
        import_events.Events.BrowserContext.ServiceWorker,
        serviceWorker,
      );
    });
    this._channel.on("console", (event) => {
      ...
      if (page) page.emit(import_events.Events.Page.Console, consoleMessage);
    });
    ...
    this._channel.on("dialog", ({ dialog }) => {
      ...
      let hasListeners = this.emit(
        import_events.Events.BrowserContext.Dialog,
        dialogObject,
      );
      const page = dialogObject.page();
      if (page)
        hasListeners =
          page.emit(import_events.Events.Page.Dialog, dialogObject) ||
          hasListeners;
      if (!hasListeners) {
    ...
    this._closedPromise = new Promise((f) =>
      this.once(import_events.Events.BrowserContext.Close, f),
    );
    this._setEventToSubscriptionMapping(
      /* @__PURE__ */ new Map([
        ...
        [
          import_events.Events.BrowserContext.RequestFinished,
          "requestFinished",
        ],
  ...
  async _onWebSocketRoute(webSocketRoute) {
    const routeHandler = this._webSocketRoutes.find((route) =>
      route.matches(webSocketRoute.url()),
    );
  ...
  async clearCookies(options = {}) {
    await this._channel.clearCookies({
      ...
      nameRegexSource: (0, import_rtti.isRegExp)(options.name)
        ? options.name.source
        : void 0,
      nameRegexFlags: (0, import_rtti.isRegExp)(options.name)
        ? options.name.flags
        : void 0,
      domain: (0, import_rtti.isString)(options.domain)
        ? options.domain
        : void 0,
      domainRegexSource: (0, import_rtti.isRegExp)(options.domain)
        ? options.domain.source
        : void 0,
      domainRegexFlags: (0, import_rtti.isRegExp)(options.domain)
        ? options.domain.flags
        : void 0,
      path: (0, import_rtti.isString)(options.path) ? options.path : void 0,
      pathRegexSource: (0, import_rtti.isRegExp)(options.path)
        ? options.path.source
        : void 0,
      pathRegexFlags: (0, import_rtti.isRegExp)(options.path)
        ? options.path.flags
        : void 0,
    });
  }
  ...
  async _updateInterceptionPatterns() {
    const patterns = network.RouteHandler.prepareInterceptionPatterns(
      this._routes,
    );
  ...
  async newCDPSession(page) {
    if (
      !(page instanceof import_page.Page) &&
      !(page instanceof import_frame.Frame)
    )
  ...
  async close(options = {}) {
    ...
    await this._wrapApiCall(async () => {
      ...
      for (const [harId, harParams] of this._harRecorders) {
        ...
        const isCompressed =
          harParams.content === "attach" || harParams.path.endsWith(".zip");
...
async function prepareStorageState(platform, options) {
  ...
  try {
    return JSON.parse(
      await platform.fs().promises.readFile(options.storageState, "utf8"),
    );
  } catch (e) {
...
function prepareRecordHarOptions(options) {
  ...
  return {
    ...
    urlGlob: (0, import_rtti.isString)(options.urlFilter)
      ? options.urlFilter
      : void 0,
    urlRegexSource: (0, import_rtti.isRegExp)(options.urlFilter)
      ? options.urlFilter.source
      : void 0,
    urlRegexFlags: (0, import_rtti.isRegExp)(options.urlFilter)
...
async function prepareBrowserContextParams(platform, options) {
  ...
  if (contextParams.recordVideo && contextParams.recordVideo.dir)
    contextParams.recordVideo.dir = platform
      .path()
      .resolve(contextParams.recordVideo.dir);
