...
class Frame extends import_channelOwner.ChannelOwner {
  ...
  async goto(url, options = {}) {
    ...
    return network.Response.fromNullable(
      (await this._channel.goto({ url, ...options, waitUntil })).response,
    );
  }
  ...
  async frameElement() {
    return import_elementHandle.ElementHandle.from(
      (await this._channel.frameElement()).element,
    );
  }
  ...
  async $$(selector) {
    ...
    return result.elements.map((e) =>
      import_elementHandle.ElementHandle.from(e),
    );
  }
  ...
  async addScriptTag(options = {}) {
    ...
    if (copy.path) {
      copy.content = (
        await this._platform.fs().promises.readFile(copy.path)
      ).toString();
      copy.content = (0, import_clientHelper.addSourceUrlToScript)(
        copy.content,
        copy.path,
      );
    }
  ...
  async textContent(selector, options = {}) {
    const value = (await this._channel.textContent({ selector, ...options }))
      .value;
  ...
  async getAttribute(selector, name, options = {}) {
    const value = (
      await this._channel.getAttribute({ selector, name, ...options })
    ).value;
    return value === void 0 ? null : value;
  }
