...
class ChannelOwner extends import_eventEmitter.EventEmitter {
  constructor(parent, type, guid, initializer) {
    ...
    this._channel = this._createChannel(
      new import_eventEmitter.EventEmitter(connection._platform),
    );
    ...
    const channel = new Proxy(base, {
      get: (obj, prop) => {
        if (typeof prop === "string") {
          const validator = (0, import_validator.maybeFindValidator)(
            this._type,
            prop,
            "Params",
          );
  ...
  async _wrapApiCall(func, isInternal) {
    ...
    const stackTrace = (0, import_clientStackTrace.captureLibraryStackTrace)(
      this._platform,
    );
    const apiZone = {
    ...
    try {
      ...
      const innerError =
        (this._platform.showInternalStackFrames() ||
          this._platform.isUnderTest()) &&
        e.stack
          ? "\n<inner error>\n" + e.stack
          : "";
...
function tChannelImplToWire(names, arg, path, context) {
  if (
    ...
    (names === "*" || names.includes(arg._object._type))
  )
