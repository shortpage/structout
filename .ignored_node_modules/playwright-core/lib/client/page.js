...
class Page extends import_channelOwner.ChannelOwner {
  constructor(parent, type, guid, initializer) {
    ...
    this._channel.on("locatorHandlerTriggered", ({ uid }) =>
      this._onLocatorHandlerTriggered(uid),
    );
    this._channel.on("route", ({ route }) =>
      this._onRoute(import_network.Route.from(route)),
    );
    this._channel.on("webSocketRoute", ({ webSocketRoute }) =>
      this._onWebSocketRoute(
        import_network.WebSocketRoute.from(webSocketRoute),
      ),
    );
    ...
    this._channel.on("webSocket", ({ webSocket }) =>
      this.emit(
        import_events.Events.Page.WebSocket,
        import_network.WebSocket.from(webSocket),
      ),
    );
    this._channel.on("worker", ({ worker }) =>
      this._onWorker(import_worker.Worker.from(worker)),
    );
  ...
  frame(frameSelector) {
    const name = (0, import_rtti.isString)(frameSelector)
      ? frameSelector
      : frameSelector.name;
    const url = (0, import_rtti.isObject)(frameSelector)
      ? frameSelector.url
      : void 0;
  ...
  async dispatchEvent(selector, type, eventInit, options) {
    return await this._mainFrame.dispatchEvent(
      selector,
      type,
      eventInit,
      options,
  ...
  async reload(options = {}) {
    ...
    return import_network.Response.fromNullable(
      (await this._channel.reload({ ...options, waitUntil })).response,
    );
  }
  ...
  async _onLocatorHandlerTriggered(uid) {
    ...
    try {
      ...
      this._wrapApiCall(
        () => this._channel.resolveLocatorHandlerNoReply({ uid, remove }),
        true,
      ).catch(() => {});
    }
  }
  ...
  async waitForRequest(urlOrPredicate, options = {}) {
    const predicate = async (request) => {
      if (
        (0, import_rtti.isString)(urlOrPredicate) ||
        (0, import_rtti.isRegExp)(urlOrPredicate)
      )
        return (0, import_urlMatch.urlMatches)(
          this._browserContext._options.baseURL,
          request.url(),
  ...
  async goBack(options = {}) {
    ...
    return import_network.Response.fromNullable(
      (await this._channel.goBack({ ...options, waitUntil })).response,
    );
  }
  async goForward(options = {}) {
    ...
    return import_network.Response.fromNullable(
      (await this._channel.goForward({ ...options, waitUntil })).response,
    );
  }
  async requestGC() {
  ...
  async routeWebSocket(url, handler) {
    this._webSocketRoutes.unshift(
      new import_network.WebSocketRouteHandler(
        this._browserContext._options.baseURL,
        url,
        handler,
      ),
  ...
  async _updateInterceptionPatterns() {
    const patterns = import_network.RouteHandler.prepareInterceptionPatterns(
      this._routes,
    );
  ...
  async _updateWebSocketInterceptionPatterns() {
    const patterns =
      import_network.WebSocketRouteHandler.prepareInterceptionPatterns(
        this._webSocketRoutes,
      );
  ...
  async pause(_options) {
    ...
    const defaultTimeout =
      this._browserContext._timeoutSettings.defaultTimeout();
  ...
  async pdf(options = {}) {
    ...
    if (options.path) {
      ...
      await platform
        .fs()
        .promises.mkdir(platform.path().dirname(options.path), {
          recursive: true,
        });
  ...
  async call(func) {
    try {
      ...
      if (this._initializer.handle)
        result = await func(
          source,
          import_jsHandle.JSHandle.from(this._initializer.handle),
        );
      else
        result = await func(
          source,
          ...this._initializer.args.map(import_jsHandle.parseResult),
        );
      this._channel
      ...
      this._channel
        .reject({ error: (0, import_errors.serializeError)(e) })
        .catch(() => {});
    }
  }
}
