...
class BrowserType extends import_channelOwner.ChannelOwner {
  ...
  async launch(options = {}) {
    ...
    const logger =
      options.logger || this._playwright._defaultLaunchOptions?.logger;
    ...
    const launchOptions = {
      ...
      env: options.env
        ? (0, import_clientHelper.envObjectToArray)(options.env)
        : void 0,
    };
  ...
  async launchPersistentContext(userDataDir, options = {}) {
    ...
    import_browserContext.prepareBrowserContextParams)(this._platform, options);
    ...
    return await this._wrapApiCall(async () => {
      const result =
        await this._channel.launchPersistentContext(persistentParams);
  ...
  async _connect(params) {
    ...
    return await this._wrapApiCall(async () => {
      const deadline = params.timeout
        ? (0, import_time.monotonicTime)() + params.timeout
        : 0;
      const headers = {
      ...
      if (params.__testHookRedirectPortForwarding)
        connectParams.socksProxyRedirectPortForTest =
          params.__testHookRedirectPortForwarding;
      ...
      const result = await (0, import_timeoutRunner.raceAgainstDeadline)(
        async () => {
          ...
          browser = import_browser.Browser.from(
            playwright._initializer.preLaunchedBrowser,
          );
          this._didLaunchBrowser(browser, {}, logger);
          browser._shouldCloseConnectionOnClose = true;
          browser.on(import_events.Events.Browser.Disconnected, () =>
            connection.close(),
          );
          return browser;
        },
  ...
  async _didCreateContext(context, contextOptions, browserOptions, logger) {
    ...
    if (this._playwright._defaultContextNavigationTimeout !== void 0)
      context.setDefaultNavigationTimeout(
        this._playwright._defaultContextNavigationTimeout,
      );
