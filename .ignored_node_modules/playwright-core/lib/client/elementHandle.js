...
class ElementHandle extends import_jsHandle.JSHandle {
  ...
  async ownerFrame() {
    return import_frame.Frame.fromNullable(
      (await this._elementChannel.ownerFrame()).frame,
    );
  }
  async contentFrame() {
    return import_frame.Frame.fromNullable(
      (await this._elementChannel.contentFrame()).frame,
    );
  }
  ...
  async setInputFiles(files, options = {}) {
    ...
    const converted = await convertInputFiles(
      this._platform,
      files,
      frame.page().context(),
    );
...
function filePayloadExceedsSizeLimit(payloads) {
  return (
    payloads.reduce(
      (size, item) => size + (item.buffer ? item.buffer.byteLength : 0),
      0,
...
async function convertInputFiles(platform, files, context) {
  ...
  if (items.some((item) => typeof item === "string")) {
    ...
    const [localPaths, localDirectory] =
      await resolvePathsAndDirectoryForInputFiles(platform, items);
    if (context._connection.isRemote()) {
      const files2 = localDirectory
        ? (
            await platform.fs().promises.readdir(localDirectory, {
              withFileTypes: true,
              recursive: true,
            })
          )
        ...
        async () =>
          context._channel.createTempFiles({
            rootDirName: localDirectory
              ? platform.path().basename(localDirectory)
              : void 0,
            items: await Promise.all(
              files2.map(async (file) => {
                const lastModifiedMs = (await platform.fs().promises.stat(file))
                  .mtimeMs;
                return {
                  name: localDirectory
      ...
      for (let i = 0; i < files2.length; i++) {
        const writable = import_writableStream.WritableStream.from(
          writableStreams[i],
        );
