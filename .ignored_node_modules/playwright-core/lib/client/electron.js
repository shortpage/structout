...
class Electron extends import_channelOwner.ChannelOwner {
  ...
  async launch(options = {}) {
    const params = {
      ...(await (0, import_browserContext.prepareBrowserContextParams)(
        this._platform,
        options,
      )),
    ...
    const app = ElectronApplication.from(
      (await this._channel.launch(params)).electronApplication,
    );
...
class ElectronApplication extends import_channelOwner.ChannelOwner {
  constructor(parent, type, guid, initializer) {
    ...
    this._context = import_browserContext.BrowserContext.from(
      initializer.context,
    );
    for (const page of this._context._pages) this._onPage(page);
    this._context.on(import_events.Events.BrowserContext.Page, (page) =>
      this._onPage(page),
    );
    ...
    this._channel.on("console", (event) =>
      this.emit(
        ...
        new import_consoleMessage.ConsoleMessage(this._platform, event),
      ),
    );
  ...
  _onPage(page) {
    ...
    page.once(import_events.Events.Page.Close, () =>
