...
import_utilsBundle.program
  .version(
    "Version " + (process.env.PW_CLI_DISPLAY_VERSION || packageJSON.version),
  )
...
import_utilsBundle.program
  ...
  .action(function (dockerImageNameTemplate) {
    (0, import_utils2.assert)(
      ...
      "dockerImageNameTemplate is required",
    );
    (0, import_server.writeDockerVersion)(dockerImageNameTemplate).catch(
      logErrorAndExit,
    );
  });
...
function defaultBrowsersToInstall(options) {
  ...
  if (options.noShell)
    executables = executables.filter(
      (e) => e.name !== "chromium-headless-shell",
    );
  ...
  .action(async function (args, options) {
    ...
    try {
      ...
      const executables = hasNoArguments
        ? defaultBrowsersToInstall(options)
        : checkBrowsersToInstall(args, options);
      if (options.withDeps)
        await import_server.registry.installDeps(executables, !!options.dryRun);
      if (options.dryRun) {
        for (const executable of executables) {
          const version = executable.browserVersion
            ? `version ` + executable.browserVersion
            : "";
          console.log(
            `browser: ${executable.name}${version ? " " + version : ""}`,
          );
          console.log(
            `  Install location:    ${executable.directory ?? "<system>"}`,
          );
...
import_utilsBundle.program
  ...
  .action(function (options) {
    (0, import_driver.runServer)({
      ...
      extension:
        options.mode === "extension" || !!process.env.PW_EXTENSION_MODE,
    }).catch(logErrorAndExit);
  });
import_utilsBundle.program
...
import_utilsBundle.program
  ...
  .option(
    ...
    "JSON file with launchServer options",
  )
  .action(function (options) {
...
import_utilsBundle.program
  ...
  .action(function (traces, options) {
    ...
    if (options.port !== void 0 || options.host !== void 0)
      (0, import_traceViewer.runTraceInBrowser)(traces, openOptions).catch(
        logErrorAndExit,
      );
    else
...
async function launchContext(options, extraOptions) {
  ...
  if (!extraOptions.headless)
    contextOptions.deviceScaleFactor =
      import_os.default.platform() === "darwin" ? 2 : 1;
  ...
  async function closeBrowser() {
    ...
    if (options.saveStorage)
      await context
        .storageState({ path: options.saveStorage })
        .catch((e) => null);
  ...
  context.on("page", (page) => {
    ...
    page.on("close", () => {
      const hasPage = browser
        .contexts()
        .some((context2) => context2.pages().length > 0);
      if (hasPage) return;
      closeBrowser().catch(() => {});
    });
  });
...
function commandWithOpenOptions(command, description, options) {
  let result = import_utilsBundle.program
    .command(command)
  ...
  return result
    ...
    .option(
      "--save-har <filename>",
      "save HAR file with all network activity at the end",
    )
    .option(
...
function buildBasePlaywrightCLICommand(cliTargetLang) {
  switch (cliTargetLang) {
    ...
    default: {
      ...
      import_utils2.getPackageManagerExecCommand)();
