...
- Returns a _positive_ value if the points `a`, `b`, and `c` occur in _counterclockwise_ order (`c` lies to the left of the directed line defined by points `a` and `b`).
- Returns a _negative_ value if they occur in _clockwise_ order (`c` lies to the right of the directed line `ab`).
- Returns _zero_ if they are _collinear_.
