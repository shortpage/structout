...
- `attribute` (`string`)
  ...
  or `'charoff'`)
- `booleanish` (`boolean`)
  — the property is _like_ a `boolean`
  (such as `draggable`);
  these properties have both an on and off state when defined,
  _and_ another state when not defined
- `boolean` (`boolean`)
...
- `overloadedBoolean` (`boolean`)
  — the property is _like_ a `boolean` (such as `download`);
  these properties have an on state _and_ more states when defined and an off
...
Yes please!
See [_How to Contribute to Open Source_][opensource-guide].
