...
const getBlockName = (file, meaninglessFileNames) => {
  const name = _path.default.basename(
    file.opts.filename,
    _path.default.extname(file.opts.filename),
  );
...
const getDisplayName = (t) => (path, state) => {
  ...
  if (file) {
    const blockName = getBlockName(
      file,
      (0, _options.useMeaninglessFileNames)(state),
    );
...
const getFileHash = (state) => {
  ...
  const filePath =
    moduleRoot &&
    _path.default.relative(moduleRoot, filename).replace(separatorRegExp, "/");
  const moduleName =
    moduleRoot &&
    JSON.parse(
      _fs.default.readFileSync(_path.default.join(moduleRoot, "package.json")),
    ).name;
...
var _default = (t) => (path, state) => {
  if (
    path.node.tag
      ...
      : /* styled()`` */ ((0, _detectors.isStyled)(t)(
          path.node.callee,
          state,
        ) &&
