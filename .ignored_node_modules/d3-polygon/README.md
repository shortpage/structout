...
Returns the signed area of the specified _polygon_. If the vertices of the polygon are in counterclockwise order (assuming a coordinate system where the origin ⟨0,0⟩ is in the top-left corner), the returned area is positive; otherwise it is negative, or zero.
...
Returns the [centroid](https://en.wikipedia.org/wiki/Centroid) of the specified _polygon_.
...
Returns true if and only if the specified _point_ is inside the specified _polygon_.
...
Returns the length of the perimeter of the specified _polygon_.
