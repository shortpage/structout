...
export interface Project<TestArgs = {}, WorkerArgs = {}>
  extends TestProject<TestArgs, WorkerArgs> {}
...
export interface FullProject<TestArgs = {}, WorkerArgs = {}> {
   ...
   */
  use: UseOptions<
    PlaywrightTestOptions & TestArgs,
    PlaywrightWorkerOptions & WorkerArgs
  >;
  /**
...
export interface Config<TestArgs = {}, WorkerArgs = {}>
  extends TestConfig<TestArgs, WorkerArgs> {}
...
export interface TestType<TestArgs extends {}, WorkerArgs extends {}> {
  ...
  (
    title: string,
    details: TestDetails,
    body: TestBody<TestArgs & WorkerArgs>,
  ): void;
   ...
   */
  skip(
    callback: ConditionBody<TestArgs & WorkerArgs>,
    description?: string,
  ): void;
...
export type TestFixture<R, Args extends {}> = (
  args: Args,
...
type TestFixtureValue<R, Args extends {}> =
  | Exclude<R, Function>
  | TestFixture<R, Args>;
type WorkerFixtureValue<R, Args extends {}> =
  | Exclude<R, Function>
...
type ClientCertificate = Exclude<
...
type CustomProperties<T> = ExcludeProps<
  T,
...
export type PlaywrightTestProject<TestArgs = {}, WorkerArgs = {}> = Project<
  PlaywrightTestOptions & CustomProperties<TestArgs>,
  PlaywrightWorkerOptions & CustomProperties<WorkerArgs>
>;
export type PlaywrightTestConfig<TestArgs = {}, WorkerArgs = {}> = Config<
  PlaywrightTestOptions & CustomProperties<TestArgs>,
  PlaywrightWorkerOptions & CustomProperties<WorkerArgs>
>;
...
type BaseMatchers<R, T> = GenericAssertions<R> &
  PlaywrightTest.Matchers<R, T> &
  SnapshotAssertions;
type AllowedGenericMatchers<R, T> = PlaywrightTest.Matchers<R, T> &
  Pick<
    GenericAssertions<R>,
    | "toBe"
    | "toBeDefined"
...
type SpecificMatchers<R, T> = T extends Page
  ? PageAssertions & AllowedGenericMatchers<R, T>
  : T extends Locator
    ? LocatorAssertions & AllowedGenericMatchers<R, T>
    : T extends APIResponse
      ? APIResponseAssertions & AllowedGenericMatchers<R, T>
      : BaseMatchers<R, T> & (T extends Function ? FunctionAssertions : {});
type AllMatchers<R, T> = PageAssertions &
  LocatorAssertions &
...
type ToUserMatcher<F> = F extends (first: any, ...args: infer Rest) => infer R
  ? (...args: Rest) => R extends PromiseLike<infer U> ? Promise<void> : void
  : never;
type ToUserMatcherObject<T, ArgType> = {
  [K in keyof T as T[K] extends (arg: ArgType, ...rest: any[]) => any
    ? K
    : never]: ToUserMatcher<T[K]>;
};
...
type MakeMatchers<R, T, ExtendedMatchers> = {
  ...
  T,
  AllMatchers<R, T>,
  SpecificMatchers<R, T> & ToUserMatcherObject<ExtendedMatchers, T>
>;
...
type PollMatchers<R, T, ExtendedMatchers> = {
  ...
  ToUserMatcherObject<ExtendedMatchers, T>;
...
type MergedT<List> = List extends [TestType<infer T, any>, ...infer Rest]
  ? T & MergedT<Rest>
  : {};
type MergedW<List> = List extends [TestType<any, infer W>, ...infer Rest]
  ? W & MergedW<Rest>
  : {};
 ...
 * Merges fixtures
 */
export function mergeTests<List extends any[]>(
  ...tests: List
): MergedTestType<List>;

type MergedExpectMatchers<List> = List extends [Expect<infer M>, ...infer Rest]
  ? M & MergedExpectMatchers<Rest>
  : {};
 ...
 * Merges expects
 */
export function mergeExpects<List extends any[]>(
  ...expects: List
): MergedExpect<List>;
