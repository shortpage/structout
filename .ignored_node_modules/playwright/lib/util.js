...
const PLAYWRIGHT_CORE_PATH = import_path.default.dirname(
...
function filterStackTrace(e) {
  ...
  const stackLines = (0, import_utils.stringifyStackFrames)(
...
function filteredStackTrace(rawStack) {
  ...
  for (const line of rawStack) {
    const frame = (0, import_utils.parseStackFrame)(
      line,
...
function createFileMatcherFromArguments(args) {
  ...
  return createFileMatcher(
    filters.map((filter) => filter.re || filter.exact || ""),
  );
}
function createFileMatcher(patterns) {
  ...
  return (filePath) => {
    ...
    for (const pattern of filePatterns) {
      if (
        (0, import_utilsBundle.minimatch)(filePath, pattern, {
          nocase: true,
          dot: true,
        })
      )
...
function expectTypes(receiver, types, matcherName) {
  if (
    ...
    const typesString = commaSeparated.length
      ...
      : lastType;
    throw new Error(
      `${matcherName} can be only used with ${typesString} object${types.length > 1 ? "s" : ""}`,
    );
  }
}
...
async function normalizeAndSaveAttachment(outputPath, name, options = {}) {
  ...
  if (options.path !== void 0) {
    ...
    const sanitizedNamePrefix =
      (0, import_utils.sanitizeForFilePath)(name) + "-";
    ...
    const contentType =
      ...
      (import_utilsBundle.mime.getType(
        import_path.default.basename(options.path),
      ) ||
