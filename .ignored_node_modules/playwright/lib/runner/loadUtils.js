...
async function collectProjectsAndTestFiles(
  ...
  const cliFileMatcher = config.cliArgs.length
    ? (0, import_util.createFileMatcherFromArguments)(config.cliArgs)
    : null;
  const allFilesForProject = /* @__PURE__ */ new Map();
  const filteredProjects = (0, import_projectUtils.filterProjects)(
    config.projects,
    config.cliProjectFilter,
  );
  for (const project of filteredProjects) {
    const files = await (0, import_projectUtils.collectFilesForProject)(
      project,
      fsCache,
    );
  ...
  const projectClosure = (0, import_projectUtils.buildProjectsClosure)([
    ...filesToRunByProject.keys(),
  ]);
    ...
    if (type === "dependency") {
      ...
      const files = treatProjectAsEmpty
        ...
        : allFilesForProject.get(project) ||
          (await (0, import_projectUtils.collectFilesForProject)(
            project,
            fsCache,
          ));
...
async function loadFileSuites(testRun, mode, errors) {
  ...
  const loaderHost =
    mode === "out-of-process"
      ? new import_loaderHost.OutOfProcessLoaderHost(config)
      : new import_loaderHost.InProcessLoaderHost(config);
  ...
  for (const file of allTestFiles) {
    for (const dependency of (0,
    import_compilationCache.dependenciesForTestFile)(file)) {
      if (allTestFiles.has(dependency)) {
        const importer = import_path.default.relative(
          config.config.rootDir,
          file,
        );
        const importee = import_path.default.relative(
          config.config.rootDir,
          dependency,
        );
        errors.push({
...
async function createRootSuite(
  ...
  {
    const cliFileFilters = (0, import_util.createFileFiltersFromArguments)(
      config.cliArgs,
    );
    const grepMatcher = config.cliGrep
      ? (0, import_util.createTitleMatcher)(
          (0, import_util.forceRegExp)(config.cliGrep),
        )
      : () => true;
    const grepInvertMatcher = config.cliGrepInvert
      ? (0, import_util.createTitleMatcher)(
          (0, import_util.forceRegExp)(config.cliGrepInvert),
        )
      : () => false;
    const cliTitleMatcher = (title) =>
      !grepInvertMatcher(title) && grepMatcher(title);
  ...
  if (config.config.forbidOnly) {
    ...
    if (onlyTestsAndSuites.length > 0) {
      const configFilePath = config.config.configFile
      ...
      errors.push(
        ...createForbidOnlyErrors(
          onlyTestsAndSuites,
          config.configCLIOverrides.forbidOnly,
          configFilePath,
        ),
      );
    }
  ...
  if (config.config.shard) {
    ...
    for (const projectSuite of rootSuite.suites) {
      testGroups.push(
        ...(0, import_testGroups.createTestGroups)(
          projectSuite,
          config.config.shard.total,
        ),
      );
    }
    const testGroupsInThisShard = (0, import_testGroups.filterForShard)(
      config.config.shard,
      testGroups,
    );
    ...
    (0, import_suiteUtils.filterTestsRemoveEmptySuites)(rootSuite, (test) =>
      testsInThisShard.has(test),
    );
  }
  if (config.lastFailedTestIdMatcher)
    (0, import_suiteUtils.filterByTestIds)(
      rootSuite,
      config.lastFailedTestIdMatcher,
    );
  {
    const projectClosure2 = new Map(
      (0, import_projectUtils.buildProjectsClosure)(
    ...
    for (const [project, level] of projectClosure2.entries()) {
      if (level === "dependency")
        rootSuite._prependSuite(
          buildProjectSuite(project, projectSuites.get(project)),
        );
    }
  }
  return rootSuite;
...
function createProjectSuite(project, fileSuites) {
  ...
  for (const fileSuite of fileSuites)
    projectSuite._addSuite(
      (0, import_suiteUtils.bindFileSuiteToProject)(project, fileSuite),
    );
  ...
  const grepInvertMatcher = project.project.grepInvert
    ? (0, import_util.createTitleMatcher)(project.project.grepInvert)
    : null;
...
function filterProjectSuite(projectSuite, options) {
  if (
    !options.cliFileFilters.length &&
    !options.cliTitleMatcher &&
    !options.testIdMatcher &&
    !options.additionalFileMatcher
  )
    return projectSuite;
  ...
  (0, import_suiteUtils.filterTestsRemoveEmptySuites)(result, (test) => {
    ...
    if (
      options.additionalFileMatcher &&
      !options.additionalFileMatcher(test.location.file)
    )
      return false;
    return true;
  });
...
function buildProjectSuite(project, projectSuite) {
  ...
  for (const fileSuite of projectSuite.suites) {
    ...
    for (
      ...
      (0, import_suiteUtils.applyRepeatEachIndex)(
        project,
        clone,
        repeatEachIndex,
      );
...
async function requireOrImportDefaultFunction(file, expectConstructor) {
  ...
  if (typeof func !== "function")
    throw (0, import_util.errorWithFile)(
      file,
...
function loadGlobalHook(config, file) {
  return requireOrImportDefaultFunction(
    import_path.default.resolve(config.config.rootDir, file),
    false,
  );
}
function loadReporter(config, file) {
  return requireOrImportDefaultFunction(
    config ? import_path.default.resolve(config.config.rootDir, file) : file,
    true,
  );
}
function sourceMapSources(file, cache) {
  ...
  try {
    const sourceMap =
      import_utilsBundle.sourceMapSupport.retrieveSourceMap(file);
    const sourceMapData =
    ...
    if (sourceMapData?.sources)
      sources = sourceMapData.sources.map((source) =>
        import_path.default.resolve(import_path.default.dirname(file), source),
      );
  } finally {
