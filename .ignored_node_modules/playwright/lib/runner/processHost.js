...
class ProcessHost extends import_events.EventEmitter {
  ...
  async startRunner(runnerParams, options = {}) {
    (0, import_utils.assert)(
      !this.process,
      "Internal error: starting the same process twice",
    );
    this.process = import_child_process.default.fork(
      require.resolve("../common/process"),
      {
        detached: false,
        env: {
          ...process.env,
          ...this._extraEnv,
          ...(import_esmLoaderHost.esmLoaderRegistered
        ...
        ...(process.env.PW_TS_ESM_LEGACY_LOADER_ON
          ? {
              execArgv: (0,
              import_esmUtils.execArgvWithExperimentalLoaderOptions)(),
            }
          : {}),
      },
    );
    ...
    this.process.on("message", (message) => {
      if (import_utilsBundle.debug.enabled("pw:test:protocol"))
        (0, import_utilsBundle.debug)("pw:test:protocol")(
          "\u25C0 RECV " + JSON.stringify(message),
        );
