...
async function applySuggestedRebaselines(config, reporter) {
  ...
  const [project] = (0, import_projectUtils.filterProjects)(
    config.projects,
  ...
  const patchFile = import_path.default.join(
  ...
  for (const fileName of [...suggestedRebaselines.keys()].sort()) {
    ...
    if (config.config.updateSourceMethod === "overwrite") {
      ...
      const relativeToGit = import_path.default.relative(
  ...
  const fileList = files
    .map((file) => "  " + import_utils2.colors.dim(file))
    .join("\n");
  reporter.onStdErr(`
...
`);
  if (config.config.updateSourceMethod === "patch") {
    await import_fs.default.promises.mkdir(
      import_path.default.dirname(patchFile),
      { recursive: true },
    );
  ...
  ` +
        import_utils2.colors.cyan(
          "git apply " + import_path.default.relative(process.cwd(), patchFile),
        ) +
        "\n",
    );
  }
...
function createPatch(fileName, before, after) {
  ...
  const text = import_utilsBundle.diff.createPatch(
    file,
    before,
    after,
    void 0,
...
function findGitRoot(dir, cache) {
  ...
  if (
    import_fs.default.existsSync(gitPath) &&
    import_fs.default.lstatSync(gitPath).isDirectory()
  ) {
    cache.set(dir, dir);
    return dir;
  }
