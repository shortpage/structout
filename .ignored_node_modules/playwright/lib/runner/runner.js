...
class Runner {
  ...
  async runAllTests() {
    ...
    (0, import_webServerPlugin.webServerPluginsForConfig)(config).forEach((p) =>
      config.plugins.push({ factory: p }),
    );
    ...
    const tasks = listOnly
      ? [
          (0, import_tasks.createLoadTask)("in-process", {
  ...
  async findRelatedTestFiles(files) {
    ...
    const status = await (0, import_tasks.runTasks)(
      new import_tasks.TestRun(this._config, reporter),
      [
  ...
  async runDevServer() {
    const reporter = new import_internalReporter.InternalReporter([
      (0, import_reporters.createErrorCollectingReporter)(
        import_base.terminalScreen,
        true,
      ),
    ]);
