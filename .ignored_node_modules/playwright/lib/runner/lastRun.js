...
class LastRunReporter {
  constructor(config) {
    ...
    if (project)
      this._lastRunFile = import_path.default.join(
  ...
  async filterLastFailed() {
    ...
    try {
      const lastRunInfo = JSON.parse(
        await import_fs.default.promises.readFile(this._lastRunFile, "utf8"),
      );
      this._config.lastFailedTestIdMatcher = (id) =>
        lastRunInfo.failedTests.includes(id);
    } catch {}
  }
  version() {
    return "v2";
  ...
  async onEnd(result) {
    ...
    await import_fs.default.promises.mkdir(
      import_path.default.dirname(this._lastRunFile),
      { recursive: true },
    );
    const failedTests = this._suite
      ?.allTests()
      .filter((t) => !t.ok())
      .map((t) => t.id);
    const lastRunReport = JSON.stringify(
      { status: result.status, failedTests },
      void 0,
      2,
    );
