...
async function detectChangedTestFiles(baseCommit, configDir) {
  function gitFileList(command) {
    try {
      return import_child_process.default
  ...
  ).map((file) => import_path.default.join(configDir, file));
  const [gitRoot] = gitFileList("rev-parse --show-toplevel");
  const trackedFilesWithChanges = gitFileList(
    `diff ${baseCommit} --name-only`,
  ).map((file) => import_path.default.join(gitRoot, file));
  return new Set(
    (0, import_compilationCache.affectedTestFiles)([
      ...untrackedFiles,
