...
class WorkerHost extends import_processHost.ProcessHost {
  constructor(testGroup, parallelIndex, config, extraEnv, outputDir) {
    ...
    this._params = {
      ...
      config,
      artifactsDir: import_path.default.join(
        outputDir,
        (0, import_folders.artifactsFolderName)(workerIndex),
      ),
    };
  }
  async start() {
    await import_fs.default.promises.mkdir(this._params.artifactsDir, {
      recursive: true,
    });
    return await this.startRunner(this._params, {
      onStdOut: (chunk) =>
        this.emit("stdOut", (0, import_ipc.stdioChunkToParams)(chunk)),
      onStdErr: (chunk) =>
        this.emit("stdErr", (0, import_ipc.stdioChunkToParams)(chunk)),
    });
  }
  async onExit() {
