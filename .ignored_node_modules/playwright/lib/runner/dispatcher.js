...
class Dispatcher {
  ...
  async _runJobInWorker(index, jobDispatcher) {
    ...
    if (!worker) {
      worker = this._createWorker(
        job,
        index,
        (0, import_ipc.serializeConfig)(this._config, true),
      );
  ...
  _createWorker(testGroup, parallelIndex, loaderData) {
    const projectConfig = this._config.projects.find(
...
class JobDispatcher {
  ...
  _onStepEnd(params) {
    ...
    if (params.suggestedRebaseline)
      (0, import_rebase.addSuggestedRebaseline)(
        step.location,
        params.suggestedRebaseline,
      );
    ...
    if (this._failureTracker.hasReachedMaxFailures()) {
      ...
      if (!hadMaxFailures)
        this._reporter.onError?.({
          message: import_utils2.colors.red(
            `Testing stopped early after ${this._failureTracker.maxFailures()} maximum allowed failures.`,
          ),
        });
    }
  }
