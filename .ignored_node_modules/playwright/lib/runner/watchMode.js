...
async function runWatchModeLoop(configLocation, initialOptions) {
  ...
  const testServerDispatcher = new import_testServer.TestServerDispatcher(
    configLocation,
    {},
  );
  ...
  const testServerConnection =
    new import_testServerConnection.TestServerConnection(transport);
  transport.emit("open");
  const teleSuiteUpdater = new import_teleSuiteUpdater.TeleSuiteUpdater({
  ...
  while (true) {
    ...
    if (command === "project") {
      const { selectedProjects } = await import_utilsBundle.enquirer
        .prompt({
          type: "multiselect",
    ...
    if (command === "file") {
      const { filePattern } = await import_utilsBundle.enquirer
        .prompt({
          type: "text",
          name: "filePattern",
    ...
    if (command === "grep") {
      const { testPattern } = await import_utilsBundle.enquirer
        .prompt({
          type: "text",
          name: "testPattern",
...
function readKeyPress(handler) {
  ...
  const listener = import_utils.eventsHelper.addEventListener(
    process.stdin,
    "keypress",
    (text, key) => {
...
function readCommand() {
  return readKeyPress((text, key) => {
    ...
    if (name === "h") {
      process.stdout
        .write(`${(0, import_base.separator)(import_base.terminalScreen)}
Run tests
...
function printConfiguration(options, title) {
  ...
  if (options.projects)
    tokens.push(
      ...options.projects.map((p) =>
        import_utils2.colors.blue(`--project ${p}`),
      ),
    );
  if (options.grep)
  ...
  if (title) tokens.push(import_utils2.colors.dim(`(${title})`));
  ...
  lines.push(
    `${import_utils2.colors.dim("Show & reuse browser:")} ${import_utils2.colors.bold(showBrowserServer ? "on" : "off")}`,
  );
...
`);
  if (dirtyTestFiles.size === 0) {
    process.stdout
      .write(`${import_utils2.colors.dim("Waiting for file changes. Press")} ${import_utils2.colors.bold("q")} ${import_utils2.colors.dim("to quit or")} ${import_utils2.colors.bold("h")} ${import_utils2.colors.dim("for more options.")}

`);
    return;
  }
  process.stdout
    .write(`${import_utils2.colors.dim(`${dirtyTestFiles.size} test ${dirtyTestFiles.size === 1 ? "file" : "files"} changed:`)}
...
async function toggleShowBrowser() {
  if (!showBrowserServer) {
    showBrowserServer = new import_playwrightServer.PlaywrightServer({
      mode: "extension",
      path: "/" + (0, import_utils.createGuid)(),
      maxConnections: 1,
    });
    connectWsEndpoint = await showBrowserServer.listen();
    process.stdout
      .write(`${import_utils2.colors.dim("Show & reuse browser:")} ${import_utils2.colors.bold("on")}
`);
  } else {
    ...
    process.stdout
      .write(`${import_utils2.colors.dim("Show & reuse browser:")} ${import_utils2.colors.bold("off")}
`);
  }
}
