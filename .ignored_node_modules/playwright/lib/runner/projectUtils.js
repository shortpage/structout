...
function wildcardPatternToRegExp(pattern) {
  return new RegExp(
    "^" + pattern.split("*").map(import_utils.escapeRegExp).join(".*") + "$",
    "ig",
  );
}
...
async function collectFilesForProject(
  ...
  const testFileExtension = (file) =>
    extensions.has(import_path.default.extname(file));
  const allFiles = await cachedCollectFiles(
    project.project.testDir,
    project.respectGitIgnore,
    fsCache,
  );
  const testMatch = (0, import_util2.createFileMatcher)(
    project.project.testMatch,
  );
  const testIgnore = (0, import_util2.createFileMatcher)(
...
async function collectFiles(testDir, respectGitIgnore) {
  if (!import_fs.default.existsSync(testDir)) return [];
  if (!import_fs.default.statSync(testDir).isDirectory()) return [];
    ...
    if (respectGitIgnore) {
      ...
      if (gitignore) {
        const content = await readFileAsync(
          import_path.default.join(dir, gitignore.name),
          "utf8",
        );
        const newRules = content
          ...
          .map((s) => {
            s = s.trim();
            if (!s) return;
            const rule = new import_utilsBundle.minimatch.Minimatch(s, {
              matchBase: true,
              dot: true,
              flipNegate: true,
            });
