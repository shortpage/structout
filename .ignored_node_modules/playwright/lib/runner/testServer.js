...
class TestServerDispatcher {
  constructor(configLocation, configCLIOverrides) {
    ...
    this._watcher = new import_fsWatcher.Watcher((events) => {
      ...
      events.forEach((f) =>
        (0, import_compilationCache.collectAffectedTestFiles)(
          f.file,
          collector,
        ),
      );
  ...
  async _collectingInternalReporter(...extraReporters) {
    const report = [];
    const collectingReporter = await (0,
    import_reporters.createReporterForTestServer)(this._serializer, (e) =>
      report.push(e),
    );
    return {
      reporter: new import_internalReporter.InternalReporter([
        collectingReporter,
        ...extraReporters,
      ]),
      report,
  ...
  async runGlobalSetup(params) {
    ...
    const config = await this._loadConfigOrReportError(
      reporter,
      this._configCLIOverrides,
    );
    if (!config) return { status: "failed", report };
    const { status, cleanup } = await (0, import_tasks.runTasksDeferCleanup)(
      new import_tasks.TestRun(config, reporter),
      [...(0, import_tasks.createGlobalSetupTasks)(config)],
    );
    if (status !== "passed") await cleanup();
    else this._globalSetup = { cleanup, report };
  ...
  async startDevServer(params) {
    ...
    if (!config) return { report, status: "failed" };
    const { status, cleanup } = await (0, import_tasks.runTasksDeferCleanup)(
      new import_tasks.TestRun(config, reporter),
      [
        (0, import_tasks.createLoadTask)("out-of-process", {
    ...
    if (status !== "passed") await cleanup();
    else this._devServer = { cleanup, report };
    return { report, status };
  }
  ...
  async listFiles(params) {
    ...
    const config = await this._loadConfigOrReportError(reporter);
    if (!config) return { status: "failed", report };
    config.cliProjectFilter = params.projects?.length
      ? params.projects
      : void 0;
    const status = await (0, import_tasks.runTasks)(
      new import_tasks.TestRun(config, reporter),
      [
  ...
  async _innerListTests(params) {
    ...
    if (!config) return { report, reporter, status: "failed" };
  ...
  async _innerRunTests(params) {
    ...
    const overrides = {
      ...
      use: {
        ...
        ...(params.trace === "on"
          ? { trace: { mode: "on", sources: false, _live: true } }
          : {}),
    ...
    if (params.trace === "on") process.env.PW_LIVE_TRACE_STACKS = "1";
    else process.env.PW_LIVE_TRACE_STACKS = void 0;
    const wireReporter = await this._wireReporter((e) =>
      this._dispatchEvent("report", e),
    );
    const config = await this._loadConfigOrReportError(
      new import_internalReporter.InternalReporter([wireReporter]),
      overrides,
    );
    ...
    const configReporters = await (0, import_reporters.createReporters)(
      config,
      "test",
      true,
    );
    ...
    const run = (0, import_tasks.runTasks)(
      new import_tasks.TestRun(config, reporter),
      tasks,
      0,
      stop,
  ...
  async watch(params) {
    ...
    for (const fileName of params.fileNames) {
      ...
      (0, import_compilationCache.dependenciesForTestFile)(fileName).forEach(
        (file) => this._watchedTestDependencies.add(file),
      );
    }
    await this._updateWatcher(true);
  }
  async findRelatedTestFiles(params) {
    const errorReporter = (0, import_reporters.createErrorCollectingReporter)(
      import_base.internalScreen,
    );
    const reporter = new import_internalReporter.InternalReporter([
      errorReporter,
    ]);
    const config = await this._loadConfigOrReportError(reporter);
    if (!config) return { errors: errorReporter.errors(), testFiles: [] };
    const status = await (0, import_tasks.runTasks)(
      new import_tasks.TestRun(config, reporter),
      [
        (0, import_tasks.createLoadTask)("out-of-process", {
    ...
    return {
      testFiles: (0, import_compilationCache.affectedTestFiles)(params.files),
    };
  }
  async stopTests() {
  ...
  async _loadConfig(overrides) {
    try {
      ...
      if (!this._plugins) {
        (0, import_webServerPlugin.webServerPluginsForConfig)(config).forEach(
          (p) => config.plugins.push({ factory: p }),
        );
...
async function resolveCtDirs(config) {
  ...
  const templateDir = await import_fs.default.promises
    .realpath(
      import_path.default.normalize(
        import_path.default.join(config.configDir, relativeTemplateDir),
      ),
    )
    .catch(() => void 0);
  if (!templateDir) return null;
  const outDir = use.ctCacheDir
    ? import_path.default.resolve(config.configDir, use.ctCacheDir)
    : import_path.default.resolve(templateDir, ".cache");
  return {
    outDir,
    templateDir,
  };
