...
async function createReporters(config, mode, isTestServer, descriptions) {
  ...
  if (config.configCLIOverrides.additionalReporters)
    descriptions = [
      ...descriptions,
      ...config.configCLIOverrides.additionalReporters,
    ];
  ...
  for (const r of descriptions) {
    ...
    if (name in defaultReporters) {
      ...
      const reporterConstructor = await (0, import_loadUtils.loadReporter)(
        config,
        name,
      );
      reporters.push(
        (0, import_reporterV2.wrapReporterAsV2)(
          new reporterConstructor(options),
        ),
      );
    }
  }
  if (process.env.PW_TEST_REPORTER) {
    ...
    reporters.push(
      (0, import_reporterV2.wrapReporterAsV2)(
        new reporterConstructor(runOptions),
      ),
    );
  }
  ...
  if (reporters.length && !someReporterPrintsToStdio) {
    if (mode === "list") reporters.unshift(new ListModeReporter());
    else if (mode !== "merge")
      reporters.unshift(
        !process.env.CI
          ? new import_line.default({ omitFailures: true })
          : new import_dot.default(),
      );
  }
  return reporters;
}
async function createReporterForTestServer(file, messageSink) {
  ...
  return (0, import_reporterV2.wrapReporterAsV2)(
    ...
    onError(error) {
      ...
      if (writeToConsole)
        process.stdout.write(
          (0, import_base.formatError)(screen, error).message + "\n",
        );
    },
    errors: () => errors,
  };
...
function computeCommandHash(config) {
  ...
  if (Object.keys(command).length)
    parts.push(
      (0, import_utils.calculateSha1)(JSON.stringify(command)).substring(0, 7),
    );
  return parts.join("-");
}
class ListModeReporter {
  ...
  onBegin(suite) {
    ...
    for (const test of tests) {
      ...
      console.log(
        `  ${projectTitle}${location} \u203A ${titles.join(" \u203A ")}`,
      );
