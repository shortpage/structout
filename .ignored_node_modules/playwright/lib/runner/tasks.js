...
async function runTasks(testRun, tasks, globalTimeout, cancelPromise) {
  const deadline = globalTimeout
    ? (0, import_utils.monotonicTime)() + globalTimeout
    : 0;
  const taskRunner = new import_taskRunner.TaskRunner(
    testRun.reporter,
    globalTimeout || 0,
  );
...
function createGlobalSetupTasks(config) {
  const tasks = [];
  if (
    !config.configCLIOverrides.preserveOutputDir &&
    !process.env.PW_TEST_NO_REMOVE_OUTPUT_DIRS
  )
    tasks.push(createRemoveOutputDirsTask());
  tasks.push(
    ...createPluginSetupTasks(config),
    ...config.globalTeardowns
      .map((file) => createGlobalTeardownTask(file, config))
      .reverse(),
...
function createClearCacheTask(config) {
  return {
    title: "clear cache",
    setup: async () => {
      await (0, import_util2.removeDirAndLogToConsole)(
        import_compilationCache.cacheDir,
      );
...
function createGlobalSetupTask(file, config) {
  ...
  return {
    ...
    setup: async ({ config: config2 }) => {
      const setupHook = await (0, import_loadUtils.loadGlobalHook)(
        config2,
        file,
      );
...
function createGlobalTeardownTask(file, config) {
  ...
  return {
    ...
    teardown: async ({ config: config2 }) => {
      const teardownHook = await (0, import_loadUtils.loadGlobalHook)(
        config2,
        file,
      );
...
function createRemoveOutputDirsTask() {
  return {
    ...
    setup: async ({ config }) => {
      ...
      const projects = (0, import_projectUtils.filterProjects)(
        config.projects,
      ...
      await Promise.all(
        Array.from(outputDirs).map((outputDir) =>
          (0, import_utils.removeFolders)([outputDir]).then(async ([error]) => {
            if (!error) return;
            if (error.code === "EBUSY") {
              const entries = await readDirAsync(outputDir).catch((e) => []);
              await Promise.all(
                entries.map((entry) =>
                  (0, import_utils.removeFolders)([
                    import_path.default.join(outputDir, entry),
                  ]),
                ),
              );
            } else {
...
function createListFilesTask() {
  return {
    ...
    setup: async (testRun, errors) => {
      testRun.rootSuite = await (0, import_loadUtils.createRootSuite)(
        testRun,
        errors,
        false,
      );
      ...
      for (const [project, files] of testRun.projectFiles) {
        const projectSuite = new import_test.Suite(
        ...
        const suites = files.map((file) => {
          const title = import_path.default.relative(
...
function createLoadTask(mode, options) {
  return {
    ...
    setup: async (testRun, errors, softErrors) => {
      await (0, import_loadUtils.collectProjectsAndTestFiles)(
        testRun,
        !!options.doNotRunDepsOutsideProjectFilter,
      );
      await (0, import_loadUtils.loadFileSuites)(
        testRun,
        mode,
      ...
      if (testRun.config.cliOnlyChanged) {
        const changedFiles = await (0, import_vcs.detectChangedTestFiles)(
...
function createPhasesTask() {
  return {
    ...
    setup: async (testRun) => {
      ...
      const teardownToSetups = (0,
      import_projectUtils.buildTeardownToSetupsMap)(allProjects);
      const teardownToSetupsDependents = /* @__PURE__ */ new Map();
      for (const [teardown, setups] of teardownToSetups) {
        const closure = (0, import_projectUtils.buildDependentProjects)(
          setups,
          allProjects,
        );
      ...
      for (let i = 0; i < projectToSuite.size; i++) {
        ...
        if (phaseProjects.length) {
          ...
          const phase = {
            dispatcher: new import_dispatcher.Dispatcher(
              testRun.config,
              testRun.reporter,
              testRun.failureTracker,
            ),
            projects: [],
          };
          ...
          for (const project of phaseProjects) {
            ...
            const testGroups = (0, import_testGroups.createTestGroups)(
              projectSuite,
          ...
          (0, import_utilsBundle.debug)("pw:test:task")(
...
function createRunTestsTask() {
  return {
    ...
    setup: async ({ phases, failureTracker }) => {
      ...
      const teardownToSetups = (0,
      import_projectUtils.buildTeardownToSetupsMap)(
        phases.map((phase) => phase.projects.map((p) => p.project)).flat(),
      );
