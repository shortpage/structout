...
(0, import_utils.setBoxedStackPrefixes)([
...
const playwrightFixtures = {
    ...
    async (
      ...
      if (connectOptions) {
        const browser2 = await playwright[browserName].connect({
          ...connectOptions,
          exposeNetwork:
            connectOptions.exposeNetwork ?? connectOptions._exposeNetwork,
          headers: {
            // HTTP headers are ASCII only (not UTF-8).
            "x-playwright-launch-options": (0,
            import_utils.jsonStringifyForceASCII)(_browserOptions),
  ...
  _setupContextOptions: [
    async (
      ...
      if ((0, import_utils.debugMode)()) testInfo._setDebugMode();
  ...
  _setupArtifacts: [
    async ({ playwright, screenshot, _optionErrorContext }, use, testInfo) => {
    ...
    async (
      ...
      const closeReason =
        testInfo.status === "timedOut"
          ...
          : "Test ended.";
      await Promise.all(
        [...contexts.keys()].map(async (context) => {
          await context._wrapApiCall(async () => {
  ...
  _optionErrorContext: [
    process.env.PLAYWRIGHT_NO_COPY_PROMPT ? void 0 : { format: "markdown" },
    { scope: "worker", option: true },
  ],
  _reuseContext: [
    async ({ video, _optionContextReuseMode }, use) => {
  ...
  context: async (
    { playwright, browser, _reuseContext, _contextFactory },
    use,
    testInfo,
  ) => {
  ...
  async _snapshotPage(page, temporary) {
    ...
    try {
      const path2 = temporary
        ? this._createTemporaryArtifact(
            (0, import_utils.createGuid)() + this._extension,
          )
...
class ArtifactsRecorder {
  ...
  async willStartTest(testInfo) {
    ...
    await Promise.all(
      this._playwright._allContexts().map(async (context) => {
        if (context[kIsReusedContext]) this._reusedContexts.add(context);
        else await this.didCreateBrowserContext(context);
      }),
    );
    {
      const existingApiRequests = Array.from(
        this._playwright.request._contexts,
      );
      await Promise.all(
  ...
  async didFinishTest() {
    ...
    const leftoverContexts = this._playwright
      ._allContexts()
      .filter((context2) => !this._reusedContexts.has(context2));
    ...
    if (this._errorContext)
      await (0, import_errorContext.attachErrorContext)(
        this._testInfo,
        this._errorContext.format,
        this._sourceCache,
        this._pageSnapshot,
      );
  }
...
function renderApiCall(apiName, params) {
  ...
  if (params) {
    for (const name of paramsToRender(apiName)) {
      ...
      if (
        name === "selector" &&
        (0, import_utils.isString)(params[name]) &&
        params[name].startsWith("internal:")
      ) {
