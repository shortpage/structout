...
async function loadUserConfig(location) {
  let object = location.resolvedConfigFile
    ? await (0, import_transform.requireOrImport)(location.resolvedConfigFile)
    : {};
...
async function loadConfig(
  ...
  const fullConfig = new import_config.FullConfigInternal(
    location,
    userConfig,
    overrides || {},
    metadata,
...
function validateConfig(file, config) {
  ...
  if ("forbidOnly" in config && config.forbidOnly !== void 0) {
    if (typeof config.forbidOnly !== "boolean")
      throw (0, import_util.errorWithFile)(
        file,
        `config.forbidOnly must be a boolean`,
      );
  }
  if ("globalSetup" in config && config.globalSetup !== void 0) {
    if (Array.isArray(config.globalSetup)) {
      ...
      throw (0, import_util.errorWithFile)(
        file,
        `config.globalSetup must be a string`,
      );
    }
  }
  if ("globalTeardown" in config && config.globalTeardown !== void 0) {
    if (Array.isArray(config.globalTeardown)) {
      ...
      throw (0, import_util.errorWithFile)(
        file,
        `config.globalTeardown must be a string`,
      );
    }
  }
    ...
    if (Array.isArray(config.grep)) {
      config.grep.forEach((item, index) => {
        if (!(0, import_utils2.isRegExp)(item))
          throw (0, import_util.errorWithFile)(
            file,
            `config.grep[${index}] must be a RegExp`,
          );
      });
    } else if (!(0, import_utils2.isRegExp)(config.grep)) {
      throw (0, import_util.errorWithFile)(
        file,
        `config.grep must be a RegExp`,
      );
    }
  }
  if ("grepInvert" in config && config.grepInvert !== void 0) {
    if (Array.isArray(config.grepInvert)) {
      ...
      throw (0, import_util.errorWithFile)(
        file,
        `config.grepInvert must be a RegExp`,
      );
    }
  }
  ...
  if ("projects" in config && config.projects !== void 0) {
    if (!Array.isArray(config.projects))
      throw (0, import_util.errorWithFile)(
        file,
        `config.projects must be an array`,
      );
  ...
  if ("quiet" in config && config.quiet !== void 0) {
    if (typeof config.quiet !== "boolean")
      throw (0, import_util.errorWithFile)(
        file,
        `config.quiet must be a boolean`,
      );
  }
  if ("reporter" in config && config.reporter !== void 0) {
    if (Array.isArray(config.reporter)) {
      ...
      throw (0, import_util.errorWithFile)(
        file,
        `config.reporter must be a string`,
      );
    }
  }
  if (
    ...
    if (!config.reportSlowTests || typeof config.reportSlowTests !== "object")
      throw (0, import_util.errorWithFile)(
        file,
        `config.reportSlowTests must be an object`,
      );
    if (
  ...
  if ("shard" in config && config.shard !== void 0 && config.shard !== null) {
    if (!config.shard || typeof config.shard !== "object")
      throw (0, import_util.errorWithFile)(
        file,
        `config.shard must be an object`,
      );
    if (
  ...
  if ("tsconfig" in config && config.tsconfig !== void 0) {
    if (typeof config.tsconfig !== "string")
      throw (0, import_util.errorWithFile)(
        file,
        `config.tsconfig must be a string`,
      );
    if (
      !import_fs.default.existsSync(
        import_path.default.resolve(file, "..", config.tsconfig),
      )
    )
      throw (0, import_util.errorWithFile)(
        file,
        `config.tsconfig does not exist`,
      );
  }
}
function validateProject(file, project, title) {
  ...
  if ("outputDir" in project && project.outputDir !== void 0) {
    if (typeof project.outputDir !== "string")
      throw (0, import_util.errorWithFile)(
        file,
        `${title}.outputDir must be a string`,
      );
  }
  ...
  if ("testDir" in project && project.testDir !== void 0) {
    if (typeof project.testDir !== "string")
      throw (0, import_util.errorWithFile)(
        file,
        `${title}.testDir must be a string`,
      );
  }
  for (const prop of ["testIgnore", "testMatch"]) {
    if (prop in project && project[prop] !== void 0) {
      ...
      if (Array.isArray(value)) {
        ...
        typeof value !== "string" &&
        !(0, import_utils2.isRegExp)(value)
      ) {
        throw (0, import_util.errorWithFile)(
          file,
  ...
  if ("ignoreSnapshots" in project && project.ignoreSnapshots !== void 0) {
    if (typeof project.ignoreSnapshots !== "boolean")
      throw (0, import_util.errorWithFile)(
        file,
        `${title}.ignoreSnapshots must be a boolean`,
      );
  }
  if ("workers" in project && project.workers !== void 0) {
    if (typeof project.workers === "number" && project.workers <= 0)
      throw (0, import_util.errorWithFile)(
        file,
        `${title}.workers must be a positive number`,
      );
    else if (
...
async function loadConfigFromFileRestartIfNeeded(
  configFile,
  overrides,
  ignoreDeps,
) {
...
function restartWithExperimentalTsEsm(configFile, force = false) {
  ...
  if (globalThis.__esmLoaderPortPreV20) {
    process.execArgv = (0,
    import_esmUtils.execArgvWithoutExperimentalLoaderOptions)();
    return false;
  }
  if (!require("node:module").register) {
    const configIsModule =
      !!configFile && (0, import_util.fileIsModule)(configFile);
    if (!force && !configIsModule) return false;
    const innerProcess = require("child_process").fork(
      require.resolve("../../cli"),
