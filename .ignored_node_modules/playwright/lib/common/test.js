...
class Suite extends Base {
  ...
  _deepClone() {
    ...
    for (const entry of this._entries) {
      if (entry instanceof Suite) suite._addSuite(entry._deepClone());
  ...
  _deepSerialize() {
    ...
    suite.entries = [];
    for (const entry of this._entries) {
      if (entry instanceof Suite) suite.entries.push(entry._deepSerialize());
  ...
  static _deepParse(data) {
    ...
    for (const entry of data.entries) {
      if (entry.kind === "suite") suite._addSuite(Suite._deepParse(entry));
  ...
  forEachTest(visitor) {
    for (const entry of this._entries) {
      if (entry instanceof Suite) entry.forEachTest(visitor);
