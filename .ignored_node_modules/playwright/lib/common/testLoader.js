...
async function loadTestFile(file, rootDir, testErrors) {
  ...
  const suite = new import_test.Suite(
    import_path.default.relative(rootDir, file) ||
      import_path.default.basename(file),
    "file",
  );
  ...
  }
  {
    ...
    if (files.size === 1) {
      ...
      if (suite.location.file !== mappedFile) {
        if (
          import_path.default.extname(mappedFile) !==
          import_path.default.extname(suite.location.file)
        )
