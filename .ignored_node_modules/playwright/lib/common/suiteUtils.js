...
function bindFileSuiteToProject(project, suite) {
  const relativeFile = import_path.default.relative(
    project.project.testDir,
    suite.location.file,
  );
  const fileId = (0, import_utils.calculateSha1)(
    (0, import_utils.toPosixPath)(relativeFile),
  ).slice(0, 20);
  ...
  result.forEachTest((test, suite2) => {
    ...
    const testId =
      fileId +
      "-" +
      (0, import_utils.calculateSha1)(testIdExpression).slice(0, 20);
    test.id = testId;
...
function filterByFocusedLine(suite, focusedTestFileLines) {
  ...
  const testFileLineMatches = (testFileName, testLine, testColumn) =>
    matchers.some((m) => m(testFileName, testLine, testColumn));
  const suiteFilter = (suite2) =>
    !!suite2.location &&
    testFileLineMatches(
      suite2.location.file,
      suite2.location.line,
      suite2.location.column,
    );
...
function createFileMatcherFromFilter(filter) {
  const fileMatcher = (0, import_util.createFileMatcher)(
    filter.re || filter.exact || "",
  );
  return (testFileName, testLine, testColumn) =>
    fileMatcher(testFileName) &&
    (filter.line === testLine || filter.line === null) &&
    (filter.column === testColumn || filter.column === null);
}
