...
function registerESMLoader() {
  ...
  require("node:module").register(
    import_url.default.pathToFileURL(require.resolve("../transform/esmLoader")),
    {
      data: { port: port2 },
      transferList: [port2],
    },
...
async function configureESMLoader() {
  ...
  await loaderChannel.send("setSingleTSConfig", {
    tsconfig: (0, import_transform.singleTSConfig)(),
  });
...
async function configureESMLoaderTransformConfig() {
  ...
  await loaderChannel.send("setTransformConfig", {
    config: (0, import_transform.transformConfig)(),
  });
}
