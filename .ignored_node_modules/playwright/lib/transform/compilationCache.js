...
function getFromCompilationCache(filename, hash, moduleUrl) {
  ...
  if (cache?.codePath) {
    try {
      return {
        cachedCode: import_fs.default.readFileSync(cache.codePath, "utf-8"),
      };
    } catch {}
  }
...
const kPlaywrightInternalPrefix = import_path.default.resolve(
  __dirname,
  "../../../playwright",
);
function belongsToNodeModules(file) {
  if (
    ...
    return true;
  if (
    file.startsWith(kPlaywrightInternalPrefix) &&
