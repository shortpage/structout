...
async function resolve(specifier, context, defaultResolve) {
  ...
  if (result?.url && result.url.startsWith("file://"))
    (0, import_compilationCache.currentFileDepsCollector)()?.add(
      import_url.default.fileURLToPath(result.url),
    );
  return result;
}
...
async function load(moduleUrl, context, defaultLoad) {
  ...
  const transformed = (0, import_transform.transformHook)(
    code,
    filename,
    moduleUrl,
  );
  if (transformed.serializedCache)
    await transport?.send("pushToCompilationCache", {
      cache: transformed.serializedCache,
    });
  return {
    format:
      kSupportedFormats.get(context.format) ||
      ((0, import_util.fileIsModule)(filename) ? "module" : "commonjs"),
...
function createTransport(port) {
  return new import_portTransport.PortTransport(
    port,
    async (method, params) => {
      ...
      if (method === "getCompilationCache")
        return {
          cache: (0, import_compilationCache.serializeCompilationCache)(),
        };
