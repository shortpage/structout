...
function setTransformConfig(config) {
  ...
  _externalMatcher = (0, import_util.createFileMatcher)(
    _transformConfig.external,
  );
}
...
function loadAndValidateTsconfigsForFile(file2) {
  if (_singleTSConfigPath && !_singleTSConfig)
    _singleTSConfig = (0, import_tsconfig_loader.loadTsConfig)(
      _singleTSConfigPath,
...
function resolveHook(filename, specifier) {
  ...
  if (isRelativeSpecifier(specifier))
    return (0, import_util.resolveImportSpecifierAfterMapping)(
  ...
  for (const tsconfig of tsconfigs) {
    ...
    for (const { key, values } of tsconfig.paths) {
      ...
      for (const value of values) {
        ...
        const existing = (0, import_util.resolveImportSpecifierAfterMapping)(
          candidate,
          true,
        );
        if (existing) {
...
function transformHook(originalCode, filename, moduleUrl) {
  ...
  const { cachedCode, addToCache, serializedCache } = (0,
  import_compilationCache.getFromCompilationCache)(filename, hash, moduleUrl);
...
async function requireOrImport(file) {
  ...
  const esmImport = () =>
    eval(`import(${JSON.stringify(import_url.default.pathToFileURL(file))})`);
  ...
  if (depsCollector) {
    ...
    if (module2) collectCJSDependencies(module2, depsCollector);
  }
  return result;
}
...
function installTransformIfNeeded() {
  ...
  import_utilsBundle.pirates.addHook(
    (code, filename) => {
...
const collectCJSDependencies = (module2, dependencies) => {
  module2.children.forEach((child) => {
    if (
      !(0, import_compilationCache.belongsToNodeModules)(child.filename) &&
...
function wrapFunctionWithLocation(func) {
  return (...args) => {
    ...
    Error.prepareStackTrace = (error, stackFrames) => {
      const frame = import_utilsBundle.sourceMapSupport.wrapCallSite(
        stackFrames[1],
      );
      const fileName = frame.getFileName();
      const file2 =
        fileName && fileName.startsWith("file://")
          ? import_url.default.fileURLToPath(fileName)
          : fileName;
      return {
        file: file2,
