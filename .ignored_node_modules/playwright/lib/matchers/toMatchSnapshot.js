...
class SnapshotHelper {
  constructor(
    ...
    if (!name) {
      ...
      expectedPathSegments = Array.isArray(name)
        ? name
        : [(0, import_util.sanitizeFilePathBeforeExtension)(name)];
      const joinedName = Array.isArray(name)
        ? name.join(import_path.default.sep)
        : (0, import_util.sanitizeFilePathBeforeExtension)(
            (0, import_util.trimLongString)(
              name,
              import_util.windowsFilesystemFriendlyLength,
            ),
      ...
      const sanitizedName =
        index > 1
          ? (0, import_util.addSuffixToFilePath)(joinedName, `-${index - 1}`)
          : joinedName;
      outputBasePath = testInfo._getOutputPath(sanitizedName);
      this.attachmentBaseName = sanitizedName;
    }
    ...
    this.expectedPath = testInfo._resolveSnapshotPath(
      configOptions.pathTemplate,
      defaultTemplate,
      expectedPathSegments,
    );
    this.legacyExpectedPath = (0, import_util.addSuffixToFilePath)(
      outputBasePath,
      "-expected",
    );
    this.previousPath = (0, import_util.addSuffixToFilePath)(
      outputBasePath,
      "-previous",
    );
    this.actualPath = (0, import_util.addSuffixToFilePath)(
      outputBasePath,
      "-actual",
    );
    this.diffPath = (0, import_util.addSuffixToFilePath)(
      outputBasePath,
      "-diff",
    );
    ...
    this.mimeType =
      import_utilsBundle.mime.getType(
        import_path.default.basename(this.expectedPath),
  ...
  handleMissing(actual, step) {
    ...
    step?._attachToStep({
      name: (0, import_util.addSuffixToFilePath)(
...
Expected: ${import_utils2.colors.yellow(this.expectedPath)}`);
    ...
    if (previous !== void 0) {
      ...
      output.push(
        `Previous: ${import_utils2.colors.yellow(this.previousPath)}`,
      );
    }
    if (actual !== void 0) {
      writeFileSync(this.actualPath, actual);
    ...
    if (log?.length) output.push((0, import_util.callLogText)(log));
    else output.push("");
...
function toMatchSnapshot(received, nameOrOptions = {}, optOptions = {}) {
  ...
  const header = (0, import_matcherHint.matcherHint)(
    this,
    void 0,
    "toMatchSnapshot",
    receiver,
...
async function toHaveScreenshot(
  ...
  const [page, locator] =
    pageOrLocator.constructor.name === "Page"
      ? [pageOrLocator, void 0]
      : [pageOrLocator.page(), pageOrLocator];
  const configOptions =
    testInfo._projectInternal.expect?.toHaveScreenshot || {};
  const helper = new SnapshotHelper(
    testInfo,
    "toHaveScreenshot",
    locator,
  ...
  if (!helper.expectedPath.toLowerCase().endsWith(".png"))
    throw new Error(
      `Screenshot name "${import_path.default.basename(helper.expectedPath)}" must have '.png' extension`,
    );
  (0, import_util.expectTypes)(
    pageOrLocator,
    ["Page", "Locator"],
  ...
  if (this.isNot) {
    ...
    const isDifferent = !(await page._expectScreenshot(expectScreenshotOptions))
      .errorMessage;
    return isDifferent
      ? helper.handleDifferentNegated()
      : helper.handleMatchingNegated();
  ...
  if (!hasSnapshot) {
    ...
    if (errorMessage2) {
      const header2 = (0, import_matcherHint.matcherHint)(
        this,
        locator,
        "toHaveScreenshot",
        receiver,
  ...
  const { actual, previous, diff, errorMessage, log, timedOut } =
    await page._expectScreenshot(expectScreenshotOptions);
  const writeFiles = () => {
  ...
  if (!errorMessage) {
    if (
      ...
      actual &&
      (0, import_utils.compareBuffersOrStrings)(actual, expected)
    ) {
      console.log(helper.expectedPath + " is re-generated, writing actual.");
      return writeFiles();
    }
