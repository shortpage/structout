...
async function toMatchAriaSnapshot(receiver, expectedParam, options = {}) {
  ...
  const pathTemplate =
    testInfo._projectInternal.expect?.toMatchAriaSnapshot?.pathTemplate;
  ...
  if ((0, import_utils.isString)(expectedParam)) {
    ...
    if (expectedParam?.name) {
      const ext = expectedParam.name.endsWith(".aria.yml")
        ? ".aria.yml"
        : void 0;
      expectedPath = testInfo._resolveSnapshotPath(
        pathTemplate,
        defaultTemplate,
        [
      ...
      if (!(await (0, import_util.fileExistsAsync)(expectedPath))) {
        const legacyPath = testInfo._resolveSnapshotPath(
          pathTemplate,
          defaultTemplate,
          [
            (0, import_utils.sanitizeForFilePath)(
    ...
    expected = await import_fs.default.promises
  ...
  const messagePrefix = (0, import_matcherHint.matcherHint)(
    this,
    receiver,
    matcherName,
    "locator",
...
Received: ${(0, import_expectBundle.EXPECTED_COLOR)("<element not found>")}` +
...
Received: ${receivedText}` +
      ...
      const printedReceived = (0,
      import_expect.printReceivedStringContainExpectedSubstring)(
        receivedText,
        receivedText.indexOf(expected),
        expected.length,
      );
      return (
        messagePrefix +
        `Expected: not ${this.utils.printExpected(expected)}
Received: ${printedReceived}` +
        (0, import_util.callLogText)(log)
      );
    } else {
      ...
      if (notFound)
        return (
          messagePrefix +
          `${labelExpected}: ${this.utils.printExpected(expected)}
Received: ${receivedText}` +
          (0, import_util.callLogText)(log)
        );
      return (
    ...
    if (
      ...
      if (expectedPath) {
        await import_fs.default.promises.mkdir(
          import_path.default.dirname(expectedPath),
          { recursive: true },
        );
