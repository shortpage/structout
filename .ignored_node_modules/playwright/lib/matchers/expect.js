...
const printReceivedStringContainExpectedSubstring = (received, start, length) =>
  (0, import_expectBundle.RECEIVED_COLOR)(
    '"' +
      printSubstring(received.slice(0, start)) +
      (0, import_expectBundle.INVERTED_COLOR)(
...
const printReceivedStringContainExpectedResult = (received, result) =>
  result === null
    ? (0, import_expectBundle.printReceived)(received)
    : printReceivedStringContainExpectedSubstring(
        received,
        result.index,
        result[0].length,
...
function createMatchers(actual, info, prefix) {
  return new Proxy(
    (0, import_expectBundle.expect)(actual),
    new ExpectMetaInfoProxyHandler(info, prefix),
  );
}
...
function createExpect(info, prefix, userMatchers) {
  const expectInstance = new Proxy(import_expectBundle.expect, {
    apply: function (target, thisArg, argumentsList) {
      ...
      const message = (0, import_utils.isString)(messageOrOptions)
        ? messageOrOptions
        : messageOrOptions?.message || info.message;
    ...
    get: function (target, property) {
        ...
        return (matchers) => {
          ...
          for (const [name, matcher] of Object.entries(matchers)) {
            wrappedMatchers[name] =
              wrapPlaywrightMatcherToPassNiceThis(matcher);
      ...
      if (property === "poll") {
        return (actual, messageOrOptions) => {
          const poll = (0, import_utils.isString)(messageOrOptions)
            ? {}
            : messageOrOptions || {};
  ...
  const configure = (configuration) => {
    ...
    if ("_poll" in configuration) {
      ...
      if (typeof configuration._poll === "object") {
        newInfo.poll.timeout =
          configuration._poll.timeout ?? newInfo.poll.timeout;
        newInfo.poll.intervals =
          configuration._poll.intervals ?? newInfo.poll.intervals;
      }
    }
...
function wrapPlaywrightMatcherToPassNiceThis(matcher) {
  return function (...args) {
    ...
    const timeout =
      context?.expectInfo.timeout ??
      context?.testInfo?._projectInternal?.expect?.timeout ??
...
class ExpectMetaInfoProxyHandler {
  ...
  get(target, matcherName, receiver) {
    ...
    for (let i = this._prefix.length; i > 0; i--) {
      const qualifiedName = qualifiedMatcherName(
        this._prefix.slice(0, i),
        matcherName,
      );
    ...
    if (this._info.poll) {
      if (
        ...
        throw new Error(
          `\`expect.poll()\` does not support "${matcherName}" matcher.`,
        );
      matcher = (...args) =>
        pollMatcher(resolvedMatcherName, this._info, this._prefix, ...args);
    }
    return (...args) => {
      ...
      const stackFrames = (0, import_util.filteredStackTrace)(
        (0, import_utils.captureRawStack)(),
      );
      const stepInfo = {
        category:
          matcherName === "toPass" || this._info.poll ? "step" : "expect",
      ...
      const reportStepError = (e) => {
        ...
        const error = jestError
          ? new import_matcherHint.ExpectError(
              jestError,
              customMessage,
              stackFrames,
            )
        ...
        if (jestError?.matcherResult.suggestedRebaseline) {
          step.complete({
            suggestedRebaseline: jestError?.matcherResult.suggestedRebaseline,
          });
          return;
        }
        step.complete({ error });
        if (this._info.isSoft) testInfo._failWithError(error);
        else throw error;
      };
      const finalizer = () => {
        step.complete({});
      ...
      try {
        ...
        const result = (0, import_utils.currentZone)()
          .with("stepZone", step)
          .run(callback);
...
async function pollMatcher(qualifiedMatcherName2, info, prefix, ...args) {
  ...
  const timeout =
    poll.timeout ??
    info.timeout ??
    testInfo?._projectInternal?.expect?.timeout ??
    defaultExpectTimeout;
  const { deadline, timeoutMessage } = testInfo
    ? testInfo._deadlineForMatcher(timeout)
    : import_testInfo.TestInfoImpl._defaultDeadlineForMatcher(timeout);
  const result = await (0, import_utils.pollAgainstDeadline)(
    async () => {
