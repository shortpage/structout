...
function toBeAttached(locator, options) {
  ...
  return import_toBeTruthy.toBeTruthy.call(
    this,
    "toBeAttached",
    locator,
    "Locator",
...
function toContainText(locator, expected, options = {}) {
  if (Array.isArray(expected)) {
    return import_toEqual.toEqual.call(
      this,
      "toContainText",
      locator,
      "Locator",
      async (isNot, timeout) => {
        const expectedText = (0, import_utils.serializeExpectedTextValues)(
          expected,
          {
            matchSubstring: true,
    ...
    return import_toMatchText.toMatchText.call(
      this,
      "toContainText",
      locator,
      "Locator",
...
function toHaveAccessibleDescription(locator, expected, options) {
  return import_toMatchText.toMatchText.call(
    ...
    async (isNot, timeout) => {
      const expectedText = (0, import_utils.serializeExpectedTextValues)(
        [expected],
        { ignoreCase: options?.ignoreCase, normalizeWhiteSpace: true },
      );
  ...
  if (Array.isArray(expected)) {
    return import_toEqual.toEqual.call(
      ...
      async (isNot, timeout) => {
        const expectedText = (0, import_utils.serializeExpectedTextValues)(
          expected,
          { normalizeWhiteSpace: true, ignoreCase: options.ignoreCase },
        );
...
function toHaveURL(page, expected, options) {
  if (typeof expected === "function")
    return import_toHaveURL.toHaveURLWithPredicate.call(
      this,
      page,
      expected,
      options,
  ...
  expected =
    typeof expected === "string"
      ? (0, import_utils.constructURLBasedOnBaseURL)(baseURL, expected)
      : expected;
  const locator = page.locator(":root");
  return import_toMatchText.toMatchText.call(
    this,
...
async function toBeOK(response) {
  ...
  const isTextEncoding =
    contentType && (0, import_utils.isTextualMimeType)(contentType);
  const [log, text] =
    this.isNot === response.ok()
      ...
      : [];
  const message = () =>
    this.utils.matcherHint(matcherName, void 0, "", { isNot: this.isNot }) +
    (0, import_util.callLogText)(log) +
    (text === null
      ? ""
      : `
Response text:
...
async function toPass(callback, options = {}) {
  ...
  const timeout = (0, import_config.takeFirst)(
    options.timeout,
    testInfo?._projectInternal.expect?.toPass?.timeout,
    0,
  );
  const intervals = (0, import_config.takeFirst)(
    options.intervals,
    testInfo?._projectInternal.expect?.toPass?.intervals,
    [100, 250, 500, 1e3],
  );
