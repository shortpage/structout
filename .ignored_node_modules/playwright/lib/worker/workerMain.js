...
class WorkerMain extends import_process.ProcessRunner {
  ...
  async gracefullyClose() {
    try {
      ...
      const fakeTestInfo = new import_testInfo.TestInfoImpl(
        this._config,
        this._project,
        this._params,
        void 0,
      ...
      await this._fixtureRunner
        ...
        .catch(() => {});
      await fakeTestInfo
        ._runWithTimeout(runnable, () => (0, import_utils.gracefullyCloseAll)())
        .catch(() => {});
  ...
  async _loadIfNeeded() {
    ...
    const config = await (0, import_configLoader.deserializeConfig)(
      this._params.config,
    );
    ...
    this._poolBuilder = import_poolBuilder.PoolBuilder.createForWorker(
      this._project,
    );
  }
  async runTestGroup(runPayload) {
    ...
    try {
      ...
      const fileSuite = await (0, import_testLoader.loadTestFile)(
        runPayload.file,
        this._config.config.rootDir,
      );
      const suite = (0, import_suiteUtils.bindFileSuiteToProject)(
        this._project,
        fileSuite,
      );
      ...
      const hasEntries = (0, import_suiteUtils.filterTestsRemoveEmptySuites)(
        suite,
        (test) => entries.has(test.id),
      );
      if (hasEntries) {
    ...
    await (async () => {
      ...
      await testInfo._runAsStep(
        ...
        async () => {
          ...
          testFunctionParams =
            await this._fixtureRunner.resolveParametersForFunction(
              test.fn,
              testInfo,
              "test",
              { type: "test" },
      ...
      ._runAsStep({ title: "After Hooks", category: "hook" }, async () => {
        ...
        try {
          await testInfo._runWithTimeout(
            ...
            async () => testInfo._onDidFinishTestFunction?.(),
          );
        } catch (error) {
  ...
  async _runAllHooksForSuite(suite, testInfo, type, extraAnnotations) {
    ...
    for (const hook of this._collectHooksAndModifiers(suite, type, testInfo)) {
      try {
        ...
        if (
          type === "beforeAll" &&
          error instanceof import_testInfo.TestSkipError
        )
          break;
  ...
  async _runEachHooksForSuites(suites, type, testInfo, slot) {
    ...
    for (const hook of hooks) {
      ...
      try {
        ...
        firstError = firstError ?? error;
        if (error instanceof import_testInfo.TestSkipError) break;
      }
    }
    if (firstError) throw firstError;
  }
