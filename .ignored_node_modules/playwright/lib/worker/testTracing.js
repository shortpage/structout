...
class TestTracing {
  ...
  async startIfNeeded(value) {
    ...
    if (!this._liveTraceFile && this._options._live) {
      this._liveTraceFile = {
        ...
        fs: new import_utils.SerializedFS(),
      };
      this._liveTraceFile.fs.mkdir(
        import_path.default.dirname(this._liveTraceFile.file),
      );
      const data =
  ...
  async stopIfNeeded() {
        ...
        await import_fs.default.promises
          ...
          .then((source) => {
            zipFile.addBuffer(
              Buffer.from(source),
              "resources/src@" +
                (0, import_utils.calculateSha1)(sourceFile) +
                ".txt",
            );
          })
          .catch(() => {});
    ...
    for (const event of this._traceEvents.filter((e) => e.type === "after")) {
      for (const attachment of event.attachments || []) {
        let contentPromise;
        if (attachment.path)
          contentPromise = import_fs.default.promises
    ...
    await new Promise((f) => {
      zipFile.end(void 0, () => {
        zipFile.outputStream
          .pipe(
            import_fs.default.createWriteStream(
  ...
  appendForError(error) {
    ...
    const stack = rawStack
      ? (0, import_util2.filteredStackTrace)(rawStack)
      : [];
  ...
  appendTopLevelAttachment(attachment) {
    ...
    this.appendBeforeActionForStep(
      callId,
      void 0,
      import_util.kTopLevelAttachmentPrefix,
      `${import_util.kTopLevelAttachmentPrefix} "${attachment.name}"`,
      void 0,
      [],
    );
...
async function mergeTraceFiles(fileName, temporaryTraceFiles) {
  temporaryTraceFiles = temporaryTraceFiles.filter((file) =>
    import_fs.default.existsSync(file),
  );
  ...
  zipFile.end(void 0, () => {
    zipFile.outputStream
      .pipe(import_fs.default.createWriteStream(fileName))
      .on("close", () => {
        void Promise.all(
          temporaryTraceFiles.map((tempFile) =>
            import_fs.default.promises.unlink(tempFile),
          ),
        )
          .then(() => {
