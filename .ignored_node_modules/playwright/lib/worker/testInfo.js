...
class TestInfoImpl {
  constructor(
    ...
    this._timeoutManager = new import_timeoutManager.TimeoutManager(
      this.project.timeout,
    );
    if (configInternal.configCLIOverrides.debug) this._setDebugMode();
    this.outputDir = (() => {
      const relativeTestFilePath = import_path.default.relative(
      ...
      let testOutputDir = (0, import_util.trimLongString)(
        sanitizedRelativePath +
          "-" +
          (0, import_utils.sanitizeForFilePath)(fullTitleWithoutSpec),
        import_util.windowsFilesystemFriendlyLength,
      );
      if (projectInternal.id)
        testOutputDir +=
          "-" + (0, import_utils.sanitizeForFilePath)(projectInternal.id);
    ...
    this._tracing = new import_testTracing.TestTracing(
      this,
      workerParams.artifactsDir,
    );
  }
  get error() {
  ...
  _deadlineForMatcher(timeout) {
    ...
    const matcherDeadline = timeout
      ? startTime + timeout
      : import_timeoutManager.kMaxDeadline;
    ...
    return {
      ...
      timeoutMessage:
        testDeadline < matcherDeadline ? testMessage : matcherMessage,
    };
  }
  static _defaultDeadlineForMatcher(timeout) {
    return {
      deadline: timeout ? (0, import_utils.monotonicTime)() + timeout : 0,
  ...
  _parentStep() {
    return (
      (0, import_utils.currentZone)().data("stepZone") ??
  ...
  _addStep(data, parentStep) {
    ...
    const filteredStack = (0, import_util.filteredStackTrace)(
      (0, import_utils.captureRawStack)(),
    );
...
${(0, import_utils.stringifyStackFrames)(data.boxedStack).join("\n")}`;
  ...
  _failWithError(error) {
    if (this.status === "passed" || this.status === "skipped")
      this.status =
        error instanceof import_timeoutManager.TimeoutManagerError
          ? "timedOut"
          : "failed";
...
${(0, import_utils.stringifyStackFrames)(step.boxedStack).join("\n")}`;
  ...
  async attach(name, options = {}) {
    ...
    this._attach(
      await (0, import_util.normalizeAndSaveAttachment)(
        this.outputPath(),
        name,
        options,
      ),
  ...
  _attach(attachment, stepId) {
    ...
    if (stepId) this._stepMap.get(stepId).attachmentIndices.push(index);
    else this._tracing.appendTopLevelAttachment(attachment);
    this._onAttach({
      testId: this.testId,
  ...
  _getOutputPath(...pathSegments) {
    ...
    const outputPath = (0, import_util.getContainedPath)(
      this.outputDir,
      joinedPath,
    );
  ...
  _fsSanitizedTestName() {
    const fullTitleWithoutSpec = this.titlePath.slice(1).join(" ");
    return (0, import_utils.sanitizeForFilePath)(
      (0, import_util.trimLongString)(fullTitleWithoutSpec),
    );
  }
  _resolveSnapshotPath(template, defaultTemplate, pathSegments, extension) {
    ...
    const parsedRelativeTestFilePath =
      import_path.default.parse(relativeTestFilePath);
    const projectNamePathSegment = (0, import_utils.sanitizeForFilePath)(
      this.project.name,
    );
    const actualTemplate =
      template || this._projectInternal.snapshotPathTemplate || defaultTemplate;
    const snapshotPath = actualTemplate
      ...
      .replace(/\{(.)?arg\}/g, "$1" + import_path.default.join(dir, name))
      .replace(/\{(.)?ext\}/g, ext ? "$1" + ext : "");
    return import_path.default.normalize(
      import_path.default.resolve(this._configInternal.configDir, snapshotPath),
    );
  }
