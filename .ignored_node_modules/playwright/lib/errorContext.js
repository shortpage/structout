...
async function attachErrorContext(testInfo, format, sourceCache, ariaSnapshot) {
  ...
  for (const [index, error] of errors) {
    ...
    if (ariaSnapshot) {
      lines.push("", "# Page snapshot", "", "```yaml", ariaSnapshot, "```");
    }
    const parsedError = error.stack
      ? (0, import_utils.parseErrorStack)(error.stack, path.sep)
      : void 0;
    const inlineMessage = (0, import_util.stripAnsiEscapes)(
    ...
    if (metadata.gitDiff) {
      lines.push("", "# Local changes", "", "```diff", metadata.gitDiff, "```");
    }
    const filePath = testInfo.outputPath(
      errors.length === 1 ? `error-context.md` : `error-context-${index}.md`,
    );
