...
function addListFilesCommand(program3) {
  const command = program3.command("list-files [file-filter...]", {
    hidden: true,
  });
  ...
  command.option(
    ...
    `Only run tests from the specified list of projects, supports '*' wildcard (default: list all projects)`,
  );
...
function addFindRelatedTestFilesCommand(program3) {
  ...
  command.action(async (files, options) => {
    const resolvedFiles = files.map((file) =>
      import_path.default.resolve(process.cwd(), file),
    );
    await withRunnerAndMutedWrite(options.config, (runner) =>
      runner.findRelatedTestFiles(resolvedFiles),
    );
  });
}
function addDevServerCommand(program3) {
  ...
  command.action(async (options) => {
    const config = await (0,
    import_configLoader.loadConfigFromFileRestartIfNeeded)(options.config);
    if (!config) return;
    const runner = new import_runner.Runner(config);
...
function addShowReportCommand(program3) {
  ...
  command.action((report, options) =>
    (0, import_html.showHTMLReport)(report, options.host, +options.port),
  );
...
function addMergeReportsCommand(program3) {
  ...
  command.option(
    "--reporter <reporter>",
    `Reporter to use, comma-separated, can be ${import_config.builtInReporters.map((name) => `"${name}"`).join(", ")} (default: "${import_config.defaultReporter}")`,
  );
  command.addHelpText(
    "afterAll",
    `
...
async function runTests(args, opts) {
  ...
  const config = await (0,
  import_configLoader.loadConfigFromFileRestartIfNeeded)(
    opts.config,
    cliOverrides,
    opts.deps === false,
  );
  ...
  (0, import_projectUtils.filterProjects)(
    config.projects,
...
async function runTestServer(opts) {
  ...
  const status = await testServer.runTestServer(
    opts.config,
    {},
    { host, port },
  );
...
async function withRunnerAndMutedWrite(configFile, callback) {
  ...
  try {
    const config = await (0,
    import_configLoader.loadConfigFromFileRestartIfNeeded)(configFile);
    if (!config) return;
    const runner = new import_runner.Runner(config);
...
async function mergeReports(reportDir, opts) {
  ...
  const config = configFile
    ? await (0, import_configLoader.loadConfigFromFileRestartIfNeeded)(
        configFile,
      )
    : await (0, import_configLoader.loadEmptyConfigForMergeReports)();
  if (!config) return;
  ...
  await (0, import_merge.createMergedReport)(
    config,
    dir,
...
function overridesFromOptions(options) {
  ...
  else updateSnapshots = "updateSnapshots" in options ? "changed" : void 0;
  const overrides = {
  ...
  if (options.headed || options.debug) overrides.use = { headless: false };
  ...
  if (!options.ui && options.trace) {
    if (!kTraceModes.includes(options.trace))
      throw new Error(
        `Unsupported trace mode "${options.trace}", must be one of ${kTraceModes.map((mode) => `"${mode}"`).join(", ")}`,
      );
...
function resolveReporter(id) {
  if (import_config.builtInReporters.includes(id)) return id;
...
const testOptions = [
  ...
  [
    "--timeout <timeout>",
    `Specify test timeout threshold in milliseconds, zero for unlimited (default: ${import_config.defaultTimeout})`,
  ],
  [
    "--trace <mode>",
    `Force tracing mode, can be ${kTraceModes.map((mode) => `"${mode}"`).join(", ")}`,
  ],
  [
    "--tsconfig <path>",
  ...
  [
    ...
    `Chooses the way source is updated. Possible values are 'overwrite', '3way' and 'patch'. Defaults to 'patch'`,
  ],
  [
    "-j, --workers <workers>",
