    ...
    const visitSuite = (project, parentSuite, parentGroup) => {
      for (const suite of parentSuite.suites) {
        ...
        let group = parentGroup.children.find(
          (item) => item.kind === "group" && item.title === suite.title,
        );
        if (!group) {
          group = {
            kind: "group",
      ...
      for (const test of parentSuite.tests) {
        ...
        let status = "none";
        if (result?.[statusEx] === "scheduled") status = "scheduled";
        else if (result?.[statusEx] === "running") status = "running";
        ...
        testCaseItem.duration = testCaseItem.children.reduce(
          (a, b) => a + b.duration,
          0,
        );
      }
...
function sortAndPropagateStatus(treeItem) {
  ...
  for (const child of treeItem.children) {
    ...
    allPassed =
      allPassed && (child.status === "passed" || child.status === "skipped");
...
function collectTestIds(treeItem) {
  ...
  const visit = (treeItem2) => {
    ...
    else if (treeItem2.kind === "test") testIds.add(treeItem2.id);
    else treeItem2.children?.forEach(visit);
  };
  visit(treeItem);
  return testIds;
}
