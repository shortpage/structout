    ...
    if (expected)
      tokens.push(
        this.screen.colors.green(`  ${expected} passed`) +
          this.screen.colors.dim(
            ` (${(0, import_utilsBundle.ms)(this.result.duration)})`,
          ),
      );
    if (
      fatalErrors.length &&
    ...
    slowTests.forEach(([file, duration]) => {
      console.log(
        this.screen.colors.yellow("  Slow test file: ") +
          file +
          this.screen.colors.yellow(
            ` (${(0, import_utilsBundle.ms)(duration)})`,
          ),
      );
    });
    if (slowTests.length)
  ...
  for (const result of test.results) {
    ...
    for (let i = 0; i < result.attachments.length; ++i) {
      ...
      if (attachment.name.startsWith("_error-context") && attachment.path) {
        ...
        resultLines.push(
          screen.colors.dim(
            `    Error Context: ${relativeFilePath(screen, config, attachment.path)}`,
          ),
        );
        continue;
      }
...
function formatTestTitle(screen, config, test, step, omitLocation = false) {
  ...
  let location;
  if (omitLocation) location = `${relativeTestPath(screen, config, test)}`;
  else
...
function separator(screen, text = "") {
  ...
  return (
    text +
    screen.colors.dim("\u2500".repeat(Math.max(0, columns - text.length)))
  );
}
...
function prepareErrorStack(stack) {
  return (0, import_utils.parseErrorStack)(
    stack,
...
function stringWidth(v) {
  let width = 0;
  for (const { segment } of new Intl.Segmenter(void 0, {
...
function suffixOfWidth(v, width) {
  const segments = [
    ...new Intl.Segmenter(void 0, { granularity: "grapheme" }).segment(v),
  ];
...
function fitToWidth(line, width, prefix) {
  const prefixLength = prefix
    ? (0, import_util.stripAnsiEscapes)(prefix).length
    : 0;
  width -= prefixLength;
...
function resolveFromEnv(name) {
  ...
  if (value) return import_path.default.resolve(process.cwd(), value);
  return void 0;
}
function resolveOutputFile(reporterName, options) {
  ...
  if (!outputFile && options.outputFile)
    outputFile = import_path.default.resolve(
      options.configDir,
      options.outputFile,
    );
  ...
  if (!outputDir && options.outputDir)
    outputDir = import_path.default.resolve(
      options.configDir,
      options.outputDir,
    );
  if (!outputDir && options.default)
    outputDir = (0, import_util.resolveReporterOutputPath)(
