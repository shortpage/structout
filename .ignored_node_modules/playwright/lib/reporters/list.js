...
const POSITIVE_STATUS_MARK = DOES_NOT_SUPPORT_UTF8_IN_TERMINAL
  ? "ok"
  : "\u2713";
...
class ListReporter extends import_base.TerminalReporter {
  constructor(options = {}) {
    ...
    this._printSteps = (0, import_utils.getAsBooleanFromENV)(
  ...
  onStepBegin(test, result, step) {
    ...
    if (this._printSteps) {
      ...
      const line =
        test.title + this.screen.colors.dim((0, import_base.stepSuffix)(step));
      this._appendLine(line, prefix);
    } else {
      this._updateLine(
        this._testRows.get(test),
  ...
  onStepEnd(test, result, step) {
    ...
    text += this.screen.colors.dim(
      ` (${(0, import_utilsBundle.ms)(step.duration)})`,
    );
    ...
    if (result.status === "skipped") {
      ...
      text +=
        ...
        this.screen.colors.dim(
          ` (${(0, import_utilsBundle.ms)(result.duration)})`,
        );
    }
  ...
  _testPrefix(index, statusMark) {
    const statusMarkLength = (0, import_util.stripAnsiEscapes)(
      statusMark,
    ).length;
    return (
      "  " +
