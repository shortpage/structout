...
class JSONReporter {
  constructor(options) {
    this._errors = [];
    this._resolvedOutputFile = (0, import_base.resolveOutputFile)(
      "JSON",
      options,
    )?.outputFile;
  }
  ...
  _relativeLocation(location) {
    ...
    return {
      file: (0, import_utils.toPosixPath)(
        import_path.default.relative(this.config.rootDir, location.file),
      ),
      line: location.line,
    ...
    for (const spec of from.specs || []) {
      const toSpec = to.specs.find(
        (s) =>
          s.title === spec.title &&
          s.file ===
            (0, import_utils.toPosixPath)(
              import_path.default.relative(this.config.rootDir, spec.file),
            ) &&
          s.line === spec.line &&
  ...
  _serializeTestResult(result, test) {
    ...
    if (result.error?.stack)
      jsonResult.errorLocation = (0, import_base.prepareErrorStack)(
        result.error.stack,
      ).location;
    return jsonResult;
  }
...
async function outputReport(report, resolvedOutputFile) {
  ...
  if (resolvedOutputFile) {
    await import_fs.default.promises.mkdir(
      import_path.default.dirname(resolvedOutputFile),
      { recursive: true },
    );
