...
class GitHubLogger {
  _log(message, type = "notice", options = {}) {
    ...
    const configs = Object.entries(options)
      ...
      .join(",");
    console.log(
      (0, import_util.stripAnsiEscapes)(`::${type} ${configs}::${message}`),
    );
  }
  ...
  _printSlowTestAnnotations() {
    this.getSlowTests().forEach(([file, duration]) => {
      ...
      this.githubLogger.warning(
        `${filePath} took ${(0, import_utilsBundle.ms)(duration)}`,
        {
          title: "Slow Test",
          file: filePath,
        },
  ...
  _printFailureAnnotations(failures) {
    failures.forEach((test, index) => {
      ...
      for (const result of test.results) {
        const errors = (0, import_base.formatResultFailure)(
          this.screen,
          test,
          result,
          "    ",
        ...
        for (const error of errors) {
          const options = {
            file: workspaceRelativePath(
              error.location?.file || test.location.file,
            ),
            title,
          };
          if (error.location) {
          ...
          const message = [
            header,
            ...(0, import_base.formatRetry)(this.screen, result),
            error.message,
          ].join("\n");
