...
class BlobReporter extends import_teleEmitter.TeleReporterEmitter {
  ...
  async onEnd(result) {
    ...
    zipFile.outputStream
      .pipe(import_fs.default.createWriteStream(zipFileName))
      .on("close", () => {
  ...
  async _prepareOutputFile() {
    const { outputFile, outputDir } = (0, import_base.resolveOutputFile)(
      "BLOB",
      {
        ...this._options,
        default: {
    ...
    await import_fs.default.promises.mkdir(
      import_path.default.dirname(outputFile),
      { recursive: true },
    );
    return outputFile;
  }
  _defaultReportName(config) {
    ...
    if (this._options._commandHash)
      reportName +=
        "-" + (0, import_utils.sanitizeForFilePath)(this._options._commandHash);
    if (config.shard) {
      const paddedNumber = `${config.shard.current}`.padStart(
        `${config.shard.total}`.length,
        "0",
      );
  ...
  _serializeAttachments(attachments) {
    return super._serializeAttachments(attachments).map((attachment) => {
      ...
      const sha1 = (0, import_utils2.calculateSha1)(
        attachment.path + this._salt,
      );
      const extension =
        import_utilsBundle.mime.getExtension(attachment.contentType) || "dat";
