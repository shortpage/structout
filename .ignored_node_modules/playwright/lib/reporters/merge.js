...
async function createMergedReport(
  ...
  const reporters = await (0, import_reporters.createReporters)(
    config,
    "merge",
    false,
    reporterDescriptions,
  ...
  const pathSeparator = rootDirOverride
    ...
    : (eventData.pathSeparatorFromMetadata ?? import_path.default.sep);
  const receiver = new import_teleReceiver.TeleReporterReceiver(multiplexer, {
    mergeProjects: false,
    mergeTestCases: false,
    resolvePath: (rootDir, relativePath) =>
      stringPool.internString(rootDir + pathSeparator + relativePath),
  ...
  for (const { reportFile, eventPatchers, metadata } of eventData.reports) {
    ...
    new import_stringInternPool.JsonStringInternalizer(stringPool).traverse(
      events,
    );
...
async function extractAndParseReports(
  ...
  await import_fs.default.promises.mkdir(
    import_path.default.join(dir, "resources"),
    { recursive: true },
  );
  ...
  for (const file of shardFiles) {
    ...
    printStatus(
      `extracting: ${(0, import_util.relativeFilePath)(absolutePath)}`,
    );
...
async function mergeEvents(
  ...
  const internalizer = new import_stringInternPool.JsonStringInternalizer(
    stringPool,
  );
  ...
  for (let i = 0; i < blobs.length; ++i) {
    ...
    for (const event of parsedEvents) {
      if (event.method === "onConfigure") configureEvents.push(event);
      else if (event.method === "onProject") projectEvents.push(event);
      else if (event.method === "onEnd") endEvents.push(event);
    }
    reports.push({
      eventPatchers,
...
class AttachmentPathPatcher {
  ...
  patchEvent(event) {
    ...
    for (const attachment of event.params.result.attachments) {
      ...
      attachment.path = import_path.default.join(
        this._resourceDir,
        attachment.path,
      );
    }
...
class PathSeparatorPatcher {
  ...
  _updateSuite(suite, isFileSuite = false) {
    ...
    for (const entry of suite.entries) {
      if ("testId" in entry) this._updateLocation(entry.location);
      else this._updateSuite(entry);
    }
  }
...
class BlobModernizer {
  ...
  _modernize(fromVersion, event) {
    ...
    for (
      ...
      version < import_blob.currentBlobReportVersion;
      ++version
    )
