...
class JUnitReporter {
  constructor(options) {
    ...
    this.stripANSIControlSequences = (0, import_utils.getAsBooleanFromENV)(
      "PLAYWRIGHT_JUNIT_STRIP_ANSI",
      !!options.stripANSIControlSequences,
    );
    this.includeProjectInTestName = (0, import_utils.getAsBooleanFromENV)(
      "PLAYWRIGHT_JUNIT_INCLUDE_PROJECT_IN_TEST_NAME",
      !!options.includeProjectInTestName,
    );
    this.configDir = options.configDir;
    this.resolvedOutputFile = (0, import_base.resolveOutputFile)(
      "JUNIT",
      options,
    )?.outputFile;
  }
  ...
  async onEnd(result) {
    ...
    if (this.resolvedOutputFile) {
      await import_fs.default.promises.mkdir(
        import_path.default.dirname(this.resolvedOutputFile),
        { recursive: true },
      );
  ...
  async _addTestCase(suiteName, namePrefix, test, entries) {
    ...
    if (!test.ok()) {
      entry.children.push({
        ...
        text: (0, import_util.stripAnsiEscapes)(
          (0, import_base.formatFailure)(
            import_base.nonTerminalScreen,
            this.config,
            test,
          ),
        ),
    ...
    for (const result of test.results) {
      ...
      for (const attachment of result.attachments) {
        ...
        let attachmentPath = import_path.default.relative(
          this.configDir,
          attachment.path,
        );
        try {
          if (this.resolvedOutputFile)
            attachmentPath = import_path.default.relative(
              import_path.default.dirname(this.resolvedOutputFile),
              attachment.path,
            );
        } catch {
...
function escape(text, stripANSIControlSequences, isCharacterData) {
  if (stripANSIControlSequences) text = (0, import_util.stripAnsiEscapes)(text);
  if (isCharacterData) {
