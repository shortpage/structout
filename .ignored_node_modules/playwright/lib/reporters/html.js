...
`);
  ...
  _resolveOptions() {
    const outputFolder =
      reportFolderFromEnv() ??
      (0, import_util.resolveReporterOutputPath)(
        "playwright-report",
  ...
  _isSubdirectory(parentDir, dir) {
    ...
    return (
      ...
      !import_path.default.isAbsolute(relativePath)
    );
  }
  onError(error) {
    ...
    if (shouldOpen) {
      ...
      const relativeReportPath =
        this._outputFolder === standaloneDefaultFolder()
          ? ""
          : " " +
            import_path.default.relative(process.cwd(), this._outputFolder);
      const hostArg = this._host ? ` --host ${this._host}` : "";
      const portArg = this._port ? ` --port ${this._port}` : "";
      console.log("");
      console.log("To open last HTML report run:");
      console.log(
        import_utils2.colors.cyan(`
...
class HtmlBuilder {
  ...
  async build(metadata, projectSuites, result, topLevelErrors) {
    ...
    for (const projectSuite of projectSuites) {
      for (const fileSuite of projectSuite.suites) {
        ...
        const fileId = (0, import_utils.calculateSha1)(
          (0, import_utils.toPosixPath)(fileName),
        ).slice(0, 20);
    ...
    const htmlReport = {
      ...
      errors: topLevelErrors.map(
        (error) =>
          (0, import_base.formatError)(import_base.internalScreen, error)
            .message,
      ),
    };
    ...
    if (process.env.PW_HMR === "1") {
      const redirectFile = import_path.default.join(
        this._reportFolder,
        "index.html",
      );
      ...
      import_fs.default.appendFileSync(
        redirectFile,
        `<script>(${redirect.toString()})()</script>`,
      );
      return { ok, singleTestId };
    }
    const appFolder = import_path.default.join(
    ...
    await (0, import_utils.copyFileAndMakeWritable)(
      import_path.default.join(appFolder, "index.html"),
      import_path.default.join(this._reportFolder, "index.html"),
    );
    if (this._hasTraces) {
      ...
      const traceViewerTargetFolder = import_path.default.join(
        this._reportFolder,
        "trace",
      );
      ...
      import_fs.default.mkdirSync(traceViewerAssetsTargetFolder, {
        recursive: true,
      });
      ...
      for (const file of import_fs.default.readdirSync(
        import_path.default.join(traceViewerFolder, "assets"),
      )) {
        if (file.endsWith(".map") || file.includes("xtermModule")) continue;
        await (0, import_utils.copyFileAndMakeWritable)(
          import_path.default.join(traceViewerFolder, "assets", file),
    ...
    await new Promise((f) => {
      this._dataZipFile.end(void 0, () => {
        this._dataZipFile.outputStream
          ...
          .pipe(import_fs.default.createWriteStream(filePath, { flags: "a" }))
          .on("close", f);
      });
    });
  ...
  _processSuite(suite, projectName, path2, outTests) {
    ...
    suite.entries().forEach((e) => {
      ...
      else this._processSuite(e, projectName, newPath, outTests);
    });
  }
    ...
    return attachments
      .map((a) => {
        ...
        if (a.path) {
          ...
          try {
            ...
            const sha1 =
              ...
              import_path.default.extname(a.path);
            fileName = this._attachmentsBaseURL + sha1;
            import_fs.default.mkdirSync(
              import_path.default.join(this._reportFolder, "data"),
              { recursive: true },
            );
            import_fs.default.writeFileSync(
              import_path.default.join(this._reportFolder, "data", sha1),
              buffer,
            );
          } catch (e) {}
          return {
        ...
        if (a.body instanceof Buffer) {
          ...
          const extension =
            (0, import_utils.sanitizeForFilePath)(
              import_path.default.extname(a.name).replace(/^\./, ""),
            ) ||
            import_utilsBundle2.mime.getExtension(a.contentType) ||
            "dat";
          const sha1 =
            (0, import_utils.calculateSha1)(a.body) + "." + extension;
          import_fs.default.writeFileSync(
            import_path.default.join(this._reportFolder, "data", sha1),
            a.body,
          );
  ...
  _createTestResult(test, result) {
    return {
      ...
      errors: (0, import_base.formatResultFailure)(
  ...
  _relativeLocation(location) {
    ...
    const file = (0, import_utils.toPosixPath)(
      import_path.default.relative(this._config.rootDir, location.file),
    );
    return {
      file,
      line: location.line,
...
function createSnippets(stepsInFile) {
  for (const file of stepsInFile.keys()) {
    ...
    const highlighted = (0, import_babelBundle.codeFrameColumns)(
      source,
