...
async function gitDiff(gitDir, ci) {
  ...
  if (ci?.prBaseHash) {
    ...
    if (diff2) return diff2.substring(0, diffLimit);
  }
  if (ci) return;
...
async function runGit(command, cwd) {
  ...
  const result = await (0, import_utils.spawnAsync)(command, [], {
    stdio: "pipe",
    cwd,
  ...
  if ((0, import_utils.monotonicTime)() - start > GIT_OPERATIONS_TIMEOUT_MS) {
    print(
      `timeout of ${GIT_OPERATIONS_TIMEOUT_MS}ms exceeded while running "${command}"`,
    );
    return;
  }
  if (result.code)
