...
class WebServerPlugin {
  ...
  async setup(config, configDir, reporter) {
    ...
    this._isAvailableCallback = this._options.url
      ? getIsAvailableFunction(
          this._options.url,
          this._checkPortOnly,
  ...
  async _waitForProcess() {
    ...
    const { timedOut } = await Promise.race([
      (0, import_utils.raceAgainstDeadline)(
        () => waitFor(this._isAvailableCallback, cancellationToken),
        (0, import_utils.monotonicTime)() + launchTimeout,
      ),
...
function prefixOutputLines(output, prefixName = "WebServer") {
  ...
  lines = lines.map(
    (line) => import_utils2.colors.dim(`[${prefixName}] `) + line,
  );
